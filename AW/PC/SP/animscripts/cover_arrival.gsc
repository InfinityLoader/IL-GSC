 ...
 * Timestamp: 10/27/2023 1:54:44 AM
*******************************************************************/
...
startcoverapproach(param_00,param_01,param_02,param_03,param_04)
{
	...
	if(var_0A <= 6 && var_09)
	{
		...
		var_0A = var_06.approachnumber;
		var_0C = param_03 - animscripts\utility::lookuptransitionanim("cover_trans_angles",param_00,var_0A);
	}
	else
	{
		self setruntopos(self.coverenterpos);
		self waittill("runto_arrived");
		var_0C = param_04 - animscripts\utility::lookuptransitionanim("cover_trans_angles",param_01,var_0C);
...
dolastminuteexposedapproach()
{
	...
	else
	{
		...
		var_06 = var_06 && var_02.type != "Path" && var_02.type != "Path 3D" && var_02.type != "Ambush" || !animscripts\utility::recentlysawenemy();
		if(var_06)
		{
	...
	var_08 = spawnstruct();
	animscripts\exit_node::calculatenodetransitionangles(var_08,var_00,1,var_05,var_04,9,-1);
	var_09 = 1;
	...
	var_0B = animscripts\utility::lookuptransitionanim("cover_trans",var_00,self.approachnumber);
	if(!isdefined(var_0B) || isdefined(self.disableapproach))
	{
		return;
	}

	var_0C = length(animscripts\utility::lookuptransitionanim("cover_trans_dist",var_00,self.approachnumber));
	var_0D = var_0C + 8;
	var_0D = var_0D * var_0D;
	while(isdefined(self.pathgoalpos) && isdefined(var_0D) && distancesquared(self.origin,self.pathgoalpos) > var_0D)
	{
		wait(0.05);
	}
	...
	{
		var_10 = var_05 - animscripts\utility::lookuptransitionanim("cover_trans_angles",var_00,self.approachnumber);
		var_11 = getarrivalstartpos(self.pathgoalpos,var_05,var_00,self.approachnumber);
	}
	else if(var_0E > 0)
	{
		var_12 = animscripts\utility::lookuptransitionanim("cover_trans_dist",var_02,self.approachnumber);
