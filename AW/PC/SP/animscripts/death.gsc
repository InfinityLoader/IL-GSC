 ...
 * Timestamp: 10/27/2023 1:54:59 AM
*******************************************************************/
...
playdeathanim(param_00)
{
	...
	else if(!animhasnotetrack(param_00,"start_ragdoll"))
	{
		...
		thread waitforragdoll(getanimlength(param_00) * var_01);
	}
...
playdeathsound()
{
	if(!animscripts\utility::damagelocationisany("head","helmet"))
	{
		if(!self.diequietly)
		{
			animscripts\face::saygenericdialogue("pain_small");
			if(isdefined(self.team))
			{
				maps\_dds::dds_notify_mod(self.team != "allies");
			}
		}
	}
	else if(isdefined(self.team))
	{
		maps\_dds::dds_notify_mod(self.team != "allies","headshot");
	}
