 ...
 * Platform: PC
 * Function Count: 857
 * Decompile Time: 12829 ms
 * Timestamp: 4/22/2024 2:24:14 AM
*******************************************************************/
...
dds_set_player_character_name(param_00)
{
	...
	switch(param_00)
	{
		...
		case "hudson":
		case "mason":
			level.dds.player_character_name = getsubstr(param_00,0,3);
			break;
...
follow_path_and_animate(param_00,param_01)
{
	...
	while(isdefined(var_02))
	{
		...
		if(!isdefined(var_02.script_requires_player) && param_01 > 0 && self.team == "allies")
		{
			while(isalive(level.player))
			{
				if(follow_path_wait_for_player(var_02,param_01))
				{
					break;
				}
		...
		if(isdefined(var_02.script_flag_wait))
		{
			common_scripts\utility::flag_wait(var_02.script_flag_wait);
		}

		var_02 script_delay();
		var_02 = var_02 common_scripts\utility::get_target_ent();
	}
