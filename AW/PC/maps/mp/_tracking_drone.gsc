/*******************************************************************
 * Decompiled By: Bog
 * Decompiled File: mp\_tracking_drone.gsc
 * Game: Call of Duty: Advanced Warfare
 * Platform: PC
 * Function Count: 48
 * Decompile Time: 726 ms
 * Timestamp: 4/22/2024 2:16:23 AM
*******************************************************************/
...
createtrackingdrone(param_00,param_01,param_02,param_03,param_04)
{
	...
	if(!param_01)
	{
		...
		{
			var_0D = self.trackingdronestartposition;
			var_07 = self.trackingdronestartangles;
		}
		else
		{
	...
	var_10 = spawnhelicopter(var_0F,var_0D,var_07,level.trackingdronesettings.vehicleinfo,level.trackingdronesettings.modelbase);
	if(!isdefined(var_10))
	{
		return;
	}
	...
	{
		var_10.type = "explosive_drone";
		var_10 setmodel("vehicle_pdrone");
	}
	else
	{
		var_10.type = "tracking_drone";
	}

	var_10 common_scripts\utility::make_entity_sentient_mp(self.team);
	var_10 makevehiclenotcollidewithplayers(1);
	var_10 addtotrackingdronelist();
	var_10 thread removefromtrackingdronelistondeath();
	var_10.health = level.trackingdronesettings.health;
	var_10.maxhealth = level.trackingdronesettings.maxhealth;
	var_10.damagetaken = 0;
	var_10.speed = 20;
	var_10.followspeed = 20;
	var_10.owner = self;
	var_10.team = self.team;
	var_10 vehicle_setspeed(var_10.speed,10,10);
	var_10 setyawspeed(120,90);
	var_10 setneargoalnotifydist(64);
	var_10 sethoverparams(4,5,5);
	var_10.fx_tag0 = undefined;
	if(isdefined(var_10.type))
	{
		if(var_10.type == "tracking_drone")
		{
			var_10.fx_tag0 = "fx_joint_0";
		}
		else if(var_10.type == "explosive_drone")
		{
			var_10.fx_tag0 = "TAG_EYE";
		}
	}

	if(level.teambased)
	{
		var_10 maps\mp\_entityheadicons::setteamheadicon(self.team,(0,0,25),var_10.fx_tag0);
	}
	else
	{
		var_10 maps\mp\_entityheadicons::setplayerheadicon(self.owner,(0,0,25),var_10.fx_tag0);
	}

	var_10.maxtrackingrange = 2000;
	var_10.maxlaserrange = 300;
	var_10.trackedplayer = undefined;
	var_11 = 45;
	var_12 = 45;
	var_10 setmaxpitchroll(var_11,var_12);
	var_10.targetpos = var_0D;
	var_10.attract_strength = 10000;
	var_10.attract_range = 150;
	var_10.attractor = missile_createattractorent(var_10,var_10.attract_strength,var_10.attract_range);
	var_10.hasdodged = 0;
	var_10.stunned = 0;
	var_10.inactive = 0;
	var_10 thread maps\mp\gametypes\_damage::setentitydamagecallback(var_10.maxhealth,undefined,::ontrackingdronedeath,undefined,0);
	var_10 thread trackingdrone_watchdisable();
	var_10 thread trackingdrone_watchdeath();
	var_10 thread trackingdrone_watchownerloss();
	var_10 thread trackingdrone_watchownerdeath();
	var_10 thread trackingdrone_watchroundend();
	var_10 thread trackingdrone_watchhostmigration();
	if(!isdefined(level.ishorde))
	{
		var_10 thread trackingdrone_watchtimeout();
	}

	if(var_10.type == "tracking_drone")
	{
		var_10 thread trackingdrone_enemy_lightfx();
		var_10 thread trackingdrone_friendly_lightfx();
		var_10 thread drone_thrusterfx();
	}
...
trackingdrone_highlighttarget()
{
	...
	for(;;)
	{
		if(isdefined(self.trackedplayer))
		{
			...
			self.lasertag.angles = vectortoangles(self.trackedplayer.origin + (0,0,var_02 - 20) + var_01 - self.origin);
		}
		...
		if(isdefined(self.trackedplayer))
		{
			var_04 = bullettrace(self.origin,self.trackedplayer.origin,1,self);
			var_03 = var_04["entity"];
		}

		if(isdefined(self.trackedplayer) && self.trackedplayer != self.owner && isdefined(var_03) && var_03 == self.trackedplayer && distancesquared(self.origin,self.trackedplayer.origin) < self.maxlaserrange * self.maxlaserrange)
		{
