/*******************************************************************
 * Decompiled By: Bog
 * Decompiled File: mp\zombies\killstreaks\_zombie_sentry.gsc
 * Game: Call of Duty: Advanced Warfare
 * Platform: PC
 * Function Count: 98
 * Decompile Time: 1540 ms
 * Timestamp: 4/22/2024 2:13:15 AM
*******************************************************************/

//Function Number: 1
init()
{
	level.turretsdeployed = 0;
	level.turretsinworld = [];
	...
	level.killstreakfuncs["zm_sentry"] = ::tryuseremotemgsentryturret;
	level.killstreakfuncs["zm_disruptor"] = ::tryusedisruptorturret;
	...
	level.turretsettings["mg_turret"].timeout = 90;
...
//Function Number: 3
tryusedisruptorturret(param_00,param_01)
{
	if(maps\mp\zombies\_util::arekillstreaksdisabled())
	{
		return 0;
	...
	return tryuseremotemgsentryturret(param_00,["sentry_disruptor"]);
}
...
{
	if(level.turretsdeployed >= 4)
	{
		self iprintlnbold(&"ZOMBIES_SENTRY_TURRET_MAX");
		return 0;
	}

	if(param_03.size == 0)
	{
		param_03 = ["sentry_guardian","sentry_rippable"];
	}
	...
	if(isdefined(var_04))
	{
		var_05 = "ss_use_turret";
		if(common_scripts\utility::array_contains(param_03,"sentry_disruptor"))
		{
			var_05 = "ss_use_disruptor";
		}

		thread maps\mp\zombies\_zombies_audio::playerkillstreakcratevo(var_05);
		level.turretsdeployed++;
		level.turretsinworld[var_04 getentitynumber()] = var_04;
		return 1;
	}
...
setcarryingturret(param_00,param_01)
{
	...
	var_02 = maps\mp\_utility::getkillstreakweapon("zm_sentry");
	...
	thread translateturretinput("place_turret");
	thread translateturretinput("cancel_turret");
	thread translateturretinput("force_cancel_placement");
	for(;;)
	{
		self waittill("translated_turret_input",var_04);
		if(var_04 == "cancel_turret" || var_04 == "force_cancel_placement")
		{
			...
			self notify("end_translate_turret_input");
			return 0;
		}
...
{
	maps\mp\killstreaks\_killstreaks::givekillstreak("zm_ripped_turret",0,0,self,[param_00],param_01);
	if(!isdefined(self.pers["rippableSentry"]))
	{
		self.pers["rippableSentry"] = spawnstruct();
	}

	var_02 = maps\mp\killstreaks\_rippedturret::getammoforturretweapontype(param_00);
	maps\mp\killstreaks\_rippedturret::playerrecordrippableammo(var_02);
	if(!common_scripts\utility::is_player_gamepad_enabled())
	{
		var_03 = param_01 + 1;
		var_04 = "streakUsed" + var_03;
		self notify(var_04);
		waittillframeend;
	}
...
//Function Number: 16
setripoffturrethead(param_00,param_01)
{
	self endon("death");
	self endon("disconnect");
	...
	{
		thread playergiveturrethead("turretheadenergy_mp",param_01);
	}
	else if(param_00.rocketturret)
	{
		thread playergiveturrethead("turretheadrocket_mp",param_01);
	}
	...
	{
		thread playergiveturrethead("turretheadmg_mp",param_01);
	}
...
deleteaftertime(param_00)
{
	...
	{
		level.turretsdeployed--;
		level.turretsinworld[self getentitynumber()] = undefined;
		deathsoundsandfx();
		self delete();
	}
}
...
updateturretplacement(param_00)
{
	...
	for(;;)
	{
		...
		if(isdefined(var_03))
		{
			if(maps\mp\zombies\_util::is_true(var_03.noturretplacement))
			{
			...
			{
				param_00.placementlinkentity = var_03;
			}
		}
		else
		{
...
createturretforplayer(param_00,param_01,param_02,param_03)
{
	...
	var_05.canremotecontrol = 0;
	var_05.linegunignore = 1;
...
turret_setactive()
{
	...
	if(!isdefined(self.aiclip) && isdefined(level.sentry_ai_clip))
	{
		self.aiclip = spawn("script_model",self.origin);
		self.aiclip setmodel("tag_origin");
		self.aiclip clonebrushmodeltoscriptmodel(level.sentry_ai_clip);
		self.aiclip vehicle_jetbikesethoverforcescale(self,"tag_origin",(0,0,0),(0,0,0));
	}
	else if(isdefined(self.aiclip))
	{
		self.aiclip solid();
	}
	...
	if(level.teambased)
	{
		...
		var_01 = game["entity_headicon_allies"];
		game["entity_headicon_allies"] = game["entity_headicon_axis"];
		maps\mp\_entityheadicons::setteamheadicon(self.team,(0,0,65),"tag_origin");
		game["entity_headicon_allies"] = var_01;
	}
	else
	{
...
turret_modifydamage(param_00,param_01,param_02,param_03)
{
	...
	if(isdefined(param_01) && param_01 == "remote_energy_turret_mp" || param_01 == "sentry_minigun_mp" || param_01 == "killstreakmahem_mp")
	{
		var_04 = -1;
	}
	...
	if(isdefined(self.owner) && self.owner.using_remote_turret && var_04 > 0)
	{
...
waittillremoteturretleavereturn()
{
	...
	for(;;)
	{
		...
		{
			var_01 = var_01 + 0.05;
			if(var_01 > 1 && var_00 playercanuseturret(self))
			{
				return;
			}
	...
	for(;;)
	{
		...
		if(maps\mp\_utility::isreallyalive(self) && self istouching(param_00.ownertrigger) && !isdefined(param_00.carriedby) && self isonground())
		{
			if(self usebuttonpressed())
			{
				...
				var_02 = maps\mp\zombies\killstreaks\_zombie_killstreaks::getslotnumber(self);
				if(!isdefined(var_02))
				{
					maps\mp\zombies\killstreaks\_zombie_killstreaks::showstreaklimitreached(self);
					continue;
				}

				param_00 setmode(level.turretsettings[param_00.turrettype].sentrymodeoff);
				param_00 sentry_stopattackingtargets();
				thread setripoffturrethead(param_00,var_02);
				param_00 turret_clearpickuphints();
				param_00.pickupent delete();
				param_00.ownertrigger delete();
				return;
...
turret_setinactive()
{
	...
	if(isdefined(self.aiclip))
	{
		self.aiclip notsolid();
	}
}
...
turret_timeout()
{
	...
	thread turret_blinkheadiconbeforedeath(var_00);
	while(var_00)
	{
		wait(1);
...
//Function Number: 59
playerownsanotherturret()
{
	foreach(var_01 in level.turretsinworld)
	{
		if(isdefined(var_01) && isdefined(var_01.owner) && var_01.owner == self)
		{
			return 1;
		}
	}
...
turret_handledeath()
{
	...
	turret_deathsounds(var_00,var_02);
	...
	{
		stopusingremoteturret();
		var_03.using_remote_turret = 0;
		var_03.turret = undefined;
		if(!var_03 playerownsanotherturret())
		{
			var_03 playerremovenotifycommands();
		}

		if(var_03 getcurrentweapon() == "none")
		{
			var_04 = var_03 common_scripts\utility::getlastweapon();
			if(isdefined(var_03.underwater) && var_03.underwater)
			{
				var_04 = var_03 maps\mp\_utility::get_water_weapon();
			}

			var_03 switchtoweapon(var_04);
		}
	}
...
turret_deathsounds(param_00,param_01)
{
	...
	{
		self.owner thread maps\mp\zombies\_zombies_audio::playerkillstreakcratevo("ss_zom_destroy_turret");
	}
}
...
turret_firerocket(param_00)
{
	...
	{
		var_06 = isdefined(var_05) && issentient(var_05) && !isalliedsentient(self.owner,var_05);
	}
	...
	var_07 = magicbullet("killstreakmahem_mp",var_01,var_03,self.owner,1);
	...
	if(!param_00)
	{
		...
		{
			if(var_0A != self.owner)
			{
				playfxontagforclients(common_scripts\utility::getfx("sentry_rocket_muzzleflash_wv"),self,"tag_flash",var_0A);
			}
		}
...
//Function Number: 87
turret_blinkheadiconbeforedeath(param_00)
{
	var_01 = 8;
	...
	while(gettime() < var_03)
	{
		...
		{
			if(self.entityheadicon.alpha == 1)
			{
				self.entityheadicon.alpha = 0;
				continue;
			}

			self.entityheadicon.alpha = 1;
		}
...
//Function Number: 88
translateturretinput(param_00)
{
	self endon("end_translate_turret_input");
	for(;;)
	{
		self waittill(param_00);
		self notify("translated_turret_input",param_00);
	}
}
...
sentry_disruptor()
{
	...
	for(;;)
	{
		...
		foreach(var_04 in var_02)
		{
			...
			if(!turretdisruptorcanhurtzombie(var_04,var_00,var_01))
			{
				continue;
			}

			if(common_scripts\utility::array_contains(self.radiationlist,var_04))
			{
				continue;
			}

			self.radiationlist = common_scripts\utility::array_add(self.radiationlist,var_04);
			thread turretdisruptorhurtzombie(var_04);
		}
...
//Function Number: 92
turretdisruptorcanhurtzombie(param_00,param_01,param_02)
{
...
//Function Number: 93
turretdisruptorhurtzombie(param_00)
{
	self endon("death");
	var_01 = 0.2;
	var_02 = 16;
	var_03 = gettime();
	var_04 = 0.05;
	for(var_05 = 1;var_05;var_05 = turretdisruptorcanhurtzombie(param_00,var_06,var_07))
	{
		var_06 = self gettagorigin("tag_flash");
		var_07 = anglestoforward(self gettagangles("tag_flash"));
		if(self.stunned)
		{
			while(self.stunned)
			{
			...
			var_05 = turretdisruptorcanhurtzombie(param_00,var_06,var_07);
			continue;
		}
		...
		if(gettime() >= var_03)
		{
			...
			param_00 maps\mp\zombies\_zombies::addbuff("disruptorBuff",param_00 getdisruptorbuff(self.owner));
		}

		var_08 = param_00 common_scripts\utility::waittill_notify_or_timeout_return("death",0.05);
		if(!isdefined(var_08) || var_08 != "timeout")
		{
...
//Function Number: 94
getdisruptorbuff(param_00)
{
	var_01 = maps\mp\zombies\_zombies::getbuff("disruptorBuff");
	if(!isdefined(var_01))
	{
		var_01 = spawndisruptorbuff();
	}
...
spawndisruptorbuff()
{
	...
	var_01 = 0.4;
	var_02 = spawnstruct();
	var_02.buffupdate = ::updatedisruptorbuff;
	var_02.buffremove = ::removedisruptorbuff;
	var_02.lifespan = 0.2;
	var_02.damageperstep = var_00 * maps\mp\zombies\_zombies::getbufftimestep();
	var_02.speedmultiplier = var_01;
	self notify("speed_debuffs_changed");
	return var_02;
}

//Function Number: 96
updatedisruptorbuff(param_00)
{
	...
	self dodamage(param_00.damageperstep,self.origin,var_01,undefined,"MOD_TRIGGER_HURT","remote_energy_turret_mp","none");
	if(!isdefined(self.isturretdisruptoremp) && maps\mp\agents\humanoid\_humanoid_util::has_humanoid_exo_abilities())
	{
		maps\mp\agents\humanoid\_humanoid_util::disable_humanoid_exo_abilities();
		self.isturretdisruptoremp = 1;
	}
}

//Function Number: 97
removedisruptorbuff(param_00)
{
	self notify("speed_debuffs_changed");
	if(isdefined(self.isturretdisruptoremp) && maps\mp\agents\humanoid\_humanoid_util::has_humanoid_exo_abilities())
	{
	...
	self.isturretdisruptoremp = undefined;
}
