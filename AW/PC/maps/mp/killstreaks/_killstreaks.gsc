/*******************************************************************
 * Decompiled By: Bog
 * Decompiled File: mp\killstreaks\_killstreaks.gsc
 * Game: Call of Duty: Advanced Warfare
 * Platform: PC
 * Function Count: 79
 * Decompile Time: 1204 ms
 * Timestamp: 4/22/2024 2:11:54 AM
*******************************************************************/
...
killstreakusewaiter()
{
	...
	for(;;)
	{
		...
		if(shouldswitchweaponpostkillstreak(var_09,var_02,var_03,var_05) && !isdefined(self.justswitchedtokillstreakweapon))
		{
			...
			{
				if(!isdefined(level.ishorde) || isdefined(level.ishorde) && level.ishorde && !level.hordeweaponsjammed && issubstr(var_02,"turrethead"))
				{
					maps\mp\_utility::switch_to_last_weapon(var_06);
				}
			}
			else
			{
				self.water_last_weapon = var_06;
			}
		}
		...
		if(isdefined(level.cb_usedkillstreak) && var_09)
		{
			[[ level.cb_usedkillstreak ]](var_03,var_04,var_07);
		}
	}
}
...
switchweaponafterraiseanimation(param_00,param_01)
{
	...
	maps\mp\_utility::switch_to_last_weapon(param_01);
}
...
shouldswitchweaponpostkillstreak(param_00,param_01,param_02,param_03)
{
	...
	{
		case "warbird":
			return common_scripts\utility::array_contains(param_03,"warbird_ai_attack") || common_scripts\utility::array_contains(param_03,"warbird_ai_follow");

		case "zm_ugv":
		case "assault_ugv":
			return common_scripts\utility::array_contains(param_03,"assault_ugv_ai");
	}

	if(maps\mp\_utility::isridekillstreak(param_02))
	{
		return 0;
	}
...
getkillstreakcrateicon(param_00,param_01)
{
	...
	return tablelookup(level.killstreak_string_table,1,param_00,var_02);
}
	...
	for(;;)
	{
		...
		if(isdefined(self.killstreakindexweapon))
		{
			...
			for(;;)
			{
				...
				{
					var_02 = maps\mp\_utility::getkillstreakweapon(self.pers["killstreaks"][self.killstreakindexweapon].streakname,self.pers["killstreaks"][self.killstreakindexweapon].modules);
					if(var_01 == var_02 || var_01 == "none" || var_02 == "killstreak_uav_mp" && var_01 == "uav_remote_mp" || var_02 == "killstreak_recreation_mp" && var_01 == "uav_remote_mp")
					{
						continue;
					}
