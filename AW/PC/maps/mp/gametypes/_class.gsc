/*******************************************************************
 * Decompiled By: Bog
 * Decompiled File: mp\gametypes\_class.gsc
 * Game: Call of Duty: Advanced Warfare
 * Platform: PC
 * Function Count: 92
 * Decompile Time: 1397 ms
 * Timestamp: 4/22/2024 2:09:10 AM
*******************************************************************/
...
table_getkillstreakmodule(param_00,param_01,param_02,param_03)
{
	...
	{
		case 0:
			return tablelookup(param_00,0,"loadoutStreakModule" + param_02 + 1 + "a",param_01 + 1);

		case 1:
			return tablelookup(param_00,0,"loadoutStreakModule" + param_02 + 1 + "b",param_01 + 1);

		case 2:
			return tablelookup(param_00,0,"loadoutStreakModule" + param_02 + 1 + "c",param_01 + 1);
...
loadouttrackvariablegrenades(param_00,param_01,param_02)
{
	...
	switch(param_01)
	{
		...
		case "emp_grenade_var_mp":
		case "stun_grenade_var_mp":
			if(isdefined(self.prevlethalvarclass) && self.prevlethalvarclass == param_00 && isdefined(self.prevlethalvartype) && self.prevlethalvartype != param_01)
			{
				var_03 = self.prevlethalvartype;
				self takeweapon(param_01);
				self.loadoutequipment = var_03;
				self setlethalweapon(var_03);
				maps\mp\_utility::giveperk(var_03,0);
				param_01 = var_03;
			}
	
			self.prevlethalvarclass = param_00;
			self.prevlethalvartype = param_01;
			break;
	...
	var_04 = maps\mp\_utility::strip_suffix(param_02,"_lefthand");
	switch(var_04)
	{
		case "contact_grenade_var_mp":
		case "semtex_grenade_var_mp":
		...
		case "emp_grenade_var_mp":
		case "stun_grenade_var_mp":
			if(isdefined(self.prevtacticalvarclass) && self.prevtacticalvarclass == param_00 && isdefined(self.prevtacticalvartype) && self.prevtacticalvartype != param_02)
			{
				var_05 = self.prevtacticalvartype;
				self takeweapon(param_02);
				self.loadoutoffhand = var_05;
				self settacticalweapon(var_05);
				maps\mp\_utility::giveperk(var_05,0);
				param_02 = var_05;
			}
	
			self.prevtacticalvarclass = param_00;
			self.prevtacticalvartype = param_02;
			break;
	...
	for(;;)
	{
		self waittill("switched_var_grenade",var_06);
		if(issubstr(var_06,"_lefthand"))
		{
			self.prevtacticalvartype = var_06;
			continue;
		}

		self.prevlethalvartype = var_06;
	}
}
...
isvalidattachment(param_00,param_01,param_02)
{
	...
	{
		var_04 = maps\mp\_utility::getweaponattachmentarrayfromstats(param_01);
