/*******************************************************************
 * Decompiled By: Bog
 * Decompiled File: mp\gametypes\_hud_message.gsc
 * Game: Call of Duty: Advanced Warfare
 * Platform: PC
 * Function Count: 33
 * Decompile Time: 679 ms
 * Timestamp: 4/22/2024 2:09:51 AM
*******************************************************************/
...
actionnotify(param_00)
{
	...
	if(param_00.type == "challenge_splash" || param_00.type == "killstreak_splash" || param_00.type == "killstreak_coop_splash")
	{
...
actionnotifymessage(param_00)
{
	...
	if(tablelookup(get_table_name(),0,param_00.name,0) != "")
	{
		...
		switch(param_00.type)
		{
			case "killstreak_splash":
			case "killstreak_coop_splash":
				if(isdefined(param_00.killstreakslot) && !level.console)
				{
					self setclientomnvar("ui_splash_killstreak_slot_idx",param_00.killstreakslot);
				}
				...
				if(isdefined(param_00.module1idx))
				{
					self setclientomnvar("ui_splash_killstreak_mod_1",param_00.module1idx);
				}
				else
				{
					self setclientomnvar("ui_splash_killstreak_mod_1",-1);
				...
				if(isdefined(param_00.module2idx))
				{
					self setclientomnvar("ui_splash_killstreak_mod_2",param_00.module2idx);
				}
				else
				{
					self setclientomnvar("ui_splash_killstreak_mod_2",-1);
				...
				if(isdefined(param_00.module3idx))
				{
					self setclientomnvar("ui_splash_killstreak_mod_3",param_00.module3idx);
				}
				else
				{
					self setclientomnvar("ui_splash_killstreak_mod_3",-1);
			...
			case "playercard_splash":
				if(isdefined(param_00.playercardplayer))
				{
					...
					if(isplayer(param_00.playercardplayer))
					{
			...
			case "rankup_splash":
				self setclientomnvar("ui_rankup_splash_idx",var_02);
				if(isdefined(param_00.rank))
				{
					self setclientomnvar("ui_rank_splash_rank",param_00.rank);
				}
	
				if(isdefined(param_00.prestige))
				{
					self setclientomnvar("ui_rank_splash_prestige",param_00.prestige);
				}
				break;

			case "perk_challenge_splash":
			case "challenge_splash":
				var_02 = int(tablelookup("mp/allchallengestable.csv",0,param_00.name,27));
				self setclientomnvar("ui_challenge_splash_idx",var_02);
				if(isdefined(param_00.challengetier))
				{
					self setclientomnvar("ui_challenge_splash_tier",param_00.challengetier);
				}
