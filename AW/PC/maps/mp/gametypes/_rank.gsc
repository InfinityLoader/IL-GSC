/*******************************************************************
 * Decompiled By: Bog
 * Decompiled File: mp\gametypes\_rank.gsc
 * Game: Call of Duty: Advanced Warfare
 * Platform: PC
 * Function Count: 28
 * Decompile Time: 435 ms
 * Timestamp: 4/22/2024 2:10:15 AM
*******************************************************************/
...
giverankxp(param_00,param_01,param_02,param_03,param_04,param_05)
{
	...
	switch(param_00)
	{
		...
		default:
			...
			{
				var_08 = max(1,int(10 / maps\mp\_utility::getgametypenumlives()));
			...
			if(level.xpscale > 1)
			{
				var_06 = int(var_06 * level.xpscale);
			}
			...
			if(isdefined(level.nukedetonated) && level.nukedetonated)
			{
				...
				{
					var_06 = var_06 * level.nukeinfo.xpscalar;
				}
			...
			{
				var_09 = int(var_06 + param_01 * self.costumebonus["xp"]["bonusTotal"]);
				var_06 = var_09;
			}
	...
	var_0A = int(var_06 * var_07);
	var_0B = getrankxp();
	incrankxp(var_06 + var_0A);
	if(maps\mp\_utility::rankingenabled() && updaterank(var_0B))
	{
		thread updaterankannouncehud();
	}

	syncxpstat();
	var_0C = maps\mp\gametypes\_missions::isweaponchallenge(param_04);
	if(var_0C)
	{
	...
	self.pers["summary"]["clanWarsXP"] = self.pers["summary"]["clanWarsXP"] + var_0A;
	self.pers["summary"]["xp"] = self.pers["summary"]["xp"] + var_06 + var_0A;
	switch(param_00)
	{
		case "loss":
		case "win":
		case "tie":
			self.pers["summary"]["match"] = self.pers["summary"]["match"] + var_06;
			break;

		case "challenge":
			self.pers["summary"]["challenge"] = self.pers["summary"]["challenge"] + var_06;
			break;

		default:
			if(isregisteredevent(param_00))
			{
				self.pers["summary"]["score"] = self.pers["summary"]["score"] + var_06;
			}
			else
			{
				self.pers["summary"]["misc"] = self.pers["summary"]["misc"] + var_06;
			}
			break;
	}
}
