 ...
 * Platform: PC
 * Function Count: 80
 * Decompile Time: 1228 ms
 * Timestamp: 4/22/2024 2:11:10 AM
*******************************************************************/
...
updateminions()
{
	...
	while(!level.gameended)
	{
		...
		if(var_0D + var_0E < var_00)
		{
			...
			foreach(var_14 in level.twar_zones)
			{
				var_11 = var_14.owner;
				...
				if(isdefined(var_17))
				{
					...
					switch(var_18)
					{
						...
						case 0:
						default:
							var_19 = "iw5_titan45_mp";
							break;
					}

					var_17 giveweapon(var_19);
					var_17 switchtoweaponimmediate(var_19);
					var_17 maps\mp\_utility::giveperk("specialty_minion",0);
					var_17.movespeedscaler = getdvarfloat("scr_twar_minionmovespeedscale",0.85);
					var_17.damage_scale = getdvarfloat("scr_twar_miniondamagescale",0.5);
					var_17.agentname = &"MP_MINION";
					var_17.nonkillstreakagent = 1;
					var_17 thread minion_ai();
					update_minion_hud_outlines();
					var_17 detachall();
					var_17 setmodel("kva_hazmat_body_infected_mp");
					var_17 attach("kva_hazmat_head_infected");
					var_17 setclothtype("cloth");
					var_1A = getdvarfloat("scr_twar_minionhealthscale",0.75);
					var_17 maps\mp\agents\_agent_common::set_agent_health(int(var_17.health * var_1A));
					var_0F[var_11]++;
				}
			}
			...
			foreach(var_1D in var_03)
			{
				...
				{
					var_1F hud_set_visible();
					var_1F setvalue(var_0F[var_0A]);
					var_1F maps\mp\_utility::delaythread(3,::hud_set_invisible);
				}
			}
		...
		if(isdefined(var_05))
		{
			var_05 hud_set_visible();
			var_05 maps\mp\_utility::delaythread(3,::hud_set_invisible);
		}
	}
}
