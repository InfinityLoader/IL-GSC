/*******************************************************************
 * Decompiled By: Bog
 * Decompiled File: mp\gametypes\_damage.gsc
 * Game: Call of Duty: Advanced Warfare
 * Platform: PC
 * Function Count: 98
 * Decompile Time: 1540 ms
 * Timestamp: 4/22/2024 2:09:15 AM
*******************************************************************/
...
dofinalkillcam()
{
	...
	waitforstream(var_03);
	var_0F = gettime() - var_02.deathtime / 1000;
	foreach(var_11 in level.players)
	{
		var_11 maps\mp\_utility::revertvisionsetforplayer(0);
		var_11 setblurforplayer(0,0);
		var_11.killcamentitylookat = var_02 getentitynumber();
		if(isdefined(var_03) && isdefined(var_03.lastspawntime))
		{
			var_12 = gettime() - var_03.lastspawntime / 1000;
		}
		else
		{
			var_12 = 0;
		...
		var_11 thread maps\mp\gametypes\_killcam::killcam(var_03,var_04,var_05,var_06,var_08,var_0F + var_09,var_0A,0,getkillcambuffertime(),var_03,var_02,var_0D,var_0E,var_12,var_07);
	}
...
getkillcamentity(param_00,param_01,param_02)
{
	...
	switch(param_02)
	{
		...
		case "assaultdrone_c4_mp":
			if(isdefined(param_01.hasaioption) && param_01.hasaioption)
			{
				return param_01;
			}
			else
		...
		case "killstreak_solar_mp":
			if(isdefined(param_01) && isdefined(param_01.killcament))
			{
				return param_01.killcament;
			}
			break;
		...
		case "sentry_minigun_mp":
			if(isdefined(param_01) && isdefined(param_01.remotecontrolled))
			{
				return undefined;
			}
			break;
	...
	if(maps\mp\_utility::isdestructibleweapon(param_02) || maps\mp\_utility::isbombsiteweapon(param_02))
	{
	...
	if(maps\mp\gametypes\_killcam::isworldkillcam(param_01,param_00))
	{
		return param_00.killcament;
	}

	if(!isdefined(param_01) || param_00 == param_01 && !isagent(param_00))
	{
		return undefined;
	}
...
processdamagetaken(param_00,param_01,param_02,param_03,param_04,param_05,param_06,param_07,param_08,param_09,param_0A,param_0B)
{
	...
	if(isdefined(param_05))
	{
		...
		if(isdefined(level.ishorde) && level.ishorde)
		{
			self.damageloc = param_0B;
		}

		var_0C = [[ self.modifydamagefunc ]](param_01,param_05,param_04,param_02);
		if(isdefined(level.ishorde) && level.ishorde)
		{
	...
	if(isdefined(param_01) && isplayer(param_01))
	{
		param_01 maps\mp\gametypes\_damagefeedback::updatedamagefeedback(self.damagefeedback);
		if(isnewattacker(param_01))
		{
			self.attackerlist[self.attackerlist.size] = param_01;
		}
	}

	if(self.damagetaken >= self.maxhealth)
	{
		if(self.biskillstreak && isplayer(param_01))
		{
		...
		{
			var_0E = maps\mp\_utility::getbaseweaponname(param_05,1);
			if(maps\mp\_utility::islootweapon(var_0E))
			{
				var_0E = maps\mp\gametypes\_class::getbasefromlootversion(var_0E);
			}

			var_0F = maps\mp\gametypes\_missions::get_challenge_weapon_class(param_05,var_0E);
			if(isdefined(self.model) && var_0F == "weapon_launcher")
			{
				if(issubstr(self.model,"uav"))
				{
					if(isdefined(level.challengeinfo["ch_uav_" + var_0E]))
					{
						param_01 maps\mp\gametypes\_missions::processchallenge("ch_uav_" + var_0E);
					}
				}

				if(issubstr(self.model,"warbird"))
				{
					if(isdefined(level.challengeinfo["ch_warbird_" + var_0E]))
					{
						param_01 maps\mp\gametypes\_missions::processchallenge("ch_warbird_" + var_0E);
					}
				}

				if(issubstr(self.model,"orbital_platform"))
				{
					if(isdefined(level.challengeinfo["ch_paladin_" + var_0E]))
					{
						param_01 maps\mp\gametypes\_missions::processchallenge("ch_paladin_" + var_0E);
					}
				}

				if(issubstr(self.model,"drone") && !issubstr(self.model,"uav"))
				{
					if(isdefined(level.challengeinfo["ch_drone_" + var_0E]))
					{
						param_01 maps\mp\gametypes\_missions::processchallenge("ch_drone_" + var_0E);
					}
				}
			}
		}
		...
		self thread [[ self.ondeathfunc ]](param_01,param_05,param_04,param_02);
	}
}
...
handlemissiledamage(param_00,param_01,param_02)
{
	...
	switch(param_00)
	{
		...
		case "killstreak_laser2_mp":
			self.largeprojectiledamage = 1;
			var_04 = level.sentrygun.ownerlist;
			var_05 = 0.34;
