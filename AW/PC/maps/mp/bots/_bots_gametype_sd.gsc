/*******************************************************************
 * Decompiled By: Bog
 * Decompiled File: mp\bots\_bots_gametype_sd.gsc
 * Game: Call of Duty: Advanced Warfare
 * Platform: PC
 * Function Count: 44
 * Decompile Time: 698 ms
 * Timestamp: 4/22/2024 2:08:44 AM
*******************************************************************/
...
setup_bot_sd()
{
	...
	if(!var_00)
	{
		maps\mp\bots\_bots_gametype_common::bot_cache_entrances_to_bombzones();
...
atk_bomber_update()
{
	...
	if(level.attack_behavior == "rush")
	{
		...
		{
			var_10 = common_scripts\utility::random(var_0B);
		}
...
bomb_defuser_update()
{
	...
	if(var_04 == "goal")
	{
		...
		var_0D = sd_press_use(level.defusetime + 2,"bomb_defused",var_0B,var_0C > 40);
...
investigate_someone_using_bomb_update()
{
	...
	self botsetscriptgoalnode(common_scripts\utility::random(var_00.bottargets),"guard");
...
//Function Number: 23
sd_press_use(param_00,param_01,param_02,param_03)
{
	var_04 = 0;
...
{
	var_01 = get_living_players_on_team(common_scripts\utility::get_enemy_team(self.team),1);
				...
				if(isdefined(var_05))
				{
					...
					foreach(var_09 in var_07)
					{
						...
						if(var_0B > 0 && var_09.role != "camp_bomb" && isdefined(var_0A) && nodesvisible(var_05,var_0A,1))
						{
							...
							if(common_scripts\utility::within_fov(var_09.origin,var_09.angles,level.sdbomb.curorigin,var_0C))
							{
