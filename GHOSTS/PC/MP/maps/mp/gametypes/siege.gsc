 ...
 * Timestamp: 10/27/2023 1:19:56 AM
*******************************************************************/
		...
		if(var_00 == 2 || var_01 == 2)
		{
			if(getdvarint("scr_siege_rushtimer"))
			{
				if(level.siegeflagcapturing.size == 0 && !var_02.flagcapsuccess || !isdefined(level.siegetimerstate) || level.siegetimerstate != "start")
				{
					...
					var_04 = int(gettime() + var_03 * 1000);
...
showcapturedbaseeffecttoplayer(param_00,param_01)
{
	...
	if(var_03 == param_00)
	{
		var_02 = spawnfxforclient(level.flagbasefxid["friendly"],self.baseeffectpos,param_01,self.baseeffectforward);
	}
	else
	{
		var_02 = spawnfxforclient(level.flagbasefxid["enemy"],self.baseeffectpos,param_01,self.baseeffectforward);
	}
