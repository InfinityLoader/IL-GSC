 ...
 * Timestamp: 10/27/2023 1:19:29 AM
*******************************************************************/
...
assignteamspawns()
{
	...
	if(getdvarint("scr_altBlitzSpawns",0) == 1 && level.portallist.size == 2)
	{
		...
		foreach(var_09 in var_00)
		{
			...
			if(var_01)
			{
				var_0E = getpathdist(var_09.origin,var_02.origin,999999);
			}

			if(isdefined(var_0E) && var_0E != -1)
			{
				var_0F = getpathdist(var_09.origin,var_03.origin,999999);
			}
...
func_420A(param_00)
{
	foreach(var_02 in level.players)
	{
		...
		var_03 = var_02 ismlgspectator();
		if((var_03 && param_00 == var_02 getmlgspectatorteam()) || !var_03 && param_00 == "allies")
		{
			if(isdefined(var_02.defend_fx_ent))
			{
				var_02.defend_fx_ent delete();
	...
	foreach(var_02 in level.players)
	{
		...
		if(var_02.team == "allies" || var_02.team == "axis")
		{
			if(var_02.team == param_00)
			{
				if(!isdefined(var_02.defend_fx_ent))
				{
					var_02.defend_fx_ent = spawnfxforclient(level._effect["portal_fx_defend"],self.origin,var_02);
					triggerfx(var_02.defend_fx_ent);
				}
			}
			else if(!isdefined(var_02.goal_fx_ent))
			{
				var_02.goal_fx_ent = spawnfxforclient(level._effect["portal_fx_goal"],self.origin,var_02);
	...
	foreach(var_02 in level.players)
	{
		...
		if(var_02.team == "allies" || var_02.team == "axis")
		{
			if(var_02.team == param_00)
			{
				if(!isdefined(var_02.closed_defend_fx_ent))
				{
					var_02.closed_defend_fx_ent = spawnfxforclient(level._effect["portal_fx_closed"],self.origin,var_02);
					triggerfx(var_02.closed_defend_fx_ent);
				}
			}
			else if(!isdefined(var_02.closed_goal_fx_ent))
			{
				var_02.closed_goal_fx_ent = spawnfxforclient(level._effect["portal_fx_closed"],self.origin,var_02);
...
func_7526()
{
	...
	if(isdefined(level.portallist[var_00]) && isdefined(level.portallist[var_01]))
	{
		...
		{
			self.defend_fx_ent = spawnfxforclient(level._effect["portal_fx_defend"],level.portallist[var_00].origin,self);
			triggerfx(self.defend_fx_ent);
		}
		else
		{
			self.closed_defend_fx_ent = spawnfxforclient(level._effect["portal_fx_closed"],level.portallist[var_00].origin,self);
			triggerfx(self.closed_defend_fx_ent);
		}

		if(level.portallist[var_01].open && !level.portallist[var_01].guarded)
		{
			self.goal_fx_ent = spawnfxforclient(level._effect["portal_fx_goal"],level.portallist[var_01].origin,self);
			triggerfx(self.goal_fx_ent);
		}

		self.closed_goal_fx_ent = spawnfxforclient(level._effect["portal_fx_closed"],level.portallist[var_01].origin,self);
...
func_9493()
{
	...
	var_00 = "allies";
	var_01 = "axis";
	var_02 = self ismlgspectator();
	if(var_02)
	{
		var_00 = self getmlgspectatorteam();
		var_01 = maps\mp\_utility::getotherteam(var_00);
	}
...
onplayerkilled(param_00,param_01,param_02,param_03,param_04,param_05,param_06,param_07,param_08,param_09)
{
	...
	var_10 = 0;
	var_11 = 90000;
	if(distance2dsquared(var_0A.origin,var_0D) < var_11)
	{
		var_10++;
	}

	if(distance2dsquared(var_0A.origin,var_0E) < var_11)
	{
		var_0F++;
	}
