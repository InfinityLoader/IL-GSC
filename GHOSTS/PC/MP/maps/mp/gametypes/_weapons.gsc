 ...
 * Platform: PC
 * Function Count: 176
 * Decompile Time: 3140 ms
 * Timestamp: 10/27/2023 1:19:26 AM
*******************************************************************/
...
{
	self hudoutlineenableforclient(param_00,6,1);
}
...
updatetogglescopestate(param_00)
{
	...
	{
		self sethybridscopestate(param_00,self.pers["toggleScopeStates"][param_00]);
	}
}
...
func_8D37(param_00)
{
	...
	for(;;)
	{
		...
		if(!issquadsmode())
		{
			...
			maps\mp\gametypes\_persistence::func_7B27("accuracy",int(var_04));
			maps\mp\gametypes\_persistence::func_7B27("misses",int(var_02 - var_03));
		}
...
checkhit(param_00,param_01)
{
	...
	if(!issquadsmode())
	{
		...
		if(var_04 <= var_03)
		{
			...
			maps\mp\gametypes\_persistence::func_7B27("misses",int(var_03 - var_04));
			var_05 = clamp(float(var_04) / float(var_03),0,1) * 10000;
			maps\mp\gametypes\_persistence::func_7B27("accuracy",int(var_05));
		}
	}
}
...
begingrenadetracking()
{
	...
	switch(var_01.weapon_name)
	{
		...
		case "flash_grenade_mp":
			...
			{
				var_01.iscooked = 1;
				var_01.ninebangticks = var_01.ninebangticks + min(int(var_02 / 875),5);
			}
...
func_3C83(param_00)
{
	...
	if(maps\mp\_utility::func_47BB())
	{
		var_01 = float(tablelookup("mp/alien/mode_string_tables/alien_statstable.csv",4,var_02,8));
	}
	else
	{
		var_01 = float(tablelookup("mp/statstable.csv",4,var_02,8));
	}
...
func_524D()
{
	...
	self hide();
	self radiusdamage(self.origin,level.minedamageradius,level.minedamagemax,level.minedamagemin,var_00,"MOD_EXPLOSIVE",self.weapon_name);
	if(isdefined(self.owner) && isdefined(level.leaderdialogonplayer_func))
	{
