 ...
 * Platform: PC
 * Function Count: 39
 * Decompile Time: 980 ms
 * Timestamp: 10/27/2023 1:16:41 AM
*******************************************************************/
...
func_8708()
{
	...
	if(self.movestate == "pursuit")
	{
		...
		self scragentsetgoalpos(var_00);
	}
}
...
func_8C05(param_00)
{
	...
	return !func_92B4(self.curmeleetarget.origin) && distance2dsquared(self.origin,self.curmeleetarget.origin) < self.meleeradiussq * 0.75 * 0.75 && !param_00 || self agentcanseesentient(self.curmeleetarget);
}
...
findpointnearowner()
{
	...
	if(self.bhasbadpath && distance2dsquared(var_1A,self.lastbadpathgoal) < 4)
	{
		return self.origin;
	}
...
func_5D4D(param_00)
{
	...
	{
		self playsoundonmovingent(common_scripts\utility::func_803F(self.biswolf,"anml_wolf_bark","anml_dog_bark"));
...
playgrowl(param_00)
{
	...
	for(;;)
	{
		...
		self playsoundonmovingent(common_scripts\utility::func_803F(self.biswolf,"anml_wolf_growl","anml_dog_growl"));
...
playpanting(param_00)
{
	...
	for(;;)
	{
		...
		{
			self playsoundonmovingent(common_scripts\utility::func_803F(self.biswolf,"anml_wolf_pants_mp_fast","anml_dog_pants_mp_fast"));
		}
		else
		{
			self playsoundonmovingent(common_scripts\utility::func_803F(self.biswolf,"anml_wolf_pants_mp_med","anml_dog_pants_mp_med"));
		}
