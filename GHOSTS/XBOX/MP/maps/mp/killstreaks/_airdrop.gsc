 ...
 * Timestamp: 10/27/2023 1:32:10 AM
*******************************************************************/
...
addcratetype(param_00,param_01,param_02,param_03,param_04,param_05,param_06,param_07,param_08)
{
	...
	level.cratetypes[param_00][param_01] = addstruct();
...
killstreakcratethink(param_00)
{
	...
	for(;;)
	{
		...
		if(isdefined(self.owner) && var_02 != self.owner)
		{
			...
			{
				self.owner thread maps\mp\gametypes\_rank::giverankxp("killstreak_giveaway",castint(maps\mp\killstreaks\_killstreaks::func_3C16(self.cratetype) / 10 * 50));
				self.owner thread maps\mp\gametypes\_hud_message::func_794D("sharepackage",castint(maps\mp\killstreaks\_killstreaks::func_3C16(self.cratetype) / 10 * 50));
			}
		}
...
func_6E04(param_00)
{
	...
	for(;;)
	{
		...
		if(isdefined(self.owner) && var_01 != self.owner)
		{
			...
			{
				self.owner thread maps\mp\gametypes\_rank::giverankxp("killstreak_giveaway",castint(maps\mp\killstreaks\_killstreaks::func_3C16("sentry") / 10) * 50);
...
func_77FC(param_00,param_01,param_02)
{
	...
	if(botsentientswap(var_07.owner))
	{
		wait(0.1);
