 ...
 * Timestamp: 10/27/2023 1:28:19 AM
*******************************************************************/
...
func_15D0()
{
	...
	for(;;)
	{
		...
		{
			if(!botsentientswap(var_01) && !isdefined(var_01.processed_for_fireteam))
			{
				if(isdefined(var_01.team) && var_01.team == "allies" || var_01.team == "axis")
				{
					...
					if(level.bots_fireteam_humans.size == 1)
					{
						...
						{
							if(isdefined(var_05) && !botsentientswap(var_05))
							{
								var_03++;
							}
						}
...
func_3322(param_00,param_01)
{
	...
	if(level.zonecount > 0)
	{
		...
		{
			var_06 = botautoconnectenabled(var_05,param_01,"enemy_predict");
			if(var_06 < var_02)
			{
				continue;
			}
			...
			if(var_06 == var_02)
			{
				var_07 = getzonenodes(param_00,var_05);
				if(!isdefined(var_07))
				{
					continue;
				}
...
func_3320(param_00)
{
	...
	{
		var_01 = 3;
		if(!isdefined(level.fireteam_hunt_leader[param_00]) || botsentientswap(level.fireteam_hunt_leader[param_00]) || isdefined(level.fireteam_hunt_leader[param_00].commanding_bot))
		{
			fireteam_tdm_set_hunt_leader(param_00);
		}
		...
		if(isdefined(level.fireteam_hunt_leader[param_00]))
		{
			var_02 = getzonecount(level.fireteam_hunt_leader[param_00].origin);
			if(!isdefined(var_02))
			{
				wait(var_01);
				continue;
			...
			if(!botsentientswap(level.fireteam_hunt_leader[param_00]))
			{
				...
				if(isdefined(var_05))
				{
					...
					if(var_04)
					{
						if(!isdefined(var_06))
						{
							...
							{
								var_0A = distance2d(getzonepath(var_09),level.fireteam_hunt_leader[param_00].origin);
								if(var_0A > var_07)
								{
									var_07 = var_0A;
									var_08 = var_09;
						...
						if(isdefined(var_06))
						{
							if(!isdefined(level.fireteam_hunt_target_zone[param_00]) || level.fireteam_hunt_target_zone[param_00] != var_06)
							{
								...
								{
									if(botsentientswap(var_0C) && var_0C.team == param_00)
									{
...
func_15D2()
{
	...
	{
		var_02 = getzonenearest(level.fireteam_hunt_target_zone[self.team],0);
		if(var_02.size <= 18)
		{
			var_02 = getzonenearest(level.fireteam_hunt_target_zone[self.team],1);
			if(var_02.size <= 18)
			{
				var_02 = getzonenearest(level.fireteam_hunt_target_zone[self.team],2);
				if(var_02.size <= 18)
				{
					var_02 = getzonenearest(level.fireteam_hunt_target_zone[self.team],3);
				}
			}
		}
		...
		{
			var_05 = "guard";
			var_06 = getzonecount(self.origin);
...
bot_fireteam_monitor_killstreak_earned()
{
	...
	for(;;)
	{
		...
		{
			if(isdefined(self) && botsentientswap(self))
			{
