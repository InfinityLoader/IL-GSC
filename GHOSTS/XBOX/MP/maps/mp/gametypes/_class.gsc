 ...
 * Timestamp: 10/27/2023 1:30:36 AM
*******************************************************************/
...
giveloadout(param_00,param_01,param_02)
{
	...
	if(isdefined(var_12))
	{
		self.loadoutprimarycamo = castint(tablelookup("mp/camoTable.csv",1,var_12,4));
	}

	self.loadoutsecondary = var_14;
	if(isdefined(var_17))
	{
		self.loadoutsecondarycamo = castint(tablelookup("mp/camoTable.csv",1,var_17,4));
	}
	...
	self.loadoutprimaryreticle = castint(tablelookup("mp/reticleTable.csv",1,var_13,5));
	self.loadoutsecondaryreticle = castint(tablelookup("mp/reticleTable.csv",1,var_18,5));
	if(!issubstr(var_0F,"iw5") && !issubstr(var_0F,"iw6"))
	{
	...
	if(!isbot(self) && func_4002() || self.class == "callback" && !issubstr(self.class,"juggernaut") && !issubstr(self.lastclass,"juggernaut") && !issubstr(param_01,"juggernaut"))
	{
	...
	var_38 = isai(self) && getactiveclientcount() && !maps\mp\_utility::func_8FFB();
...
{
	var_06 = self;
	if(maps\mp\_utility::bot_is_fireteam_mode() && botsentientswap(self))
	{
	...
	if(!maps\mp\_utility::isjuggernaut())
	{
		...
		{
			var_11 = castint(tablelookup("mp/killCamAbilitiesBitMaskTable.csv",1,var_0F[var_10],0));
			if(var_11 == 0)
			{
				continue;
			}

			var_12 = castint(var_11 - 1 / 24);
...
isattachmentunlocked(param_00,param_01)
{
	...
	var_06 = castint(tablelookup("mp/weaponRankTable.csv",var_02,maps\mp\_utility::getweaponclass(param_00),var_03));
	var_07 = castint(tablelookup("mp/weaponRankTable.csv",var_06,param_01,var_04));
	if(var_05 >= var_07)
	{
		return 1;
	}
...
isperk(param_00)
{
	return castint(tablelookup("mp/perktable.csv",1,param_00,0));
}
