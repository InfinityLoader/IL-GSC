 ...
 * Timestamp: 10/27/2023 1:29:16 AM
*******************************************************************/
...
func_704F(param_00,param_01)
{
	...
	func_9458("ui_cranked_bomb_timer_end_milliseconds",castint(gettime() + param_01 * 1000));
}
...
inithordesettings()
{
	...
	level.hordedifficultylevel = castint(clamp(getdvarint("scr_horde_difficulty",3),1,3));
...
func_3BDA(param_00)
{
	param_00 = castint(clamp(param_00,1,20));
	param_00 = param_00 - 1;
...
func_3B27(param_00)
{
	...
	var_02 = castint(tablelookupbyrow("mp/hordeSettings.csv",var_01,1));
	if(param_00 > 20)
	{
		var_03 = param_00 - 20;
...
{
	var_01 = func_3BDA(param_00);
	var_02 = castint(tablelookupbyrow("mp/hordeSettings.csv",var_01,2));
	if(param_00 > 20)
	{
		var_03 = param_00 - 20;
		var_04 = 1 + castint(var_03 / 5);
...
monitorsupportdropprogress()
{
	...
	for(;;)
	{
		...
		setomnvar("ui_horde_support_drop_progress",castint(level.currentpointtotal / var_00 * 100));
	}
}
...
func_3C1C()
{
	...
	var_01 = castint(level.currentroundnumber / 4);
	var_02 = 45 + var_01 * 15;
...
func_8F33(param_00)
{
	...
	var_05 = castint(max(level.currentroundnumber,1));
...
func_9596()
{
	...
	for(;;)
	{
		...
		var_00 = castint(max(level.currentroundnumber,1));
		...
		foreach(var_02 in level.players)
		{
			...
			var_02 setclientomnvar("ui_horde_weapon_progress",castint(var_02.weaponstate[var_04]["vaule"] / var_05 * 100));
...
func_53D9(param_00)
{
	...
	for(;;)
	{
		...
		param_00 setclientomnvar("ui_horde_weapon_progress",castint(param_00.weaponstate[var_02]["vaule"] / var_03 * 100));
...
func_955B(param_00,param_01,param_02)
{
	...
	param_00 getvieworigin(param_02,getcamoindex(param_00.weaponstate[param_01]["level"]));
}
...
func_5327(param_00,param_01,param_02,param_03,param_04,param_05,param_06,param_07)
{
	...
	if(isdefined(param_01) && isplayer(param_01))
	{
		...
		{
			param_02 = castint(param_02 + param_02 * 0.1 * param_01.weaponstate[var_08]["level"] - 1);
		}
	...
	if(isdefined(param_01) && isdefined(param_01.owner) && isplayer(param_01.owner))
	{
		...
		if(isbot(param_01))
		{
			param_02 = castint(param_02 + param_02 * 0.1 * level.currentroundnumber - 2);
			var_09 = 1;
		}
	...
	if(isplayer(param_00) || maps\mp\gametypes\_horde_util::isonhumanteam(param_00))
	{
		...
		{
			param_02 = castint(param_02 * 0.125);
		}
		else
		{
			param_02 = castint(param_02 * getagentdamagescalar());
		}

		if(isdefined(param_01) && isbot(param_01))
		{
			param_01 hudoutlineenable(level.enemyoutlinecolor,0);
			param_01.outlinecolor = level.enemyoutlinecolor;
			if(param_03 == "MOD_MELEE")
			{
				param_02 = castint(param_00.maxhealth / 2) + 1;
			}

			if(param_01.agent_type == "dog")
			{
				param_02 = castint(param_00.maxhealth / 4) + 1;
			}
		}
