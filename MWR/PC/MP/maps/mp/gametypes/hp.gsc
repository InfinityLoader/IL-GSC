 ...
 * Timestamp: 10/27/2023 2:38:16 AM
*******************************************************************/
...
getspawnpoint()
{
	...
	{
		var_01 = maps\mp\gametypes\_spawnlogic::getrandomstartspawn("mp_hp_spawn_" + var_00 + "_start");
	}
	else
	{
		var_02 = maps\mp\gametypes\_spawnlogic::getteamspawnpoints(var_01);
		var_01 = maps\mp\gametypes\_spawnscoring::getspawnpoint_hardpoint(var_02);
	}
...
onzonecapture(param_00)
{
	...
	setteamicons(var_01);
	level.zone namedborderowned(var_01);
	...
	if(isdefined(self.lastcaptureteam) && self.lastcaptureteam != var_01)
	{
		for(var_07 = 0;var_07 < level.players.size;var_07++)
		{
			...
			{
				if(isdefined(param_00.lastkilldefendertime) && param_00.lastkilldefendertime + 500 > gettime())
				{
					param_00 maps\mp\gametypes\_misions::processchallenge("ch_hp_killedLastContester");
				}
			}
		}
	}
...
give_capture_credit(param_00,param_01,param_02,param_03)
{
	...
	for(var_05 = 0;var_05 < var_04.size;var_05++)
	{
		...
		{
			var_06 maps\mp\gametypes\_misions::processchallenge("ch_hpcaptures");
			if(level.hpstarttime + 500 > param_01)
			{
				var_06 maps\mp\gametypes\_misions::processchallenge("ch_hp_immediateCapture");
			}
...
//Function Number: 19
namedbordervisible(param_00,param_01)
{
	if(!isdefined(self.namedborders[param_00]))
	{
		return;
	}

	foreach(var_03 in self.namedborders[param_00])
	{
		var_03 notify("stopShowFXToTeam");
		if(param_01)
		{
			var_03.state = "show";
			var_03 show();
...
//Function Number: 20
namedbordershowtoteam(param_00,param_01)
{
	if(!isdefined(self.namedborders[param_00]))
	{
		return;
	...
	foreach(var_03 in self.namedborders[param_00])
	{
		var_03.state = "showToTeam";
		var_03 notify("stopShowFXToTeam");
		var_03 thread maps\mp\_utility::showfxtoteam(param_01);
	}
}
...
//Function Number: 24
namedborderowned(param_00)
{
	namedbordervisible("neutral",0);
	namedbordervisible("contested",0);
	namedbordershowtoteam("friendly",param_00);
	namedbordershowtoteam("enemy",maps\mp\_utility::getotherteam(param_00));
}
...
onzoneuncontested(param_00)
{
	...
	level.zone namedborderowned(param_00);
	...
	if(!isdefined(level.zone.gameobject.wascontested) || level.zone.gameobject.wascontested == 0)
	{
		if(!isdefined(level.zone.gameobject.wasleftunoccupied) || level.zone.gameobject.wasleftunoccupied == 0)
		{
			...
			foreach(var_03 in level.players)
			{
				...
				{
					var_03 maps\mp\gametypes\_misions::processchallenge("ch_hp_controlZoneEntirely");
				}
			}
		}
	}
...
awardcapturepoints(param_00,param_01)
{
	...
	while(!level.gameended)
	{
		...
		if(!level.zone.gameobject.iscontested)
		{
			...
			var_04 = level.zone.gameobject.touchlist[param_00];
			var_05 = getarraykeys(var_04);
			for(var_06 = 0;var_06 < var_05.size;var_06++)
			{
				var_07 = var_04[var_05[var_06]].player;
				if(!isplayer(var_07))
				{
					continue;
				}

				var_07 thread maps\mp\_events::holdhardpointevent();
			}

			maps\mp\gametypes\_gamescores::giveteamscoreforobjective(param_00,var_03);
		}
	}
}
	...
	for(var_02 = 0;var_02 < param_00.size;var_02++)
	{
		...
		if(isdefined(var_04.target))
		{
			...
			foreach(var_09 in var_07)
			{
				...
				if(isdefined(var_0A))
				{
					if(!isdefined(var_04.namedborders[var_0A]))
					{
						var_04.namedborders[var_0A] = [];
					}

					var_0B = var_04.namedborders[var_0A].size;
					var_04.namedborders[var_0A][var_0B] = var_09;
					continue;
				}
		...
		var_04.gameobject.objpoints["allies"] method_8518(var_04.trig);
		var_04.gameobject.objpoints["axis"] method_8518(var_04.trig);
		var_04.gameobject.objpoints["mlg"] method_8518(var_04.trig);
		var_04 setupnearbyspawns();
		var_04 namedborderhidden();
	}
...
onplayerkilled(param_00,param_01,param_02,param_03,param_04,param_05,param_06,param_07,param_08,param_09)
{
	...
	var_0A = level.zone.gameobject maps\mp\gametypes\_gameobjects::getownerteam();
	...
	{
		param_01.lastkilldefendertime = gettime();
		if(var_0B)
		{
			return;
		}
		...
		param_01 maps\mp\gametypes\_misions::processchallenge("ch_hp_zoneDefense");
...
updatecapsperminute(param_00)
{
	...
	{
		var_01 = max(self.timeplayed["total"],1) / 60;
	}
