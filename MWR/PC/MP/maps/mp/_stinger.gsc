 ...
 * Timestamp: 10/27/2023 2:40:36 AM
*******************************************************************/
...
gettargetlist()
{
	...
	if(level.teambased)
	{
		...
		if(level.multiteambased)
		{
			...
			{
				if(self.team != level.teamnamelist[var_04])
				{
					if(level.uavmodels[level.teamnamelist[var_04]].size)
					{
						foreach(var_06 in level.uavmodels[level.teamnamelist[var_04]])
						{
		...
		{
			foreach(var_06 in level.uavmodels[level.otherteam[self.team]])
			{
				var_00[var_00.size] = var_06;
			}
		}
		...
		{
			foreach(var_0B in level.littlebirds)
			{
	...
	else
	{
		...
		{
			foreach(var_06 in level.uavmodels)
			{
				if(isdefined(var_06.owner) && var_06.owner == self)
				{
					continue;
				}
