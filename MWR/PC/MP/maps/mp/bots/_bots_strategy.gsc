 ...
 * Timestamp: 10/27/2023 2:37:34 AM
*******************************************************************/
...
bot_get_ambush_trap_item(param_00,param_01,param_02)
{
	...
	foreach(var_0E, var_0A in var_03)
	{
		...
		{
			if(maps\mp\bots\_bots_util::bot_item_matches_purpose(var_0A,var_07))
			{
				param_01["purpose"] = var_0A;
				if(isweaponmanuallydetonatedbyemptythrow(var_06) || isweaponmanuallydetonatedbydoubletap(var_06))
				{
...
bot_set_ambush_trap(param_00,param_01,param_02,param_03,param_04)
{
	...
	if(isdefined(param_04))
	{
		...
		if(var_12)
		{
			...
			if(var_13 == "goal")
			{
				thread maps\mp\bots\_bots_util::bot_force_stance_for_time("stand",4);
				if(!isdefined(self.enemy) || !self botcanseeentity(self.enemy))
				{
					thread maps\mp\bots\_bots_util::bot_disable_movement_for_time(4);
					var_14 = (0,0,0);
					...
					if(isdefined(var_10))
					{
						self botlookatpoint(var_05.origin + var_14,3,"script_forced");
					}
					else
					{
						self botlookatpoint(self.origin + var_14 + anglestoforward(self getplayerangles()) * 50,3,"script_forced");
					}
					...
					bot_try_use_ambush_item(param_00["item_action"]);
					...
					var_15 = common_scripts\utility::ter_op(param_00["purpose"] == "tacticalinsertion",6,3);
					common_scripts\utility::waittill_any_timeout(var_15,"missile_fire","grenade_fire");
					wait(0.05);
...
//Function Number: 5
bot_try_use_ambush_item(param_00)
{
...
//Function Number: 7
bot_watch_c4_detonate(param_00,param_01)
{
	self endon("death");
	self endon("disconnect");
	...
	for(;;)
	{
		...
		{
			var_03 = self getclosestenemysqdist(param_00.origin,1);
			if(var_03 < var_02)
			{
...
defense_cautious_approach()
{
	...
	for(var_17 = 1;var_17 < var_05.size - 2;var_17++)
	{
		...
		{
			if(!common_scripts\utility::within_fov(self.origin,self getplayerangles(),var_18[var_0C].origin,0))
			{
				continue;
			}
	...
	for(;;)
	{
		...
		if(isdefined(var_01))
		{
			...
			for(var_03 = 0;var_03 < self.locations_to_investigate.size;var_03++)
			{
				...
				{
					var_04 = common_scripts\utility::within_fov(self.origin,self getplayerangles(),self.locations_to_investigate[var_03].origin,var_02);
		...
		foreach(var_07 in var_05)
		{
			...
			if(var_07.health == 0 && isdefined(var_07.deathtime) && var_04 - var_07.deathtime < 5000)
			{
				if(var_04 - var_07.last_investigation_time > 10000 && var_04 > var_00[var_08])
				{
					...
					{
						if(distancesquared(var_07.body.origin,self.origin) < var_02)
						{
							self botgetimperfectenemyinfo(var_07.lastattacker,var_07.body.origin);
							var_09 = getclosestnodeinsight(var_07.body.origin);
							if(isdefined(var_09))
							{
...
bot_monitor_watch_entrances_bodyguard()
{
	...
	for(;;)
	{
		var_00 = anglestoforward(self.bot_defend_player_guarding getplayerangles()) * (1,1,0);
	...
	for(;;)
	{
		if(self.tactical_goals.size > 0 && !maps\mp\bots\_bots_util::bot_is_remote_or_linked())
		{
			...
			if(!isdefined(var_00.abort))
			{
				...
				if(maps\mp\_utility::is_true(var_00.clear_prev_goal))
				{
					self botclearscriptgoal();
				}

				self botsetscriptgoal(var_00.goal_position,var_00.goal_radius,var_01,var_00.goal_yaw,var_00.objective_radius,var_00.short_term_goal);
...
bot_new_tactical_goal(param_00,param_01,param_02,param_03)
{
	...
	var_04.short_term_goal = param_03.short_term_goal;
	var_04.optional_params = param_03.optional_params;
	var_04.clear_prev_goal = param_03.clear_prev_goal;
...
//Function Number: 48
bot_get_active_tactical_goals_of_type(param_00)
{
	var_01 = [];
	if(isdefined(self.tactical_goals))
	{
		foreach(var_03 in self.tactical_goals)
		{
			if(!maps\mp\_utility::is_true(var_03.abort) && var_03.type == param_00)
			{
