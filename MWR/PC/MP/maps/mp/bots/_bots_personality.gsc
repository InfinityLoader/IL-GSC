 ...
 * Timestamp: 10/27/2023 2:37:30 AM
*******************************************************************/
...
bot_balance_personality()
{
	...
	var_01 = getarraykeys(level.bot_personality_types_desired);
	var_02 = [];
	var_03 = [];
	foreach(var_09, var_05 in level.bot_personality)
	{
		var_03[var_09] = 0;
	...
	foreach(var_0B in level.participants)
	{
		if(var_0B == self)
		{
			continue;
		...
		if(!maps\mp\_utility::isteamparticipant(var_0B) || !isdefined(var_0B.has_balanced_personality))
		{
			continue;
		}

		if((isdefined(var_0B.team) && var_0B.team == var_00) || !level.teambased)
		{
			var_07 = var_0B botgetpersonality();
			var_09 = level.bot_personality_type[var_07];
	...
	{
		var_0D[var_0F] = int(var_03[var_0F] / level.bot_personality_types_desired[var_0F]);
	}
	...
	if(!isdefined(var_11))
	{
		...
		foreach(var_0F in var_01)
		{
			var_17[var_0F] = level.bot_personality_types_desired[var_0F] - var_03[var_0F] % level.bot_personality_types_desired[var_0F];
		}
	...
	var_23 = undefined;
	var_24 = -9999;
	var_25 = common_scripts\utility::array_randomize(level.bot_personality[var_11]);
	...
	{
		var_20 = common_scripts\utility::random(level.bot_personality[var_11]);
	}

	if(self botgetpersonality() != var_20)
	{
...
update_personality_camper()
{
	if(should_select_new_ambush_point() && !maps\mp\bots\_bots_util::bot_is_defending() && !maps\mp\bots\_bots_util::bot_is_remote_or_linked())
	{
		...
		if((!var_02 || var_03) && !maps\mp\bots\_bots_util::bot_out_of_ammo())
		{
			...
			{
				var_01 = [[ level.override_find_camp_node ]]();
			}
...
update_personality_default()
{
	...
	if(!maps\mp\bots\_bots_strategy::bot_has_tactical_goal() && !maps\mp\bots\_bots_util::bot_is_remote_or_linked())
	{
		...
		if(!var_01 || var_02 < var_03 * var_03)
		{
			...
			{
				var_08 = maps\mp\bots\_bots_strategy::bot_get_ambush_trap_item("trap_directional","trap");
				if(isdefined(var_08))
				{
					var_09 = self botgetscriptgoal();
					if(isdefined(var_09))
					{
						var_0A = getclosestnodeinsight(var_09);
						if(isdefined(var_0A))
						{
							var_0B = bot_find_ambush_entrances(var_0A,0);
							var_0C = maps\mp\bots\_bots_strategy::bot_set_ambush_trap(var_08,var_0B,var_0A);
