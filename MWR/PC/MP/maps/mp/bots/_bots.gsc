 ...
 * Timestamp: 10/27/2023 2:37:04 AM
*******************************************************************/
...
init()
{
	...
	if(var_00 == "enabled_fill_open" || var_00 == "enabled_fill_open_dev")
	{
...
bot_connect_monitor(param_00,param_01)
{
	...
	for(;;)
	{
		...
		{
			var_0B = level.bot_last_team_ally;
		}

		var_0C = "axis";
		if(isdefined(level.bot_last_team_enemy))
		{
			var_0C = level.bot_last_team_enemy;
		}
		...
		level.bot_last_team_ally = var_0B;
		level.bot_last_team_enemy = var_0C;
...
bot_think()
{
	...
	for(;;)
	{
		...
		if(var_01)
		{
			...
			if(!bot_gametype_chooses_class())
			{
				if(maps\mp\_utility::ishodgepodgeph() && var_00 == game["attackers"] && game["roundsPlayed"] > 0)
				{
					wait(0.5);
				}
...
bot_3d_sighting_model_thread(param_00)
{
	...
	for(;;)
	{
		if(isalive(self) && !self botcanseeentity(param_00) && common_scripts\utility::within_fov(self.origin,self getplayerangles(),param_00.origin,self botgetfovdot()))
		{
...
monitor_smoke_grenades()
{
	...
	for(;;)
	{
		...
		if(var_02 == "h1_smokegrenade_mp")
		{
			var_00 thread handle_smoke();
		}
	}
}
...
handle_smoke()
{
	...
	wait(var_02);
	var_02 = 0.65;
	var_01 clonebrushmodeltoscriptmodel(level.bot_smoke_sight_clip_small);
	wait(var_02);
	var_02 = 1.5;
	var_01 clonebrushmodeltoscriptmodel(level.bot_smoke_sight_clip_medium);
	wait(var_02);
	var_02 = 7.95;
	var_01 clonebrushmodeltoscriptmodel(level.bot_smoke_sight_clip_large);
	wait(var_02);
	var_01 delete();
}
