 ...
 * Timestamp: 10/27/2023 2:49:59 AM
*******************************************************************/
...
guy_enter(param_00,param_01)
{
	...
	if(isdefined(var_06.getout))
	{
		...
		if(isdefined(var_06.getout_ik))
		{
			param_00.getout_ik = 1;
		}
	}

	param_00.allowdeath = 0;
	if(isdefined(param_00.deathanim) && !isdefined(param_00.magic_bullet_shield))
	{
		param_00.allowdeath = 1;
	}

	if(isdefined(var_06.death))
	{
...
guy_deathimate_me(param_00,param_01)
{
	...
	detach_models_with_substr(param_00,"weapon_");
	param_00 linkto(self,param_01.sittag,(0,0,0),(0,0,0));
	param_00 notsolid();
	thread animontag(param_00,param_01.sittag,param_01.death);
	if(!isdefined(param_01.death_delayed_ragdoll))
	{
		param_00 waittillmatch("start_ragdoll","animontagdone");
	}
	else
	{
		param_00 unlink();
	...
	while(gettime() < var_02 && !param_00 isragdoll())
	{
		param_00 startragdoll();
		wait(0.05);
	}
...
{
	var_00 = [];
	var_01 = maps\_vehicle::get_script_vehicles();
	for(var_02 = 0;var_02 < var_01.size;var_02++)
	{
		var_03 = var_01[var_02];
		if(!isdefined(var_03.script_vehicleride))
...
guy_unload(param_00,param_01)
{
	...
	if(isdefined(var_02.nodeath))
	{
		param_00.a.nodeath = 0;
	}
	...
	if(!var_09)
	{
		...
		var_0D = 0;
		if(!isai(param_00) && !param_00 should_stay_drone())
		{
			var_0D = 1;
		...
		if(isdefined(var_02.fastroperig))
		{
			param_00 maps\_utility::enable_achievement_reinforcement_denied(self,var_02.fastroperig);
		}
		...
		else
		{
			...
			if(isdefined(param_00.getout_ik))
			{
				thread animontag(param_00,var_0A,var_0B,undefined,undefined);
				param_00 thread wait_do_ik_on();
				param_00.a.nodeath = 1;
				var_12 = param_00 wait_interrupt_or_done();
				if(isdefined(var_12) && var_12 == "interrupt")
				{
					while(!isdefined(param_00.ik_turned_on))
					{
						wait 0.05;
					}
				...
				param_00 method_8570(0);
				self.ik_turned_on = undefined;
				param_00.getout_ik = undefined;
				param_00.a.nodeath = 0;
			}
			else
			{
				animontag(param_00,var_0A,var_0B,undefined,undefined);
			}
		}
	...
	if(!isai(param_00) && param_00 should_stay_drone())
	{
		if(param_00.drone_delete_on_unload)
		{
			param_00 delete();
			return;
		}
	...
	if(isalive(param_00))
	{
		...
		if(isdefined(var_02.fastroperig) && isalive(param_00))
		{
			param_00 maps\_utility::disable_achievement_reinforcement_denied();
		}
...
wait_do_ik_on()
{
	...
	self waittillmatch("start_foot_ik","animontagdone");
	self method_8570(1);
	self.ik_turned_on = 1;
}
...
animontag_ragdoll_death(param_00,param_01)
{
	...
	for(;;)
	{
		...
		param_00 waittill("damage",var_02,var_03);
		while(isdefined(param_00.getout_ik) && !isdefined(param_00.ik_turned_on))
		{
			wait 0.05;
		}
...
animontag_ragdoll_death_fall(param_00,param_01,param_02)
{
	...
	if(isdefined(param_00.script_stay_drone) || isdefined(param_00.drone_delete_on_unload))
	{
		param_00 notsolid();
