 ...
 * Timestamp: 10/27/2023 2:48:28 AM
*******************************************************************/
...
register_kill(param_00,param_01,param_02,param_03)
{
	...
	var_06 = level.player getplayerdata(common_scripts\utility::getstatsgroup_sp(),"career","kills_total");
	if(isdefined(var_06))
	{
		level.player setplayerdata(common_scripts\utility::getstatsgroup_sp(),"career","kills_total",var_06 + 1);
	}
	...
	if(isdefined(param_00))
	{
		...
		if(param_00.code_classname == "script_vehicle")
		{
			...
			if(var_04 should_register_kills_for_vehicle_occupants())
			{
				...
				{
					foreach(var_08 in param_00.riders)
					{
						if(isdefined(var_08))
						{
							var_04 register_kill(var_08,param_01,param_02,param_03);
						}
					}
				}
			}
