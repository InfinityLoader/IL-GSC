 ...
 * Timestamp: 10/27/2023 2:47:10 AM
*******************************************************************/
...
init()
{
	...
	var_00 = should_use_mw_cover_moveup_behaviour();
	setsaveddvar("ai_useCoverMoveUpBehaviour",var_00);
	var_01 = should_use_cover_suppression_behaviour();
	setsaveddvar("ai_suppressionAffectsCover",var_01);
	...
	var_02 = should_enable_foot_ik();
	setsaveddvar("xanim_disableFootIK",!var_02);
...
should_use_bullet_whizby()
{
	...
	var_00 = maps\_utility::getdvarintdefault("ai_bulletWhizbyEnable",0);
	if(var_00 != 0)
	{
		return var_00 > 0;
	}
...
should_use_turn_animations()
{
	...
	var_00 = maps\_utility::getdvarintdefault("ai_turnAnimsOverride",0);
	if(var_00 != 0)
	{
		return var_00 > 0;
	}
...
should_use_improved_grenade_returns()
{
	...
	var_00 = maps\_utility::getdvarintdefault("ai_grenadeReturn_improvements",0);
	if(var_00 != 0)
	{
		return var_00 > 0;
	}
...
should_use_forced_teleport_to_anim_start()
{
	...
	var_00 = maps\_utility::getdvarintdefault("ai_forcedTeleportToAnimStart",0);
	if(var_00 != 0)
	{
		return var_00 > 0;
	}
...
{
	var_00 = maps\_utility::getdvarintdefault("ai_coverArrival_ignorePlayer",0);
	if(var_00 != 0)
	{
		return var_00 > 0;
	}
...
{
	var_00 = maps\_utility::getdvarintdefault("ai_use_rpg_repulsors",0);
	if(var_00 != 0)
	{
		return var_00 > 0;
	}
...
{
	var_00 = maps\_utility::getdvarintdefault("ai_use_grenade_dist_checks",0);
	if(var_00 != 0)
	{
		return var_00 > 0;
	}
...
//Function Number: 15
orient_to_entity_until_flags(param_00,param_01)
{
	orient_to_point_until_flags(param_00.origin,param_01);
}

//Function Number: 16
orient_to_point_until_flags(param_00,param_01)
{
	self endon("death");
	while(!common_scripts\utility::flags(param_01))
	{
		self orientmode("face point",param_00);
		wait 0.05;
	}
...
{
	if(isdefined(param_02) && isdefined(self.model) && !maps\_utility::hastag(self.model,param_02))
	{
		anim_generic_loop_preh1(param_00,param_01,param_02,param_03);
		return;
	}
	...
	var_04["tag"] = param_02;
	var_05[0] = var_04;
	anim_loop_packet(var_05,param_01,param_03,"generic");
}
...
anim_first_frame_on_guy(param_00,param_01,param_02,param_03,param_04)
{
	...
	if(isarray(level.scr_anim[var_05][param_01]))
	{
		param_00 setanimknob(level.scr_anim[var_05][param_01][0],1,0,0);
		param_00 setanimtime(level.scr_anim[var_05][param_01][0],0);
		return;
	}
	...
	for(;;)
	{
		...
		foreach(var_22, var_05 in param_00)
		{
			...
			if(var_1B)
			{
				...
				if(animhasanimforcedefaultrate(level.scr_anim[var_1F][param_01][var_0B]))
				{
					var_06 setanimrate(level.scr_anim[var_1F][param_01][var_0B],1);
				}
...
//Function Number: 44
anim_single(param_00,param_01,param_02,param_03,param_04,param_05,param_06)
{
	if(isdefined(param_03) && !isnumber(param_03))
	{
		param_06 = param_03;
		param_03 = undefined;
	}
	...
	anim_single_internal(param_00,param_01,param_02,param_03,param_04,param_05,param_06);
}
...
anim_single_internal(param_00,param_01,param_02,param_03,param_04,param_05,param_06)
{
	...
	var_0A = get_anim_position(param_02,param_06);
	...
	foreach(var_1F, var_08 in param_00)
	{
		...
		if(var_16)
		{
			...
			{
				var_1D = level.scr_anim[var_1F][param_02];
				var_08 setflaggedanim(var_12,var_1D,1,var_1B);
			}
			else
			{
				var_1D = level.scr_anim[var_1F][param_02];
				if(!var_14 && animscripts\face::animhasfacialoverride(var_1D))
				{
					var_08 animscripts\face::clearfacialanim();
				}

				if(animhasanimforcedefaultrate(var_1D))
				{
					var_08 setanimrate(var_1D,1);
				}

				var_08 animscripted(var_12,var_0B,var_0C,var_1D,undefined,undefined,var_1B);
			}
...
notetrack_wait(param_00,param_01,param_02,param_03,param_04,param_05)
{
	...
	foreach(var_12, var_0B in var_09)
	{
		if(isdefined(level.scr_notetrack[var_07][var_12][var_14]))
		{
			...
			{
				for(var_1A = 0;var_1A < level.scr_notetrack[var_07][var_12][var_14].size;var_1A++)
				{
					var_0F = level.scr_notetrack[var_07][var_12][var_14][var_1A];
	...
	var_1D = getsubstr(var_14,0,3);
	if(var_1D == "ps_")
	{
		var_1E = getsubstr(var_14,3);
		if(isdefined(param_00.anim_playsound_func))
		{
			param_00 thread [[ param_00.anim_playsound_func ]](var_1E,"j_head",1);
		}
		else
		{
			param_00 thread maps\_utility::play_sound_on_tag(var_1E,undefined,1);
		}
	...
	if(var_1D == "vo_")
	{
		...
		{
			param_00 thread maps\_utility::play_sound_on_tag(var_1E,"j_head",1);
		}
	...
	{
		var_1E = getsubstr(var_14,3);
		param_00 thread maps\_utility::smart_dialogue(var_1E);
		continue;
	}

	if(var_1D == "fx_")
	{
		var_1F = strtok(tolower(getsubstr(var_14,3)),",() ");
		if(var_1F.size == 2)
		{
			if(var_1F[0] == "exploder")
			{
				common_scripts\_exploder::exploder(var_1F[1]);
				continue;
			}
			else if(var_1F[0] == "stop_exploder")
			{
				maps\_utility::stop_exploder(var_1F[1]);
				continue;
			}
			else
			{
				playfxontag(level._effect[var_1F[0]],param_00,var_1F[1]);
				continue;
			}
		}
		else if(var_1F.size == 3)
		{
			if(var_1F[0] == "playfxontag")
			{
				playfxontag(level._effect[var_1F[1]],param_00,var_1F[2]);
				continue;
			}
			else if(var_1F[0] == "stopfxontag")
			{
				stopfxontag(level._effect[var_1F[1]],param_00,var_1F[2]);
				continue;
			}
			else if(var_1F[0] == "killfxontag")
			{
				killfxontag(level._effect[var_1F[1]],param_00,var_1F[2]);
				continue;
			}
		}
		else if(var_1F.size == 11)
		{
			var_20 = (float(var_1F[2]),float(var_1F[3]),float(var_1F[4]));
			var_21 = (float(var_1F[5]),float(var_1F[6]),float(var_1F[7]));
			var_22 = (float(var_1F[8]),float(var_1F[9]),float(var_1F[10]));
			playfx(level._effect[var_1F[1]],var_20,var_21,var_22);
		}
	}

	var_1D = getsubstr(var_14,0,4);
	if(var_1D == "psr_")
	{
		var_1E = getsubstr(var_14,4);
		maps\_utility::radio_dialogue(var_1E);
		continue;
	}
...
idle_on_reach(param_00,param_01,param_02,param_03,param_04)
{
	...
	if(isalive(param_00))
	{
		anim_loop_solo(param_00,param_01,param_03,param_02);
	}
}

//Function Number: 73
get_anim_position(param_00,param_01)
{
	var_02 = (0,0,0);
	var_03 = (0,0,0);
	if(!isdefined(param_01))
	...
	if(can_get_anim_position_from_tag(param_01,param_00))
	{
		var_02 = param_01 gettagorigin(param_00);
		var_03 = param_01 gettagangles(param_00);
	}
	else
	{
...
//Function Number: 74
can_get_anim_position_from_tag(param_00,param_01)
{
	if(!isdefined(param_00))
	{
		return 0;
	...
	if(!isdefined(param_00 getmodelfromentity()))
	{
		return 0;
	}

	if(param_00 getmodelfromentity() == "")
	{
		return 0;
	}

	if(!maps\_utility::hastag(param_00 getmodelfromentity(),param_01))
	{
		return 0;
	}
...
//Function Number: 78
anim_reach(param_00,param_01,param_02,param_03,param_04,param_05)
{
	var_06 = self;
	if(isdefined(param_04) && !isnumber(param_04))
	{
		var_06 = param_04;
		param_03 = param_05;
		param_04 = undefined;
	}
	else if(isdefined(param_03) && !isstring(param_03))
	{
		var_06 = param_03;
		param_03 = param_05;
		param_04 = undefined;
	...
	return var_06 anim_reach_with_funcs(param_00,param_01,param_02,param_03,::reach_with_standard_adjustments_begin,::reach_with_standard_adjustments_end,undefined,param_04);
}

//Function Number: 79
anim_reach_with_funcs(param_00,param_01,param_02,param_03,param_04,param_05,param_06,param_07)
{
	var_08 = maps\_utility::get_anim_reach_unique_id();
	...
	var_0D = 0;
	var_0E = 0;
	foreach(var_10 in param_00)
	{
		if(isdefined(param_06))
		{
			var_10.scriptedarrivalent = self;
		}
		...
		{
			var_11 = var_10.animname;
		}

		if(isdefined(level.scr_anim[var_11][param_01]))
		{
			if(isarray(level.scr_anim[var_11][param_01]))
			{
				var_12 = getstartorigin(var_0A,var_0B,level.scr_anim[var_11][param_01][0]);
			}
			else
			{
				var_12 = getstartorigin(var_0A,var_0B,level.scr_anim[var_11][param_01]);
			}
		}
		else
		{
		...
		var_0E++;
		var_10 thread begin_anim_reach(var_0C,var_12,param_04,param_05,param_01,param_07,var_08);
	}

	var_14 = 1;
	var_0C thread monitor_reach_thread_death(thisthread,param_00,self,param_06,var_08);
	while(var_0E)
	{
		var_0C waittill("reach_notify",var_10,var_15,var_16);
		if(isdefined(var_10))
		{
			var_10.anim_reach_success = var_15;
		}
	...
	var_0C notify("end_reach_thread_monitor");
	return var_14;
}
...
//Function Number: 81
monitor_reach_thread_death(param_00,param_01,param_02,param_03,param_04)
{
	...
	foreach(var_06 in param_01)
	{
		if(isdefined(var_06))
		{
			var_06.anim_reach_success = 1;
			var_06.reach_aborted = 1;
			var_06 notify("goal",param_04);
			if(isdefined(var_06.name))
			{
			}
			else
			{
...
//Function Number: 91
reach_death_notify(param_00,param_01,param_02)
{
	self endon("death");
	for(;;)
	{
		self waittill("goal",var_03);
		if(!isdefined(var_03) || var_03 == param_02)
		{
			break;
		}
	}

	var_04 = isalive(self) && distancesquared(self.origin,param_01) <= 2500;
	param_00 notify("reach_notify",self,var_04,param_02);
}

//Function Number: 92
begin_anim_reach(param_00,param_01,param_02,param_03,param_04,param_05,param_06)
{
	self endon("death");
	self endon("new_anim_reach");
	param_01 = [[ param_02 ]](param_01);
	thread reach_death_notify(param_00,param_01,param_06);
...
{
	if(isnumber(param_03) || isnumber(param_04))
	{
		anim_single_solo_preh1(param_00,param_01,param_02,param_03,param_04);
		return;
	}
	...
	self endon("death");
	var_05[0] = param_00;
	anim_single(var_05,param_01,param_02,undefined,undefined,undefined,param_03);
}
...
//Function Number: 108
anim_reach_and_arrive_facing_anim_solo(param_00,param_01,param_02,param_03,param_04)
{
	var_05 = get_anim_position(undefined);
	var_06 = var_05["origin"];
	var_07 = var_05["angles"];
	if(!isdefined(param_04))
	{
		param_04 = param_00.animname;
	}

	if(isdefined(level.scr_anim[param_04][param_01]))
	{
		if(isarray(level.scr_anim[param_04][param_01]))
		{
			var_08 = level.scr_anim[param_04][param_01][0];
		}
		else
		{
			var_08 = level.scr_anim[var_05][param_02];
		}

		var_06 = getstartorigin(var_06,var_07,var_08);
		var_07 = getstartangles(var_06,var_07,var_08);
	}
	...
	var_09 = !isdefined(param_00.script_forcecolor) && isdefined(param_00.old_forcecolor);
	var_0A = var_07[1];
	var_0B = spawnstruct();
	param_00 maps\_utility::ai_ignore_everything();
	param_00.scriptedarrivalent = undefined;
	param_00 reach_with_standard_adjustments_begin(var_06);
	param_00 get_to_transition_point(var_0B,var_0A,var_06,var_0A,param_02);
	var_0C = var_0A - param_00 animscripts\utility::lookuptransitionanim("cover_trans_angles",param_02,var_0B.approach_number);
	if(!isdefined(param_03))
	{
		param_03 = 0;
	}

	param_00 play_arrival_animation(param_02,var_0B,var_0C,param_03);
	param_00 reach_with_standard_adjustments_end();
	param_00 maps\_utility::ai_unignore_everything(undefined,var_09);
}

//Function Number: 109
play_arrival_animation(param_00,param_01,param_02,param_03)
{
	var_04 = animscripts\utility::lookupanim("cover_trans",param_00)[param_01.approach_number];
	var_05 = getanimlength(var_04);
	self.approachnumber = param_01.approach_number;
	self.approachtype = param_00;
	self startcoverarrival(param_01.start_point,param_02,0);
	wait(var_05 - param_03);
}

//Function Number: 110
get_to_transition_point(param_00,param_01,param_02,param_03,param_04)
{
	if(!isdefined(param_00))
	{
		return;
	...
	if(param_03 == "exposed")
	{
		var_05 = level.archetypes["soldier"]["longestExposedApproachDist"];
	}
	else
	{
		var_05 = level.archetypes["soldier"]["CoverTransLongestDist"][param_04];
	}

	maps\_utility::set_goal_pos(param_02);
	self.reach_goal_pos = param_02;
	self.goalradius = 0;
	var_06 = var_05 * var_05;
	while(distancesquared(self.origin,param_02) > var_06)
	{
		wait(0.05);
	}

	param_00.approach_number = calculate_approach_number(self,param_02,param_01,param_03);
	param_00.start_point = animscripts\cover_arrival::getarrivalstartpos(param_02,param_01,param_03,param_00.approach_number);
	var_06 = lengthsquared(animscripts\utility::lookuptransitionanim("cover_trans_dist",param_03,param_00.approach_number));
	while(distancesquared(self.origin,param_02) > var_06)
	{
		wait(0.05);
	}
...
//Function Number: 111
calculate_approach_number(param_00,param_01,param_02,param_03)
{
	if(!isdefined(param_03))
	{
		param_03 = "exposed";
	...
	var_04 = vectornormalize(param_01 - param_00.origin);
	var_05 = spawnstruct();
	animscripts\exit_node::calculatenodetransitionangles(var_05,param_03,1,param_02,var_04,9,-1);
	var_06 = 1;
	for(var_07 = 2;var_07 <= 9;var_07++)
	{
		if(var_05.transitions[var_07] > var_05.transitions[var_06])
		{
			var_06 = var_07;
		}
	}

	return var_05.transindex[var_06];
}
...
{
	if(isstring(param_04) || isvector(param_05))
	{
		anim_loop_solo_preh1(param_00,param_01,param_03,param_02,param_04,param_05);
		return;
	}
	...
	var_06["entity"] = param_04;
	var_06["tag"] = param_02;
	var_07[0] = var_06;
	anim_loop_packet(var_07,param_01,param_03);
}

//Function Number: 114
anim_loop_solo_preh1(param_00,param_01,param_02,param_03,param_04,param_05)
{
	self endon("death");
	param_00 endon("death");
	var_06 = [];
...
anim_facialanim(param_00,param_01,param_02)
{
	...
	param_00 setanim(%scripted_talking,level.scriptedtalkingweight,0.2);
...
force_face_anim_to_play(param_00,param_01,param_02)
{
	...
	{
		disabledefaultfacialanims();
		param_00 setanim(%scripted_talking,level.scriptedtalkingweight,0.4);
...
anim_facialfiller(param_00,param_01)
{
	...
	{
		var_04 = self method_843E("jnt_lips_L");
		if(var_04 != -1)
		{
...
clearfaceanimonanimdone(param_00,param_01,param_02)
{
	...
	var_03 = 0.3;
	param_00 clearanim(%scripted_talking,level.facialclearduration);
	disabledefaultfacialanims(0);
}
...
//Function Number: 175
animhasanimforcedefaultrate(param_00)
{
	return animhasnotetrack(param_00,"animrate_forcedefault");
}