/*******************************************************************
 * Decompiled By: Bog
 * Decompiled File: maps\killhouse_anim.gsc
 * Game: Call of Duty: Modern Warfare Remastered
 * Platform: PC
 * Function Count: 8
 * Decompile Time: 156 ms
 * Timestamp: 10/27/2023 2:46:01 AM
*******************************************************************/
...
anim_view_body()
{
	...
	level.scr_model["view_body_door"] = "worldbody_h1_sas_ct";
...
//Function Number: 6
play_idle_facial_anim(param_00)
{
	if(!isdefined(param_00))
	{
		param_00 = randomint(level.facial["idle"].size);
	}

	self setanimknoballrestart(level.facial["idle"][param_00],%head,1);
	self setanimlimited(%head,1);
	wait(getanimlength(level.facial["idle"][param_00]));
}

//Function Number: 7
gaz_face_think(param_00)
{
	level waittill("navigationTraining_end");
	level endon("out_of_firing_range");
	for(;;)
	{
		if(param_00.speaking == 1)
		{
			wait(0.1);
			param_00 setanimknoballlimited(%facial_idle_1,%head,0.1,0.1);
			param_00 setanimlimited(%head,0.11,0.1);
			continue;
		}

		if(param_00.speaking == 0)
		{
			param_00 setanimknoballlimitedrestart(%facial_idle_1,%head,0.8);
			param_00 setanimlimited(%head,0.8);
			for(;;)
			{
				if(param_00.speaking == 1)
				{
...
//Function Number: 8
newcastle_face_think(param_00)
{
	level endon("explosivesTraining_end");
	for(;;)
	{
		if(param_00.speaking)
		{
			wait(0.4);
			param_00 setanimknoballlimited(%h1_killhouse_nwc_idle_facial,%newcastle_face,0.1,0.4);
			param_00 setanimlimited(%newcastle_face,0.1,0.4);
			continue;
		}

		param_00 setanimknoballlimitedrestart(%h1_killhouse_nwc_idle_facial,%newcastle_face,0.8);
		param_00 setanimlimited(%newcastle_face,0.8);
		for(;;)
		{
			if(param_00.speaking == 1)
			{
