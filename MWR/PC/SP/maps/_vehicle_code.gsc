/*******************************************************************
 * Decompiled By: Bog
 * Decompiled File: maps\_vehicle_code.gsc
 * Game: Call of Duty: Modern Warfare Remastered
 * Platform: PC
 * Function Count: 239
 * Decompile Time: 3821 ms
 * Timestamp: 10/27/2023 2:50:11 AM
*******************************************************************/
...
sort_by_startingpos(param_00)
{
	...
	return common_scripts\utility::array_combine_reverse_keys(var_01,var_02);
}
...
vehicle_paths_non_heli(param_00)
{
	...
	{
		if(isdefined(var_04.vehicles_anims))
		{
		...
		if(isdefined(var_04.vehicles_anims))
		{
			vehicle_play_anim_from_node(var_04);
			break;
		}
		...
		if(isdefined(var_04.script_volumedown))
		{
			thread maps\_vehicle::volume_down(var_04.script_volumedown);
		}

		if(isdefined(var_04.script_volumeup))
		{
			thread maps\_vehicle::volume_up(var_04.script_volumeup);
		}
		...
		if(isdefined(var_04.script_flag_set))
		{
			...
			{
				self.vehicle_flags[var_04.script_flag_set] = 1;
			}

			self notify("vehicle_flag_arrived",var_04.script_flag_set);
		...
		if(isdefined(var_04.script_noteworthy))
		{
			if(var_04.script_noteworthy == "kill" || var_04.script_noteworthy == "forcekill")
			{
				_force_kill();
			}
		...
		if(isdefined(var_04.script_vehicleaianim))
		{
			...
			if(isdefined(var_04.script_startingposition))
			{
				self.groupedanim_pos = var_04.script_startingposition;
			}

			thread maps\_vehicle::vehicle_ai_event(var_04.script_vehicleaianim);
		}
		...
		if(isdefined(var_04.script_flag_wait))
		{
			...
			self.vehicle_flags[var_04.script_flag_wait] = 1;
			self notify("vehicle_flag_arrived",var_04.script_flag_wait);
			if(!common_scripts\utility::flag(var_04.script_flag_wait))
			{
		...
		{
			var_09 = max(0.01,length(self vehicle_getvelocity()));
			var_0A = distance(self.origin,var_04.origin);
			var_0B = max(0.01,var_0A / var_09);
			self notify("about_to_stop",var_0B);
		}
	}
...
vehicle_paths_helicopter(param_00,param_01,param_02)
{
	...
	while(isdefined(var_07))
	{
		...
		if(isdefined(var_07.script_volumedown))
		{
			thread maps\_vehicle::volume_down(var_07.script_volumedown);
		}

		if(isdefined(var_07.script_volumeup))
		{
			thread maps\_vehicle::volume_up(var_07.script_volumeup);
		}
		...
		if(isdefined(var_07.script_flag_set))
		{
			if(isdefined(self.vehicle_flags))
			{
				self.vehicle_flags[var_07.script_flag_set] = 1;
			}

			self notify("vehicle_flag_arrived",var_07.script_flag_set);
		...
		if(isdefined(var_07.script_noteworthy))
		{
			if(var_07.script_noteworthy == "kill" || var_07.script_noteworthy == "forcekill")
			{
				_force_kill();
			}
		...
		if(isdefined(var_07.script_vehicleaianim))
		{
			...
			if(isdefined(var_07.script_startingposition))
			{
				self.groupedanim_pos = var_07.script_startingposition;
			}

			thread maps\_vehicle::vehicle_ai_event(var_07.script_vehicleaianim);
		}
		...
		if(isdefined(var_07.script_flag_wait))
		{
			...
			self.vehicle_flags[var_07.script_flag_wait] = 1;
			self notify("vehicle_flag_arrived",var_07.script_flag_wait);
...
//Function Number: 36
must_stop_at_next_point(param_00)
{
	if(isdefined(param_00.script_unload))
	{
		return 1;
	}

	return isdefined(param_00.script_flag_wait) && !common_scripts\utility::flag(param_00.script_flag_wait);
}

//Function Number: 37
heli_wait_node(param_00,param_01)
{
	self endon("newpath");
	if(isdefined(param_00.script_unload) && isdefined(self.fastropeoffset))
	{
		param_00.radius = 2;
		var_02 = maps\_utility::groundpos(param_00.origin) + (0,0,self.fastropeoffset);
		if(var_02[2] > param_00.origin[2] - 2000)
		{
	...
	if(isdefined(param_01))
	{
		if(isdefined(param_01.speed))
		{
			var_03 = param_01.speed;
			var_04 = 25;
			var_05 = undefined;
			if(isdefined(param_01.script_decel))
			{
				var_05 = param_01.script_decel;
			}
			else if(must_stop_at_next_point(param_00))
			{
			}

			if(isdefined(param_01.script_accel))
			{
				var_04 = param_01.script_accel;
			}
			else
			{
				var_06 = var_03 / 4;
	...
	setvehgoalnode(param_00);
	if(isdefined(param_00.radius))
	{
		self neargoalnotifydist(param_00.radius);
	...
	if(isdefined(param_00.script_stopnode))
	{
		if(param_00.script_stopnode)
		{
...
//Function Number: 38
helipath(param_00,param_01,param_02)
{
	self setairresitance(30);
	self vehicle_setspeed(param_01,param_02,10);
	maps\_vehicle::vehicle_paths(common_scripts\utility::getstruct(param_00,"targetname"));
}
...
setvehgoalnode(param_00)
{
	...
	if(isdefined(param_00.script_stopnode))
	{
		var_01 = param_00.script_stopnode;
	}
	...
	var_02 = isdefined(param_00.script_anglevehicle) && param_00.script_anglevehicle;
	var_03 = isdefined(param_00.script_goalyaw) && param_00.script_goalyaw;
	if(isdefined(param_00.script_anglevehicle) || isdefined(param_00.script_goalyaw))
	{
		forcetarget(param_00,var_03,var_02);
	}
	else
	{
		unforcetarget();
	...
	{
		if(!common_scripts\utility::flag(param_00.script_flag_wait))
		{
			var_01 = 1;
		}
	}
	...
	setvehgoalpos_wrap(param_00.origin,var_01);
}

//Function Number: 40
forcetarget(param_00,param_01,param_02)
{
	if(param_01)
	{
		self cleartargetyaw();
...
vehicle_kill_common(param_00,param_01)
{
	...
	var_06 = get_deathanim_info(var_03);
	if(isdefined(var_06) && isdefined(var_06.need_dummy) && var_06.need_dummy)
	{
		var_07 = "body_animate_jnt";
		maps\_vehicle::vehicle_to_dummy(var_07,0);
	}
	...
	var_08 = vehicle_should_do_rocket_death(var_04,param_00,param_01);
	thread kill_deathanim_thread(var_03,param_00);
	thread _kill_fx(var_04,var_08);
...
vehicle_kill()
{
	...
	for(;;)
	{
		...
		{
			self.modeldummy thread lights_off("all",var_01);
		}
		...
		if(isdefined(var_00) && isdefined(::maps\_vehicle_aianim::guy_vehicle_death) && isdefined(self.riders))
		{
...
//Function Number: 58
vehicle_do_crash_helicopter(param_00,param_01,param_02)
{
	...
	detach_getoutrigs();
	if(isdefined(param_00) && param_00 == level.player && isdefined(self.rocket_destroyed_for_achievement) && level.player _hasweapon("rpg"))
	{
...
_hasweapon(param_00)
{
	...
	{
		if(issubstr(var_01[var_02],param_00))
		{
			return 1;
		}
	}
...
helicopter_crash_move(param_00,param_01)
{
	...
	if(var_05)
	{
		...
		self neargoalnotifydist(var_02.radius);
		self setgoalpos(var_02.origin,0);
...
helicopter_crash_path(param_00)
{
	...
	while(isdefined(param_00.target))
	{
		...
		self neargoalnotifydist(var_01);
		self setgoalpos(param_00.origin,0);
		common_scripts\utility::waittill_any("goal","near_goal");
	}
}
...
_kill_fx(param_00,param_01)
{
	...
	var_04 = 0;
	foreach(var_06 in level.vehicle_death_fx[var_03])
	{
		if((!isdefined(var_06.waitdelay) || var_06.waitdelay == 0) && param_00 == "vehicle_bm21_mobile_cover" && level.script == "hunted")
		{
			var_06.waitdelay = int(var_04 / 2) * 0.05 + 0.05;
		}

		thread kill_fx_thread(param_00,var_06,var_02);
		var_04++;
	}
}
...
aircraft_wash_thread(param_00,param_01)
{
	...
	{
		var_0A = level.treadsfx_max_distance;
	}
	...
	for(;;)
	{
		...
		if(isdefined(var_04))
		{
			var_14 = var_12 gettagorigin(var_04);
			var_15 = var_12 gettagangles(var_04);
			var_15 = combineangles(var_15,(-90,0,0));
		}
		else
		{
		...
		var_16 = anglestoup(var_15) * -1;
		var_0E++;
		if(var_0E > var_0D)
		{
			var_0E = var_0D;
			var_10 = bullettrace(var_14,var_14 + var_16 * var_06,0,var_12,0,0,0,0,0,1,0,1);
		}

		var_17 = distance(var_14,level.player.origin);
		if(var_17 > var_0A)
		{
		...
		var_18 = distance(var_14,var_10["position"]);
		var_19 = get_wash_info(self,var_10,var_16,var_04);
		var_1A = get_wash_fx(var_19,var_04);
		if(!isdefined(var_1A))
		{
			stop_tread_audio_loop();
			continue;
		...
		var_1B = get_wash_soundfx(var_19,var_04);
		if(tread_audio_should_stop_loop(var_1B,self.oldtreadsoundfx))
		{
			stop_tread_audio_loop();
		}

		if(var_10["fraction"] < var_07)
		{
			if(isdefined(level._heli_landed_fx) && isdefined(level._heli_landed_fx[var_1A]))
			{
				var_1A = level._heli_landed_fx[var_1A];
			}
		}
		...
		var_0C = var_18 - var_05 / var_06 - var_05 * var_08 - var_09 + var_09;
		var_0C = max(var_0C,var_09);
		var_1C = var_10["position"];
		var_1D = var_10["normal"];
		var_18 = vectordot(var_1C - var_14,var_1D);
		var_1E = var_14 + (0,0,var_18);
		var_1F = var_1C - var_1E;
		if(var_1D[2] < -0.99)
		{
			continue;
		}

		var_20 = vectortoangles(var_1D);
		if(angleclamp180(var_20[0]) > -90 + var_13)
		{
			var_1D = anglestoforward((-90 + var_13,var_20[1],0));
		}

		if(length(var_1F) < 1)
		{
			var_1F = anglestoforward(var_15 + (0,180,0));
		}

		if(abs(vectordot(vectornormalize(var_1F),var_1D)) > 0.999)
		{
			continue;
		}

		playfx(var_1A,var_1C,var_1D,var_1F);
		if(isdefined(var_1B) && !self.soundisalreadyplaying && var_17 < var_0A)
		{
			thread maps\_utility::play_loop_sound_on_tag(var_1B,var_04,1,1);
			self.soundisalreadyplaying = 1;
			self.oldtreadsoundfx = var_1B;
		}
	}
}
...
//Function Number: 128
get_wash_info(param_00,param_01,param_02,param_03)
{
	var_04 = spawnstruct();
	var_04.surface = param_01["surfacetype"];
	var_04.bank = undefined;
	var_05 = vectordot((0,0,-1),param_02);
	if(var_05 >= 0.97)
	{
		var_04.bank = undefined;
	}
	...
	{
		var_04.bank = "_bank_lg";
	}

	var_04.vehicleclass = param_00.classname;
	if(!issubstr(param_00.classname,"vehicle") && isdefined(param_00.vehicletype))
	{
		var_04.vehicleclass = "script_vehicle_" + param_00.vehicletype;
	}

	if(isdefined(level._vehicle_effect_custom_param) && isdefined(level._vehicle_effect_custom_param[var_04.vehicleclass]) && isdefined(level._vehicle_effect_custom_param[var_04.vehicleclass].get_surface_override_function))
	{
		var_04.surface = param_00 [[ level._vehicle_effect_custom_param[var_04.vehicleclass].get_surface_override_function ]](var_04.surface,param_03);
	}
...
//Function Number: 129
get_wash_fx(param_00,param_01)
{
	return get_wash_effect(param_00.vehicleclass,param_00.surface,param_00.bank);
}

//Function Number: 130
get_wash_soundfx(param_00,param_01)
{
	return get_wash_sound_effect(param_00.vehicleclass,param_00.surface,param_00.bank);
}
...
//Function Number: 132
get_wash_sound_effect(param_00,param_01,param_02)
{
	if(isdefined(param_02))
	{
		var_03 = param_01 + param_02;
		if(isdefined(level._vehicle_sound_effect[param_00][var_03]))
		{
			return level._vehicle_sound_effect[param_00][var_03];
		}
	}

	return get_vehicle_sound_effect(param_00,param_01);
}
...
//Function Number: 134
get_vehicle_sound_effect(param_00,param_01)
{
	if(!isdefined(level._vehicle_sound_effect[param_00]))
	{
		return undefined;
	}

	if(!isdefined(level._vehicle_sound_effect[param_00][param_01]) && param_01 != "default" && param_01 != "default_script_model")
	{
		return get_vehicle_sound_effect(param_00,"default");
	}

	return level._vehicle_sound_effect[param_00][param_01];
}
...
do_multiple_treads()
{
	...
	for(;;)
	{
		...
		tread(_get_dummy(),var_00,"tag_wheel_back_left","back_left",0);
		wait(0.05);
		tread(_get_dummy(),var_00,"tag_wheel_back_right","back_right",0);
		wait(0.05);
	}
}
...
tread_wait()
{
	...
	if(isdefined(self.vehicle_treadfx_min_speed) && var_00 < self.vehicle_treadfx_min_speed)
	{
		return -1;
	}
...
//Function Number: 141
get_treadsfx(param_00,param_01)
{
	var_02 = self getwheelsurface(param_01);
	if(!isdefined(param_00.vehicletype))
	{
	...
	var_04 = param_01.classname;
	return get_vehicle_sound_effect(var_04,var_03);
}
...
vehicle_shoot_shock()
{
	...
	for(;;)
	{
		...
		soundscripts\_snd::snd_message("start_vehicle_shell_shock",var_02);
	}
}
...
vehicle_handleunloadevent()
{
	...
	for(;;)
	{
		self waittill("unload",var_01);
		if(isdefined(var_01))
		{
			self.unload_group = var_01;
		}

		maps\_vehicle::vehicle_unload(var_01);
	}
}
...
regen_front_armor(param_00,param_01)
{
	...
	{
		self.health = self.health + int(param_01 * level.vehicle_frontarmor[self.classname]);
	}
}
...
getonpath(param_00)
{
	...
	else if(isdefined(var_01.speed))
	{
		...
		{
			self vehicle_setspeed(var_01.speed,20,10);
		}
	}
...
_scripted_spawn(param_00)
{
	...
	level notify("vehiclegroup spawned" + param_00,var_02);
	return var_02;
}
...
_vehicle_spawn(param_00)
{
	...
	{
		level notify("new_vehicle_spawned" + var_01.script_noteworthy,var_01);
	}
...
//Function Number: 218
move_effects_ent_here(param_00,param_01)
{
	var_02 = deathfx_ent();
	var_02 unlink();
	if(isdefined(param_01))
...
move_lights_here(param_00,param_01)
{
	...
	foreach(var_04 in var_02)
	{
		param_00 lights_on_internal(var_04,self.classname);
		thread lights_off(var_04,self.classname);
	}
}
...
get_light_on_groups()
{
	...
	var_00 = [];
	var_01 = getarraykeys(self.lights);
	var_02 = getarraykeys(level.vehicle_lights_group[self.classname]);
	foreach(var_04 in var_02)
	{
		if(var_04 == "all")
		{
		...
		foreach(var_06 in var_01)
		{
			if(common_scripts\utility::array_contains(level.vehicle_lights_group[self.classname][var_04],var_06))
			{
				var_00[var_00.size] = var_04;
				break;
			}
...
//Function Number: 223
kill_deathanim_thread(param_00,param_01)
{
	...
	var_02 = get_deathanim_info(param_00);
	if(!isdefined(var_02))
	{
		return;
	}

	var_03 = get_deathanim(var_02,param_01);
	kill_deathanim_play(var_03,var_02);
}

//Function Number: 224
get_deathanim_info(param_00)
{
	...
	{
		var_01 = param_00 + "_" + self.vehicle_deathanim_state;
		if(isdefined(level.vehicle_deathanim[var_01]))
		{
			return level.vehicle_deathanim[var_01];
		}
	}

	if(isdefined(level.vehicle_deathanim[param_00]))
	{
		return level.vehicle_deathanim[param_00];
	}
...
//Function Number: 225
get_deathanim(param_00,param_01)
{
	...
	if(param_00.is_directional)
	{
		return get_directional_deathanim(param_00,param_01);
	}

	return common_scripts\utility::random(param_00.anims);
}

//Function Number: 226
get_directional_deathanim(param_00,param_01)
{
	if(!isdefined(param_01))
	{
		return common_scripts\utility::random(param_00.anims);
	...
	var_02 = get_attacker_direction(param_01);
	if(!isarray(param_00.anims[var_02]))
	{
		return param_00.anims[var_02];
	}

	return common_scripts\utility::random(param_00.anims[var_02]);
}

//Function Number: 227
get_attacker_direction(param_00)
{
	var_01 = vectornormalize(anglestoforward(self.angles));
	var_02 = vectortoangles(var_01);
	var_03 = vectortoangles(param_00.origin - self.origin);
...
//Function Number: 228
kill_deathanim_play(param_00,param_01)
{
	self endon("death");
	maps\_utility::ent_flag_init("kill_deathanim_active");
	maps\_utility::ent_flag_set("kill_deathanim_active");
	self notify("kill_deathanim",param_00);
	if(isdefined(param_01.death_fx_on_self) && param_01.death_fx_on_self)
	{
		self.death_fx_on_self = 1;
	}

	maps\_utility::ent_flag_init("kill_deathanim_force_crash_end");
	if(isdefined(param_01.delay_crash_force_end) && param_01.delay_crash_force_end)
	{
		maps\_utility::ent_flag_set("kill_deathanim_force_crash_end");
	}

	self setcandamage(param_01.can_damage);
	thread kill_deathanim_delete(param_01.delete_time,param_00);
	if(isdefined(param_01.notetrack_notifies))
	{
		foreach(var_03 in param_01.notetrack_notifies)
		{
			kill_deathanim_notify(param_00,var_03);
		}
	}

	if(param_01.turn_off_engine)
	{
		self vehicle_turnengineoff();
	}

	if(param_01.alter_velocity)
	{
		self vehicle_setspeed(param_01.alter_velocity_speed,param_01.alter_velocity_accel,param_01.alter_velocity_decel);
	}

	self useanimtree(#animtree);
	if(param_01.clear_anims)
	{
		self clearanim(%root,param_01.clear_anims_time);
	}

	var_05 = _get_dummy();
	var_05 setanim(param_00,1,0,1);
	kill_deathanim_crash_delay(param_00,param_01);
...
{
	self endon("death");
	if(!param_01.delay_crash)
	{
		return;
	}

	var_02 = 0;
	if(animhasnotetrack(param_00,"stop_deathanim_crash_delay"))
	{
		var_03 = getanimlength(param_00);
		var_04 = getnotetracktimes(param_00,"stop_deathanim_crash_delay");
		var_02 = var_03 * var_04[0];
	}
	else if(param_01.delay_crash_time > 0)
	{
		var_02 = param_01.delay_crash_time;
	}
	else
	{
		var_02 = getanimlength(param_00);
...
//Function Number: 230
kill_deathanim_delete(param_00,param_01)
{
	...
	else
	{
		...
		if(param_00 == 0)
		{
			param_00 = getanimlength(param_01);
		}
...
//Function Number: 231
kill_deathanim_notify(param_00,param_01)
{
	self endon("death");
	if(!animhasnotetrack(param_00,param_01))
	{
		return;
	}
...
//Function Number: 232
kill_deathanim_set_state(param_00)
{
	self.vehicle_deathanim_state = param_00;
}
...
//Function Number: 234
tread_audio_should_stop_loop(param_00,param_01)
{
...
//Function Number: 237
vehicle_play_anim_from_node(param_00)
{
	var_01 = "vehicles";
	...
	var_02 = strtok(param_00.vehicles_anims,":;, ");
	var_03 = var_02[0];
	if(!isdefined(level.scr_anim[var_01][var_03]))
	{
		return;
	}

	var_04 = "body_animate_jnt";
	maps\_vehicle::vehicle_to_dummy(var_04,1);
	vehicle_dummy_add_collison(var_04);
	self method_8451();
	self makevehiclenotcollidewithplayers(1);
	self setcontents(0);
	var_05 = self vehicle_getvelocity();
	self vehicle_setspeedimmediate(0,3000,3000);
	self.modeldummy maps\_utility::assign_animtree(var_01);
	var_06 = self.modeldummy;
	if(var_02.size > 1)
	{
		var_07 = common_scripts\utility::get_target_ent(var_02[1]);
		if(isdefined(var_07))
		{
			var_06 = var_07;
		}
	...
	if(var_06 != self.modeldummy)
	{
		var_08 = length(var_05);
		var_09 = distance(self.origin,var_06.origin);
		var_0A = var_09 / var_08;
		self.modeldummy moveto(var_06.origin,var_0A);
		self.modeldummy rotateto(var_06.angles,var_0A);
		var_0B = int(var_0A / 0.05) * 0.05;
		if(var_0B >= 0.05)
		{
			wait(var_0B);
		}
	...
	var_06 maps\_anim::anim_single_solo(self.modeldummy,var_03);
}

//Function Number: 238
vehicle_dummy_add_collison(param_00)
{
	if(isdefined(param_00))
	{
		...
		var_01 = self.modeldummy.origin - self.modeldummy gettagorigin(param_00);
		var_02 = self.modeldummy.angles - self.modeldummy gettagangles(param_00);
		self.modeldummycoll linkto(self.modeldummy,param_00,var_01,var_02);
		self.modeldummycoll method_8450();
	...
	self.modeldummy method_8450();
}

//Function Number: 239
model_dummycoll_death()
{
	var_00 = self.modeldummycoll;
	var_00 endon("death");
	var_00 endon("stop_model_dummy_death");
	while(isdefined(self))
	{
