 ...
 * Timestamp: 10/27/2023 2:44:32 AM
*******************************************************************/
...
init_animset_reactions()
{
	...
	{
		var_03 = length(getmovedelta(var_02,0,1));
		if(var_03 > level.longestrunwizby)
		{
			anim.longestrunwizby = var_03;
		}
	}
...
bulletwhizbyreaction()
{
	...
	var_01 = isdefined(self.whizbyenemy) && distancesquared(self.origin,self.whizbyenemy.origin) < 160000;
	self animmode("gravity");
	self orientmode("face current");
	if(isdefined(self.disable_crouch_whizby_react) || var_01 || common_scripts\utility::cointoss())
	{
		self clearanim(%animscript_root,0.1);
		var_02 = [];
		if(animscripts\utility::usingsmg())
		{
			var_02[0] = %smg_exposed_idle_reacta;
			var_02[1] = %smg_exposed_idle_reactb;
			var_02[2] = %smg_exposed_idle_twitch;
			var_02[3] = %smg_exposed_idle_twitch_v4;
		}
		else
		{
			var_02[0] = %exposed_idle_reacta;
			var_02[1] = %exposed_idle_reactb;
			var_02[2] = %exposed_idle_twitch;
			var_02[3] = %exposed_idle_twitch_v4;
		}

		var_03 = var_02[randomint(var_02.size)];
		if(var_01)
		{
			var_04 = 1 + randomfloat(0.5);
		}
		else
		{
			var_04 = 0.2 + randomfloat(0.5);
		}

		self setflaggedanimknobrestart("reactanim",var_03,1,0.1,1);
		animscripts\notetracks::donotetracksfortime(var_04,"reactanim");
		self clearanim(%animscript_root,0.1);
		if(!var_01 && self.stairsstate == "none" && !isdefined(self.disable_dive_whizby_react))
		{
			var_05 = 1 + randomfloat(0.2);
			var_06 = animscripts\utility::randomanimoftwo(%exposed_dive_grenade_b,%exposed_dive_grenade_f);
			self setflaggedanimknobrestart("dive",var_06,1,0.1,var_05);
	...
	else
	{
		...
		var_05 = 1.2 + randomfloat(0.3);
		if(self.a.pose == "stand")
		{
			self clearanim(%animscript_root,0.1);
			self setflaggedanimknobrestart("crouch",%exposed_stand_2_crouch,1,0.1,var_05);
		...
		{
			var_08 = vectornormalize(self.whizbyenemy.origin - self.origin);
		}
		else
		{
			var_08 = var_08;
		...
		{
			var_09 = animscripts\utility::randomanimoftwo(%exposed_crouch_idle_twitch_v2,%exposed_crouch_idle_twitch_v3);
			self clearanim(%animscript_root,0.1);
			self setflaggedanimknobrestart("twitch",var_09,1,0.1,1);
			animscripts\shared::donotetracks("twitch");
		}
		else
		{
			var_0A = animscripts\utility::randomanimoftwo(%exposed_crouch_turn_180_left,%exposed_crouch_turn_180_right);
			self clearanim(%animscript_root,0.1);
			self setflaggedanimknobrestart("turn",var_0A,1,0.1,1);
