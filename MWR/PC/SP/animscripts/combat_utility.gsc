 ...
 * Platform: PC
 * Function Count: 92
 * Decompile Time: 1668 ms
 * Timestamp: 10/27/2023 2:43:44 AM
*******************************************************************/
...
fireuntiloutofammointernal(param_00,param_01,param_02,param_03,param_04)
{
	...
	var_05 = 0;
	var_06 = getanimlength(param_01);
	var_07 = animhasnotetrack(param_01,"fire");
	var_08 = weaponclass(self.weapon) == "rocketlauncher";
	...
	while(var_05 < param_03 && param_03 > 0)
	{
		...
		if(!var_07 || param_03 == 1 && self.shootstyle == "single")
		{
			self waittillmatch("end",param_00);
			var_09 = var_06;
		}

		if(param_04 - var_09 > 0)
		{
			wait(param_04 - var_09);
		}
	}
...
aimedatshootentorpos()
{
	...
	var_05 = level.aimpitchdifftolerance;
	if(self.a.pose == "crouch" && distancesquared(self geteye(),self.shootpos) < level.aimpitchdiffclosedistsq)
	{
		var_05 = level.aimpitchdiffclosetolerance;
	}

	return animscripts\utility::absangleclamp180(var_00[0] - var_02[0]) <= var_05;
}
...
//Function Number: 38
h1_grenade_attach_detach_listener(param_00)
{
	if(param_00 == "grenade_left" || param_00 == "grenade_right")
	{
		self.h1_scripted_grenade = spawn("script_model",self gettagorigin("tag_inhand"));
		var_01 = animscripts\utility::getgrenademodel();
		self.h1_scripted_grenade setmodel(var_01);
		self.h1_scripted_grenade linkto(self,"tag_inhand",(0,0,3),(0,0,0));
		return;
	}

	if(param_00 == "grenade_throw")
	{
...
//Function Number: 50
isgrenadepossafe(param_00,param_01)
{
	if(isdefined(level.throwgrenadeatplayerasap) && usingplayergrenadetimer())
	{
		return 1;
	}
	...
	var_06 = undefined;
	var_07 = 100000000;
	for(var_08 = 0;var_08 < self.squad.members.size;var_08++)
	{
		if(!isalive(self.squad.members[var_08]))
		{
			continue;
		}

		var_09 = distancesquared(self.squad.members[var_08].origin,param_01);
		if(var_09 > var_03)
		{
			continue;
		}
		...
		if(var_09 < var_05)
		{
			...
			var_06 = var_04;
			var_05 = var_09;
			var_04 = self.squad.members[var_08];
			continue;
		}
		...
		{
			var_07 = var_09;
			var_06 = self.squad.members[var_08];
		}
	}

	if(isdefined(var_04) && sighttracepassed(var_04 geteye(),param_01,0,undefined))
	{
		return 0;
	}

	if(isdefined(var_06) && sighttracepassed(var_04 geteye(),param_01,0,undefined))
	{
		return 0;
	}
...
{
	if(!isgrenadepossafe(param_00,param_01))
	{
		return 0;
	}
...
trygrenade(param_00,param_01)
{
	...
	if(isdefined(self.enemy) && param_00 == self.enemy)
	{
		...
		{
			if(!isgrenadepossafe(param_00,param_00.origin))
			{
				return 0;
			}
		...
		else
		{
			...
			if(!isgrenadepossafe(param_00,var_03))
			{
				return 0;
			}
