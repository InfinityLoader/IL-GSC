 ...
 * Timestamp: 10/27/2023 2:44:02 AM
*******************************************************************/
...
init_animset_cover_wall()
{
	...
	var_00["hide_idle_twitch"] = animscripts\utility::array(%covercrouch_twitch_2,%covercrouch_twitch_3,%covercrouch_twitch_4);
	...
	var_00["shotgun_reload"] = %shotgun_covercrouch_reload;
	...
	var_00["hide_idle_twitch"] = animscripts\utility::array(%coverstand_hide_idle_twitch01,%coverstand_hide_idle_twitch03,%coverstand_hide_idle_twitch04,%coverstand_hide_idle_twitch05);
...
cover_wall_think(param_00)
{
	...
	if(isdefined(self.weapon) && animscripts\utility::usingmg() && isdefined(self.node) && isdefined(self.node.turretinfo) && canspawnturret())
	{
		...
		if(ismuzzleoptimizedsmall(self.weapon) && turretoptimizedsmallversionexists(var_01))
		{
			var_01 = var_01 + "_muzzle_small";
		}

		useselfplacedturret(var_01,var_02,0);
	}
	...
	{
		var_05 = 0.4;
		var_06 = loophide(var_05);
		if(distancesquared(self.origin,self.covernode.origin) > 1)
		{
			thread animscripts\shared::movetonodeovertime(self.covernode,var_06);
			wait(var_06 / 2);
			if(param_00 == "crouch")
			{
				self.a.pose = "crouch";
	...
	if(!self.fixednode)
	{
		var_07.movetonearbycover = ::animscripts\cover_behavior::movetonearbycover;
	}

	var_07.reload = ::coverreload;
	var_07.leavecoverandshoot = ::popupandshoot;
	var_07.look = ::look;
	var_07.fastlook = ::fastlook;
	var_07.idle = ::idle;
	var_07.flinch = ::flinch;
	var_07.grenade = ::trythrowinggrenade;
	var_07.grenadehidden = ::trythrowinggrenadestayhidden;
	var_07.blindfire = ::blindfire;
	animscripts\cover_behavior::main(var_07);
}
...
//Function Number: 4
ismuzzleoptimizedsmall(param_00)
{
	return isendstr(param_00,"_muzzle_small");
}

//Function Number: 5
turretoptimizedsmallversionexists(param_00)
{
	return param_00 == "rpd_bipod_crouch" || param_00 == "rpd_bipod_stand" || param_00 == "saw_bipod_crouch";
}
...
coverreload()
{
	var_00 = animscripts\utility::animarray("reload");
	if(animscripts\utility::isshotgun(self.weapon) && isdefined(self.a.array["shotgun_reload"]))
	{
		var_00 = animscripts\utility::animarray("shotgun_reload");
	}

	var_01 = animscripts\combat_utility::reload(2,var_00);
...
//Function Number: 29
createturret(param_00,param_01,param_02,param_03)
{
	...
	var_04 = spawnturret("misc_turret",param_00.origin,param_01);
	...
	{
		var_04.leftarc = param_00.leftarc;
	}

	if(isdefined(param_00.rightarc))
	{
		var_04.rightarc = param_00.rightarc;
	}

	if(isdefined(param_00.toparc))
	{
		var_04.toparc = param_00.toparc;
	}

	if(isdefined(param_00.bottomarc))
	{
		var_04.bottomarc = param_00.bottomarc;
	}
...
//Function Number: 31
useselfplacedturret(param_00,param_01,param_02)
{
	...
	var_03 = createturret(self.node.turretinfo,param_00,param_01,param_02);
	if(self useturret(var_03))
	{
		var_03 thread deleteifnotused(self);
		if(isdefined(self.turret_function))
		{
			thread [[ self.turret_function ]](var_03);
		}
...
loophide(param_00)
{
	...
	var_01 = param_00;
	if(self.covertype == "stand" && self getanimweight(%exposed_aiming) != 0)
	{
		self setanimknoball(%exposed_stand_2_coverstand_hide,%body,1,param_00);
		var_01 = getanimlength(%exposed_stand_2_coverstand_hide);
	}
	else
	{
...
getbestcovermode()
{
	...
	else
	{
		...
		if(self.a.atconcealmentnode)
		{
			...
			if(var_01 > 10 || !self.covernode.crouchingisok)
			{
				return "stand";
			}
		}
		else
		{
			...
			if((var_01 > 0 || !self.covernode.crouchingisok) && !isdefined(self.force_crouch) || !self.force_crouch)
			{
				return "stand";
			}
		}
