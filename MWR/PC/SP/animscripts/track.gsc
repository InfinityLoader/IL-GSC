 ...
 * Timestamp: 10/27/2023 2:44:55 AM
*******************************************************************/
...
trackloop(param_00,param_01,param_02,param_03,param_04)
{
	...
	{
		incranimaimweight();
		if(self method_843E("tag_flash") == -1)
		{
			wait(0.05);
			continue;
		}
		...
		else if(isdefined(var_18) && isdefined(self.forceaimtorwardsenemy) && isdefined(self.enemy))
		{
			var_18 = self.enemy getshootatpos();
		}
		...
		{
			var_20 = self.proneaimlimits;
			var_1E = clamp(var_1E,var_20.downaimlimit,var_20.upaimlimit);
			var_1F = clamp(var_1F,var_20.leftaimlimit,var_20.rightaimlimit);
		}
		else
		{
			var_20 = spawnstruct();
			var_20.rightaimlimit = self.rightaimlimit;
			var_20.leftaimlimit = self.leftaimlimit;
			var_20.upaimlimit = self.upaimlimit;
			var_20.downaimlimit = self.downaimlimit;
		}

		if(animscripts\utility::isspaceai())
		{
			var_21 = self.angles[2] * -1;
			var_22 = var_1E * cos(var_21) - var_1F * sin(var_21);
			var_23 = var_1E * sin(var_21) + var_1F * cos(var_21);
			var_1E = var_22;
			var_1F = var_23;
			var_1E = clamp(var_1E,var_20.downaimlimit,var_20.upaimlimit);
			var_1F = clamp(var_1F,var_20.leftaimlimit,var_20.rightaimlimit);
		...
		var_24 = squared(var_0B);
		var_09 = self.relativedir;
		var_25 = self.movemode != "stop" || !var_08;
		if(var_25)
		{
			var_26 = var_1F - var_05;
			if(squared(var_26) > var_24)
			{
				var_1F = var_05 + clamp(var_26,-1 * var_0B,var_0B);
				var_1F = clamp(var_1F,var_20.leftaimlimit,var_20.rightaimlimit);
			}

			var_27 = var_1E - var_06;
			if(squared(var_27) > var_24)
			{
				var_1E = var_06 + clamp(var_27,-1 * var_0B,var_0B);
				var_1E = clamp(var_1E,var_20.downaimlimit,var_20.upaimlimit);
			}
		}
		...
		var_05 = var_1F;
		var_06 = var_1E;
		trackloop_setanimweights(param_00,param_01,param_02,param_03,param_04,var_1E,var_1F,var_20);
		wait(0.05);
	}
}
...
//Function Number: 7
trackloop_setanimweights(param_00,param_01,param_02,param_03,param_04,param_05,param_06,param_07)
{
	var_08 = 0;
	var_09 = 0;
	var_0A = 0;
	...
	{
		var_0D = self.aimblendtime;
	}

	if(param_06 > 0)
	{
		var_0B = clamp(param_06 / self.animaimlimit.rightaimlimit * self.a.aimweight,0,1);
		var_0A = 1;
	}
	else if(param_06 < 0)
	{
		var_09 = clamp(param_06 / self.animaimlimit.leftaimlimit * self.a.aimweight,0,1);
		var_0A = 1;
	}

	if(param_05 > 0)
	{
		var_0C = param_05 / param_07.upaimlimit * self.a.aimweight;
		var_0A = 1;
	}
	else if(param_05 < 0)
	{
		var_08 = param_05 / param_07.downaimlimit * self.a.aimweight;
		var_0A = 1;
	}

	self setanimlimited(param_00,var_08,var_0D,1,1);
	self setanimlimited(param_01,var_09,var_0D,1,1);
	self setanimlimited(param_02,var_0B,var_0D,1,1);
	self setanimlimited(param_03,var_0C,var_0D,1,1);
	if(isdefined(param_04))
	{
		self setanimlimited(param_04,var_0A,var_0D,1,1);
	}
}
