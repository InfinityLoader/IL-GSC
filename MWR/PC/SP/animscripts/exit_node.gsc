 ...
 * Timestamp: 10/27/2023 2:44:09 AM
*******************************************************************/
...
startmovetransition()
{
	...
	{
		var_02 = self [[ self.exittypefunc ]](var_04);
	}
	...
	{
		if(!self [[ self.exitconditioncheckfunc ]](var_04))
		{
			return;
		}
	}
	else if(!checktransitionconditions(var_02,var_04))
	{
		return;
	}

	var_07 = isdefined(level.exposedtransition[var_02]);
	if(!var_03 && !isdefined(self.exittypefunc))
	{
	...
	var_10 = distancesquared(self.origin,self.coverexitpos) * 1.25;
...
determinenodeexittype(param_00)
{
	...
	var_01 = self.a.pose;
	if(var_01 == "prone" && !isdefined(self.enableproneexitnode))
	{
		var_01 = "crouch";
	}
...
donodeexitanimation(param_00,param_01)
{
	...
	donodeexitanimation_impl(var_02);
}
...
//Function Number: 12
donodeexitanimation_impl(param_00)
{
	...
	self setflaggedanimknoballrestart("coverexit",param_00,%body,1,var_01,self.movetransitionrate);
	childthread animscripts\stairs_utility::threadcheckstairstransition(param_00,0,0.1,::donodeexitcleanupfunc);
	...
	animscripts\utility::checkforturnwithanim(param_00);
	self orientmode("face motion");
	self animmode("none",0);
	childthread animscripts\stairs_utility::threadcheckstairstransition(param_00,0,0.1);
	finishcoverexitnotetracks("coverexit",param_00);
	self notify("killThreadCheckStairsTransition");
	self clearanim(%animscript_root,0.2);
	self orientmode("face default");
	self animmode("normal",0);
...
//Function Number: 13
finishcoverexitnotetracks(param_00,param_01)
{
