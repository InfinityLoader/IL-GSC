/*******************************************************************
 * Decompiled By: Bog
 * Decompiled File: maps\mp\zombies\_zm_perks.gsc
 * Game: Call of Duty: Black Ops 2
 * Platform: PC
 * Function Count: 95
 * Decompile Time: 496 ms
 * Timestamp: 10/27/2023 3:03:16 AM
*******************************************************************/
...
pap_weapon_move_in(trigger,origin_offset,angles_offset)
{
	...
	trigger.worldgun rotateto(90 + VectorScale((0,1,0)),self.angles + angles_offset,0.35,0);
	offsetdw = VectorScale((1,1,1));
	if(IsDefined(trigger.worldgun.worldgundw))
	{
		trigger.worldgun.worldgundw rotateto(90 + VectorScale((0,1,0)),self.angles + angles_offset,0.35,0);
	}
...
third_person_weapon_upgrade(current_weapon,upgrade_weapon,packa_rollers,perk_machine,trigger)
{
	...
	if(IsDefined(rel_entity))
	{
		...
		{
			origin_offset = VectorScale((0,0,1));
		}

		angles_offset = VectorScale((0,1,0));
	...
	{
		perk_machine.wait_flag rotateto(179 + VectorScale((1,0,0)),perk_machine.wait_flag.angles,0.25,0);
	}
	...
	trigger.worldgun = spawn_weapon_model(upgrade_weapon,undefined,origin_base + origin_offset,90 + VectorScale((0,1,0)),angles_base + angles_offset);
	worldgundw = undefined;
	if(maps/mp/zombies/_zm_magicbox::weapon_is_dual_wield(upgrade_weapon))
	{
		worldgundw = spawn_weapon_model(upgrade_weapon,maps/mp/zombies/_zm_magicbox::get_left_hand_weapon_model_name(upgrade_weapon),origin_base + origin_offset + offsetdw,90 + VectorScale((0,1,0)),angles_base + angles_offset);
	}
...
activate_packapunch()
{
	...
	self vibrate(VectorScale((0,-1,0)),100,0.3,0.4);
	timer = 0;
	duration = 0.05;
}
...
turn_sleight_on()
{
	while(1)
	{
		...
		level notify("specialty_fastreload_power_on",3);
		...
		for(i = 0;i < machine.size;i++)
		{
			if(IsDefined(machine[i].classname) && machine[i].classname == "script_model")
			{
				...
				else
				{
					...
					machine[i] notify("stop_loopsound",3);
					machine[i] thread play_loop_on_machine();
...
turn_jugger_on()
{
	while(1)
	{
		...
		level notify("specialty_armorvest_power_on",3);
		array_thread(machine_triggers,::set_power_on,1);
...
turn_doubletap_on()
{
	while(1)
	{
		...
		level notify("specialty_rof_power_on",3);
		array_thread(machine_triggers,::set_power_on,1);
...
turn_marathon_on()
{
	while(1)
	{
		...
		level notify("specialty_longersprint_power_on",3);
		array_thread(machine_triggers,::set_power_on,1);
...
turn_deadshot_on()
{
	while(1)
	{
		...
		level notify("specialty_deadshot_power_on",3);
		array_thread(machine_triggers,::set_power_on,1);
...
turn_tombstone_on()
{
	...
	while(1)
	{
		...
		level notify("specialty_scavenger_power_on",3);
		array_thread(machine_triggers,::set_power_on,1);
...
turn_additionalprimaryweapon_on()
{
	while(1)
	{
		...
		level notify("specialty_additionalprimaryweapon_power_on",3);
		array_thread(machine_triggers,::set_power_on,1);
...
turn_chugabud_on()
{
	...
	while(1)
	{
		...
		level notify("specialty_finalstand_power_on",3);
		array_thread(machine_triggers,::set_power_on,1);
...
vending_trigger_think()
{
	...
	if(!(start_on))
	{
		...
		level waittill(_k2045,notify_name);
	}
...
#/
	...
	result == perk_str
		...
		case "specialty_deadshot":
			...
			break;
	IsDefined(level.disable_deadshot_clientfield) && level.disable_deadshot_clientfield
			break;
...
quantum_bomb_give_nearest_perk_result(position)
{
	...
	for(i = 0;i < players.size;i++)
	{
		...
		else if(!player hasperk(perk) && (!IsDefined(player.perk_purchased) || player.perk_purchased != perk) && randomint(5))
		{
			if(player == self)
			{
...
perk_machine_spawn_init()
{
	...
	for(i = 0;i < pos.size;i++)
	{
		...
		{
			use_trigger = spawn("trigger_radius_use",30 + VectorScale((0,0,1)),pos[i].origin,0,40);
...
reenable_quickrevive(machine_clip,solo_mode)
{
	...
	if(is_true(solo_mode))
	{
		...
		unhide_quickrevive();
		restart_quickrevive();
		level notify("revive_off",_k3918);
		wait(0.1);
...
restart_quickrevive()
{
	...
	foreach(trigger in triggers)
	{
		...
		else if(trigger.script_noteworthy == "specialty_quickrevive" || trigger.script_noteworthy == "specialty_quickrevive_upgrade")
		{
...
disable_quickrevive(machine_clip)
{
	if(is_true(level.solo_revive_init) && flag("solo_revive") && IsDefined(level.quick_revive_machine))
	{
		...
		level.quick_revive_machine moveto(40 + VectorScale((0,0,1)),move_org);
		...
		level.quick_revive_machine waittill(_k4096,_k4108,3,"movedone");
...
unhide_quickrevive()
{
	...
	else
	{
		...
		level.quick_revive_machine vibrate(VectorScale((0,-1,0)),100,0.3,0.4);
	}
