/*******************************************************************
 * Decompiled By: Bog
 * Decompiled File: maps\mp\gametypes\hq.gsc
 * Game: Call of Duty: Black Ops 2
 * Platform: PC
 * Function Count: 44
 * Decompile Time: 421 ms
 * Timestamp: 10/27/2023 3:00:25 AM
*******************************************************************/
...
hqmainloop()
{
	...
	while(1)
	{
		...
		level waittill(_k300,_k345,_k360,"hq_captured");
		...
		while(1)
		{
			...
			level waittill(_k395,_k409,_k427,"hq_destroyed",destroy_team);
...
onradiocapture(player)
{
	...
	level notify("hq_captured",_k550);
	player notify("event_ended");
}
...
onradiodestroy(firstplayer)
{
	...
	level notify("hq_destroyed",destroyed_team,_k651,_k624);
	if(level.kothmode)
	{
...
pickradiotospawn()
{
	...
	for(i = 0;i < level.radios.size;i++)
	{
		...
		else if(IsDefined(level.prevradio2) && radio == level.prevradio2)
		{
