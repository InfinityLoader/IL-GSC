...
getbestspawnpoint(remotemissilespawnpoints)
{
	...
	foreach(spawnpoint in remotemissilespawnpoints)
	{
		foreach(player in spawnpoint.validplayers)
		{
			...
			if(bullettracepassed(32 + VectorScale((0,0,1)),player.origin,spawnpoint.origin,0))
			{
...
_fire(lifeid,player,team,killstreak_id)
{
	...
	player clearusingremote();
	player enableweaponcycling();
	player waittill(_k180,"Remotemissle_killstreak_done");
	return 1;
}
...
getvalidtargets(rocket,trace)
{
	...
	foreach(player in enemies)
	{
		if(distance2dsquared(player.origin,aimtarget) < 360000 && !player hasperk("specialty_nokillstreakreticle"))
		{
			if(trace)
			{
				if(bullettracepassed(60 + VectorScale((0,0,1)),player.origin,180 + VectorScale((0,0,1)),player.origin))
				{
	...
	foreach(dog in dogs)
	{
		if(dog.aiteam != self.team && distance2dsquared(dog.origin,aimtarget) < 360000)
		{
			if(trace)
			{
				if(bullettracepassed(60 + VectorScale((0,0,1)),dog.origin,180 + VectorScale((0,0,1)),dog.origin))
				{
	...
	foreach(tank in tanks)
	{
		if(tank.aiteam != self.team && distance2dsquared(tank.origin,aimtarget) < 360000)
		{
			if(trace)
			{
				if(bullettracepassed(60 + VectorScale((0,0,1)),tank.origin,180 + VectorScale((0,0,1)),tank.origin))
				{
	...
	foreach(turret in turrets)
	{
		if(turret.team != self.team && distance2dsquared(turret.origin,aimtarget) < 360000)
		{
			if(trace)
			{
				if(bullettracepassed(60 + VectorScale((0,0,1)),turret.origin,180 + VectorScale((0,0,1)),turret.origin))
				{
	...
	while(1)
	{
		if(self attackbuttonpressed())
		{
			...
			if(rocket.origin[2] - self.origin[2] > 4000)
			{
				...
				rocket notify("stop_impact_sound",_k1017);
			}
...
fire_bomblet(rocket,explosionradius,target,waitframes)
{
	...
	targetorigin = 50 + VectorScale((0,0,1));
	...
	bomblet = magicbullet("remote_missile_bomblet_mp",origin,targetorigin,self,target,VectorScale((0,0,1)));
	bomblet.team = self.team;
	bomblet setteam(self.team);
	bomblet.killcament = self;
	bomblet thread setup_bomblet_map_icon();
