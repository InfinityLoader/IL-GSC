/*******************************************************************
 * Decompiled By: Bog
 * Decompiled File: maps\mp\killstreaks\_spyplane.gsc
...
init()
{
	...
	{
		uavorigin = 1200 + VectorScale((0,1,0));
	}
	...
	level.uavrig = spawn("script_model",1100 + VectorScale((0,0,1)));
	level.uavrig setmodel("tag_origin");
	level.uavrig.angles = VectorScale((0,1,0));
	level.uavrig hide();
	level.uavrig thread rotateuavrig(1);
	level.uavrig thread swayuavrig();
	level.counteruavrig = spawn("script_model",1500 + VectorScale((0,0,1)));
	level.counteruavrig setmodel("tag_origin");
	level.counteruavrig.angles = VectorScale((0,1,0));
...
addactivecounteruav()
{
	if(level.teambased)
	{
		...
		foreach(team in level.teams)
		{
			...
			else if(level.activesatellites[team] > 0)
			{
...
#/
		...
		for(i = 0;i < keys.size;i++)
		{
			...
			else if(level.activecounteruavs[keys[i]])
			{
				self.owner maps/mp/_challenges::blockedsatellite();
				break;
			}
	...
	for(;;)
	{
		...
		switch(type)
		{
			...
			case "MOD_RIFLE_BULLET":
				...
				break;
		attacker hasperk("specialty_armorpiercing")
				break;
