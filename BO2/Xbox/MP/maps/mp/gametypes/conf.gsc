 ...
 * Timestamp: 10/27/2023 3:04:30 AM
*******************************************************************/
...
spawndogtags(victim,attacker)
{
	...
	else
	{
		...
		level.dogtags[victim.entnum] = maps/mp/gametypes/_gameobjects::createuseobject("any",trigger,visuals,VectorScale((0,0,1)));
		foreach(team in level.teams)
		{
	...
	level notify("dogtag_spawned",victim.origin,_k189,16);
}
...
showtoteam(gameobject,team)
{
	...
	for(;;)
	{
		level waittill(_k245,"joined_team");
		self hide();
		foreach(player in level.players)
		{
			if(player.team == team)
...
showtoenemyteams(gameobject,friend_team)
{
	...
	for(;;)
	{
		level waittill(_k274,"joined_team");
		self hide();
		foreach(player in level.players)
		{
			if(player.team != friend_team)
...
resettags()
{
	...
	self.visuals[0] hide();
	self.visuals[1] hide();
	self.curorigin = VectorScale((0,0,1));
	self.trigger.origin = VectorScale((0,0,1));
	self.visuals[0].origin = VectorScale((0,0,1));
	self.visuals[1].origin = VectorScale((0,0,1));
	self.tacinsert = 0;
