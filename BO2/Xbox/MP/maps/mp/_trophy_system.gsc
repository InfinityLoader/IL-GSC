/*******************************************************************
 * Decompiled By: Bog
 * Decompiled File: maps\mp\_trophy_system.gsc
 * Game: Call of Duty: Black Ops 2
 * Platform: Console
 * Function Count: 12
 * Decompile Time: 71 ms
 * Timestamp: 10/27/2023 3:05:14 AM
*******************************************************************/
					...
					if(IsDefined(grenade.owner))
					{
						if(level.teambased)
						{
							...
							else
							{
								...
								{
									if(bullettracepassed(grenade.origin,29 + VectorScale((0,0,1)),self.origin,0))
									{
										playfx(level.trophylongflashfx,15 + VectorScale((0,0,1)),self.origin,grenade.origin - self.origin);
			...
			else if(IsDefined(tac_insert.owner))
			{
				if(level.teambased)
				{
					...
					else
					{
						...
						if(grenadedistancesquared < 262144)
						{
							if(bullettracepassed(tac_insert.origin,29 + VectorScale((0,0,1)),self.origin,0))
							{
								playfx(level.trophylongflashfx,15 + VectorScale((0,0,1)),self.origin,tac_insert.origin - self.origin);
