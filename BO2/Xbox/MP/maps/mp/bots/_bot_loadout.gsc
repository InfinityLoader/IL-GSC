/*******************************************************************
 * Decompiled By: Bog
 * Decompiled File: maps\mp\bots\_bot_loadout.gsc
 * Game: Call of Duty: Black Ops 2
 * Platform: Console
 * Function Count: 15
 * Decompile Time: 151 ms
 * Timestamp: 10/27/2023 3:04:15 AM
*******************************************************************/
...
bot_construct_class(class,items,allocation_max)
{
	...
	while(allocation < allocation_max)
	{
		...
		if(remaining >= 1 && bot_make_choice(95,claimed_count["primary"],1))
		{
			...
			allocation = allocation + bot_choose_primary_attachments(class,weapon,allocation,allocation_max);
		}
		...
		if(remaining >= 1 && bot_make_choice(secondary_chance,claimed_count["secondary"],1))
		{
			...
			allocation = allocation + bot_choose_secondary_attachments(class,weapon,allocation,allocation_max);
		}
...
bot_build_weapon_options_list(optiontype)
{
	...
	for(row = 0;row < 255;row++)
	{
		if(tablelookupcolumnforrow("mp/attachmentTable.csv",row,1) == optiontype)
		{
			...
			prob = prob + int(tablelookupcolumnforrow("mp/attachmentTable.csv",row,15));
