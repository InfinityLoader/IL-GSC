 ...
 * Platform: PC
 * Function Count: 52
 * Decompile Time: 758 ms
 * Timestamp: 10/27/2023 2:12:52 AM
*******************************************************************/
...
init()
{
	...
	var_00 = 0;
	var_01 = 0;
	for(var_00 = 0;var_00 <= level.maxprestige;var_00++)
	{
...
onplayerconnect()
{
	for(;;)
	{
		...
		var_00.pers["rankxp"] = var_00 maps\mp\gametypes\_persistance::statget("experience");
...
setgamesplayed()
{
	...
	for(;;)
	{
		...
		maps\mp\gametypes\_persistance::statadd("gamesPlayed",1);
		break;
	}
}
...
//Function Number: 22
giverankxp(param_00,param_01,param_02,param_03,param_04)
{
	self endon("disconnect");
	var_05 = "none";
	...
	switch(param_00)
	{
		...
		case "save":
			...
			if(isdefined(level.nukedetonated) && level.nukedetonated)
			{
				...
				{
					param_01 = param_01 * level.nukeinfo.var_36;
				}
	...
	incrankxp(param_01);
	if(maps\mp\_utility::rankingenabled() && updaterank(var_0F))
	{
	...
	if(weaponshouldgetxp(param_02,param_03) || var_10)
	{
		...
		if(self isitemunlocked(var_12))
		{
			if(self.primaryweapon == param_02 || self.secondaryweapon == param_02 || weaponaltweaponname(self.primaryweapon) == param_02 || isdefined(self.tookweaponfrom) && isdefined(self.tookweaponfrom[param_02]))
			{
				...
				{
					var_18 = getweaponmaxrankxp(var_12);
					if(var_17 > var_18)
					{
						var_17 = var_18;
					...
					if(maps\mp\_utility::rankingenabled() && updateweaponrank(var_17,var_12))
					{
...
//Function Number: 23
weaponshouldgetxp(param_00,param_01)
{
	if(self isitemunlocked("cac") && !maps\mp\_utility::isjuggernaut() && isdefined(param_00) && isdefined(param_01) && !maps\mp\_utility::iskillstreakweapon(param_00))
	{
...
//Function Number: 25
updateweaponrank(param_00,param_01)
{
	var_02 = getweaponrank(param_01);
	if(var_02 == self.pers["weaponRank"])
	{
		return 0;
	}

	var_03 = self.pers["weaponRank"];
	self.pers["weaponRank"] = var_02;
	self setplayerdata("weaponRank",param_01,var_02);
	thread maps\mp\gametypes\_missions::masterychallengeprocess(param_01);
	return 1;
}
...
//Function Number: 34
func_2F45(param_00,param_01,param_02)
{
...
//Function Number: 38
levelforexperience(param_00)
{
	return getrankforxp(param_00);
}

//Function Number: 39
weaponlevelforexperience(param_00)
{
...
getprestigelevel()
{
	return maps\mp\gametypes\_persistance::statget("prestige");
}
...
syncxpstat()
{
	...
	var_00 = getrankxp();
	maps\mp\gametypes\_persistance::statset("experience",var_00);
}