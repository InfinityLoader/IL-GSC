 ...
 * Timestamp: 10/27/2023 2:14:16 AM
*******************************************************************/
...
giveims(param_00)
{
	...
	removeperks();
	var_02 = setcarryingims(var_01,1);
	thread restoreperks();
	return var_02;
}

//Function Number: 4
setcarryingims(param_00,param_01)
{
	self endon("death");
	...
	for(;;)
	{
		...
		if(var_02 == "cancel_ims" || var_02 == "force_cancel_placement")
		{
			...
			var_03 = maps\mp\killstreaks\_killstreaks::getkillstreakweapon(level.imssettings[param_00.imstype].streakname);
...
createims(param_00)
{
	...
	var_03 thread ims_handledamage();
	var_03 thread ims_timeout();
...
ims_handleuse()
{
	...
	for(;;)
	{
		...
		ims_setinactive();
		ims_hideallparts();
		var_00 setcarryingims(var_01,0);
	}
}
...
ims_setcarried(param_00)
{
	...
	param_00 thread updateimsplacement(self);
	...
	if(isdefined(self.ims))
	{
		...
		if(isdefined(self.ims.var_13))
		{
			self.ims.var_13 hide();
		}
	}
}

//Function Number: 20
updateimsplacement(param_00)
{
	self endon("death");
	self endon("disconnect");
	level endon("game_ended");
...
ims_setactive()
{
	...
	foreach(var_05 in level.players)
	{
		thread ims_playerjoinedteam(var_05);
	}
}
...
//Function Number: 31
fire_sensor(param_00,param_01,param_02)
{
	...
	if(isdefined(param_01.killcament))
	{
		...
		param_01.killcament thread deleteaftertime(5);
	}
	...
	var_03 = [];
	for(var_04 = 0;var_04 < var_7B.size;var_04++ = var_7B[0])
	{
		if(isdefined(self.owner))
		{
			magicbullet("ims_projectile_mp",param_02.origin,var_7B[var_04],self.owner);
			continue;
		}

		magicbullet("ims_projectile_mp",param_02.origin,var_7B[var_04]);
	}
