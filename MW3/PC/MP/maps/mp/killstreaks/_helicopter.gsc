 ...
 * Timestamp: 10/27/2023 2:14:09 AM
*******************************************************************/
...
//Function Number: 7
usehelicopterblackbox(param_00)
{
...
tryusehelicopter(param_00,param_01)
{
	...
	if((!isdefined(param_01) || param_01 == "flares") && isdefined(level.chopper))
	{
		...
		var_04 hide();
		var_04 thread deleteonentnotify(self,"disconnect");
		var_04.player = self;
		var_04.lifeid = param_00;
		var_04.helitype = param_01;
		var_04.streakname = var_03;
		maps\mp\_utility::queueadd("helicopter",var_04);
		var_05 = undefined;
		if(!self hasweapon(common_scripts\utility::getlastweapon()))
		{
			var_05 = maps\mp\killstreaks\_killstreaks::getfirstprimaryweapon();
		}
		else
		{
		...
		var_06 = maps\mp\killstreaks\_killstreaks::getkillstreakweapon("helicopter_flares");
	...
	else if(maps\mp\_utility::currentactivevehiclecount() >= maps\mp\_utility::maxvehiclesallowed() || level.fauxvehiclecount + var_06 >= maps\mp\_utility::maxvehiclesallowed())
	{
...
//Function Number: 13
starthelicopter(param_00,param_01)
{
...
heliride(param_00,param_01)
{
	...
	thread endrideonhelicopterdone(param_01);
	thread weaponlockthink(param_01);
	for(;;)
	{
...
endride(param_00)
{
	...
	{
		var_01 = maps\mp\killstreaks\_killstreaks::getkillstreakweapon("helicopter_minigun");
...
//Function Number: 21
endrideonhelicopterdone(param_00)
{
	self endon("disconnect");
...
heli_think(param_00,param_01,param_02,param_03,param_04)
{
	...
	var_09 thread heli_damage_monitor();
	var_09 thread heli_health();
	var_09 thread heli_existance();
	...
	switch(param_04)
	{
		case "minigun":
			...
			var_09 thread heli_leave_on_timeout(40);
			if(var_0A.size)
			{
				var_09 thread heli_fly_well(var_0A);
			}
			else
			{
...
heli_existance()
{
	...
	{
		var_01 maps\mp\killstreaks\_killstreaks::func_30BE(var_04,1);
		var_01 starthelicopter(var_03,var_05);
		return;
	}
		...
		if(self.damagetaken >= self.maxhealth)
		{
			if((level.teambased && self.team != var_01.team) || !level.teambased)
			{
				...
				if(isdefined(var_0B))
				{
					...
					{
						case "flares":
							var_0B thread maps\mp\gametypes\_rank::func_2F45(&"SPLASHES_DESTROYED_PAVELOW");
							thread maps\mp\_utility::teamplayercardsplash("callout_destroyed_helicopter_flares",var_0B);
							var_0C = 400;
							break;
	
						case "minigun":
							var_0C thread maps\mp\gametypes\_rank::func_2F45(&"SPLASHES_DESTROYED_MINIGUNNER");
						...
						case "osprey_gunner":
						case "osprey":
							var_0C thread maps\mp\gametypes\_rank::func_2F45(&"SPLASHES_DESTROYED_OSPREY");
							thread maps\mp\_utility::teamplayercardsplash("callout_destroyed_osprey",var_0C);
							var_0C = 300;
							break;
	
						case "littlebird":
							var_0C thread maps\mp\gametypes\_rank::func_2F45(&"SPLASHES_DESTROYED_LITTLE_BIRD");
							thread maps\mp\_utility::teamplayercardsplash("callout_destroyed_little_bird",var_0C);
							var_0C = 200;
							break;
	
						default:
							var_0B thread maps\mp\gametypes\_rank::func_2F45(&"SPLASHES_DESTROYED_HELICOPTER");
							thread maps\mp\_utility::teamplayercardsplash("callout_destroyed_helicopter",var_0B);
							var_0C = 200;
							break;
					}

					var_0B thread maps\mp\gametypes\_rank::giverankxp("kill",var_0C,var_09,var_04);
...
heli_health()
{
	...
	for(;;)
	{
		...
		else if(self.damagetaken > self.maxhealth)
		{
			...
			if(isdefined(self.var_30C8) && self.var_30C8)
			{
...
fire_missile(param_00,param_01,param_02)
{
	...
	for(var_08 = 0;var_08 < param_02;var_08++)
	{
		...
		if(isdefined(var_03))
		{
			var_09 = self fireweapon(var_7B[var_07],var_03);
			var_09 missile_setflightmodedirect();
			var_09 missile_settargetent(var_03);
		}
		else
		{
			var_09 = self fireweapon(var_7B[var_07]);
			var_09 missile_setflightmodedirect();
			var_09 missile_settargetent(var_03);
		}
...
attack_secondary()
{
	...
	for(;;)
	{
		if(isdefined(self.secondarytarget))
		{
			...
			self.var_30D9 = self.secondarytarget;
			var_00 = 0;
			while(isdefined(self.var_30D9) && isalive(self.var_30D9))
			{
				if(missile_target_sight_check(self.var_30D9))
				{
					thread missile_support(self.var_30D9,level.heli_missile_rof);
				}
				else
				{
					break;
				...
				if(!isdefined(self.secondarytarget) || isdefined(self.secondarytarget) && self.var_30D9 != self.secondarytarget)
				{
					break;
				}
			}

			if(isdefined(self.var_30D9))
			{
				self.var_30D9.antithreat = undefined;
			}
		}
...
deployflares()
{
	...
	var_00 thread deleteaftertime(5);
	return var_00;
}
...
handleincomingstinger(param_00)
{
	...
	for(;;)
	{
		...
		var_02 thread watchstingerproximity(var_01,var_01.team,var_03);
	}
}

//Function Number: 83
watchstingerproximity(param_00,param_01,param_02)
{
	self endon("death");
	for(;;)
	{
