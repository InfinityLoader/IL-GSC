 ...
 * Timestamp: 10/27/2023 2:14:30 AM
*******************************************************************/
...
tryuseremotetank(param_00)
{
	...
	var_02 = givetank(param_00,"remote_tank");
	if(var_02)
	{
...
{
	var_01 = maps\mp\killstreaks\_killstreaks::getkillstreakweapon(level.tanksettings[param_00].streakname);
...
{
	var_02 = createtankforplayer(param_01,self);
...
//Function Number: 11
createtankforplayer(param_00,param_01)
{
...
setcarryingtank(param_00,param_01)
{
	...
	for(;;)
	{
		...
		if(var_02 == "cancel_tank" || var_02 == "force_cancel_placement")
		{
			...
			var_03 = maps\mp\killstreaks\_killstreaks::getkillstreakweapon(level.tanksettings[param_00.tanktype].streakname);
		...
		param_01 thread tank_setplaced();
		common_scripts\utility::_enableweapon();
		return 1;
	}
}
...
tank_setcarried(param_00)
{
	...
	param_00 thread updatetankplacement(self);
...
createtank(param_00)
{
	...
	var_04 thread tank_giveweapononplaced();
	return var_04;
}
...
tank_setactive()
{
	...
	if(level.teambased)
	{
		...
		foreach(var_03 in level.players)
		{
			...
			{
				var_04 = self.mgturret maps\mp\_entityheadicons::setheadicon(var_03,maps\mp\gametypes\_teams::init(self.team),var_01,10,10,0,0.05,0,1,0,1);
				if(isdefined(var_04))
				{
...
callback_vehicledamage(param_00,param_01,param_02,param_03,param_04,param_05,param_06,param_07,param_08,param_09,param_0A,param_0B)
{
	...
	if(!maps\mp\gametypes\_weapons::friendlyfirecheck(var_0C.owner,param_01))
	{
		return;
	}
	...
	if(isexplosivedamagemod(param_04) && param_05 == "destructible_car")
	{
	...
	if(var_0C.damagetaken >= var_0C.maxhealth)
	{
		if(isplayer(param_01) && !isdefined(var_0C.owner) || param_01 != var_0C.owner)
		{
			...
			param_01 thread maps\mp\gametypes\_rank::func_2F45(&"SPLASHES_DESTROYED_REMOTE_TANK");
