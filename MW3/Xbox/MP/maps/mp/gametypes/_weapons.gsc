 ...
 * Timestamp: 10/27/2023 2:26:00 AM
*******************************************************************/
...
bombsquadvisibilityupdater(param_00,param_01)
{
	...
	foreach(var_03 in level.players)
	{
		if(level.teambased)
		{
			if(var_03.team == param_00 && var_03 maps\mp\_utility::func_27AF("specialty_detectexplosive"))
			{
		...
		if(!var_03 maps\mp\_utility::func_27AF("specialty_detectexplosive"))
		{
			continue;
		}
...
onplayerconnect()
{
	for(;;)
	{
		...
		var_00 thread func_2858();
		var_00 thread func_2821();
	}
}
...
onplayerspawned()
{
	...
	for(;;)
	{
		...
		thread maps\mp\gametypes\_class::func_281D();
		thread maps\mp\gametypes\_equipment::func_2788();
		thread func_2888();
		self.lasthittime = [];
...
watchweaponchange()
{
	...
	if(maps\mp\_utility::func_2827(var_00) && !isdefined(self.hitsthismag[var_00]))
	{
	...
	for(;;)
	{
		...
		if(maps\mp\_utility::func_2829(var_01))
		{
			if(maps\mp\_utility::isjuggernaut() && !maps\mp\killstreaks\_airdrop::func_2E9F(var_01))
			{
		...
		{
			if(maps\mp\_utility::func_2827(var_01) && !isdefined(self.hitsthismag[var_01 + "_mp"]))
			{
		...
		{
			if(maps\mp\_utility::func_2827(var_01) && !isdefined(self.hitsthismag[var_01]))
			{
		...
		else if(var_01 != "none" && func_2834(var_01 + "_mp"))
		{
		...
		if(isdefined(self.class_num))
		{
			...
			{
				if(var_01 == self.primaryweapon && !maps\mp\_utility::func_27AF(self.loadoutprimarybuff))
				{
				...
				if(var_01 != self.primaryweapon && maps\mp\_utility::func_27AF(self.loadoutprimarybuff))
				{
					maps\mp\_utility::func_282E(self.loadoutprimarybuff);
				}
			}

			if(isdefined(self.loadoutsecondarybuff) && self.loadoutsecondarybuff != "specialty_null")
			{
				if(var_01 == self.secondaryweapon && !maps\mp\_utility::func_27AF(self.loadoutsecondarybuff))
				{
				...
				if(var_01 != self.secondaryweapon && maps\mp\_utility::func_27AF(self.loadoutsecondarybuff))
				{
					if(var_01 == self.primaryweapon)
					{
						...
						{
							maps\mp\_utility::func_282E(self.loadoutsecondarybuff);
						}
...
func_283D(param_00)
{
	...
	{
		if(!maps\mp\_utility::func_2827(var_09) && !level.scavenger_secondary)
		{
			continue;
		}
...
func_2844(param_00)
{
	...
	for(;;)
	{
		...
		if(!maps\mp\_utility::func_2827(var_01) && !maps\mp\_utility::iscacsecondaryweapon(var_01))
		{
			continue;
		}
		...
		{
			thread func_2849(var_01);
		}

		var_02 = maps\mp\gametypes\_persistence::func_2845("totalShots") + 1;
		var_03 = maps\mp\gametypes\_persistence::func_2845("hits");
		var_04 = clamp(float(var_03) / float(var_02),0,1) * 10000;
		maps\mp\gametypes\_persistence::func_2846("totalShots",var_02);
		maps\mp\gametypes\_persistence::func_2846("accuracy",int(var_04));
		maps\mp\gametypes\_persistence::func_2846("misses",int(var_02 - var_03));
...
{
	if(maps\mp\_utility::func_323E(param_00,"alt_"))
	{
		...
		foreach(var_04 in var_02)
		{
			if(var_04 == "shotgun")
			{
				...
				if(!isprimaryweapon(var_05) && !func_287D(var_05))
				{
					self.hits = 1;
				}
	...
	if(!isprimaryweapon(param_00) && !func_287D(param_00))
	{
		return;
	}
	...
	{
		thread maps\mp\gametypes\_gamelogic::func_2F59("riotshield",self.hits,"hits");
		self.hits = 0;
	}
	...
	var_08 = maps\mp\gametypes\_persistence::func_2845("totalShots");
	var_09 = maps\mp\gametypes\_persistence::func_2845("hits") + 1;
	if(var_09 <= var_08)
	{
		maps\mp\gametypes\_persistence::func_2846("hits",var_09);
		maps\mp\gametypes\_persistence::func_2846("misses",int(var_08 - var_09));
		var_0A = clamp(float(var_09) / float(var_08),0,1) * 10000;
		maps\mp\gametypes\_persistence::func_2846("accuracy",int(var_0A));
	}
}

//Function Number: 37
func_284D(param_00,param_01)
{
	return func_27A1(param_01,param_00);
}

//Function Number: 38
func_27A1(param_00,param_01,param_02)
{
	if(!isdefined(param_00))
	{
		return 1;
...
watchsmokeexplode()
{
	...
	var_02 = 128;
	var_03 = 8;
	level thread func_2855(var_03,var_02,var_01);
	for(;;)
	{
		if(!isdefined(var_00))
		{
...
//Function Number: 43
func_2855(param_00,param_01,param_02)
{
...
func_285B()
{
	thread maps\mp\gametypes\_shellshock::func_2374();
	self endon("end_explode");
	self waittill("explode",var_00);
	var_01 = func_2877(var_00,512,0);
	foreach(var_03 in var_01)
	{
		if(isdefined(var_03.owner) && !func_27A1(self.owner,var_03.owner))
		{
			continue;
		}
...
watchc4()
{
	...
	for(;;)
	{
		...
		if(var_01 == "c4" || var_01 == "c4_mp")
		{
			...
			var_00 thread func_285F();
			var_00 thread func_2790();
...
watchclaymores()
{
	...
	for(;;)
	{
		...
		if(var_01 == "claymore" || var_01 == "claymore_mp")
		{
			...
			var_00 thread equipmentwatchuse(self);
			var_00 thread func_2860(self.pers["team"]);
			self.changingweapon = undefined;
		}
	}
}
...
claymoredetonation()
{
	...
	for(;;)
	{
		...
		if(getdvarint("scr_claymoredebug") != 1)
		{
			...
			if(!func_27A1(self.owner,var_01,0))
			{
				continue;
			}
		}
	...
	if(isplayer(var_01) && var_01 maps\mp\_utility::func_27AF("specialty_delaymine"))
	{
...
func_17B8()
{
	...
	for(;;)
	{
		...
		if(!func_27A1(self.owner,var_00))
		{
			continue;
		}
...
func_17BE(param_00,param_01,param_02,param_03)
{
	...
	for(var_07 = 0;var_07 < var_06.size;var_07++)
	{
		...
		{
			var_04[var_04.size] = maps\mp\_utility::func_2872(var_06[var_07],var_08);
		}
	}
...
//Function Number: 75
func_2877(param_00,param_01,param_02,param_03)
{
	var_04 = [];
	if(!isdefined(param_02))
	{
...
func_288E()
{
	...
	for(;;)
	{
		...
		thread maps\mp\gametypes\_hud_message::func_2892("stuck_semtex",100);
...
//Function Number: 98
func_2895(param_00,param_01,param_02,param_03)
{
	if(!isdefined(param_03))
	{
	...
	var_05 thread func_2860(param_01.pers["team"]);
	var_05 thread minedamagemonitor();
	var_05 thread func_289B();
	return var_05;
}
...
minedamagedebug(param_00,param_01,param_02,param_03,param_04,param_05)
{
	...
	thread debugcircle(var_08,level.minedamageradius,var_41[var_07],32);
	...
	thread debugline(var_08,var_09,var_41[var_07]);
}

//Function Number: 107
func_28A2(param_00,param_01)
{
	if(isplayer(param_01) && isalive(param_01) && param_01.sessionstate == "playing")
	{
		var_02 = param_01 maps\mp\_utility::func_28A3();
	}
...
minethrown(param_00)
{
	...
	var_05 = func_2895(var_02,param_00,"equipment",var_04);
...
minebeacon()
{
	...
	thread minebeaconteamupdater(var_41);
	self waittill(spawnfx(level.mine_beacon["enemy"],self gettagorigin("tag_fx")),var_41["enemy"],"death");
	var_41["friendly"] delete();
	var_41["enemy"] delete();
}
