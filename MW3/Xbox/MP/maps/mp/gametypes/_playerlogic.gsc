...
teamkilldelay()
{
	...
	return maps\mp\gametypes\_tweakables::func_270D("team","teamkillspawndelay") * var_01;
}
...
spawnclient()
{
	if(isdefined(self.var_2AF3))
	{
		maps\mp\gametypes\_menus::func_2AF3(self.var_2AF3);
		self.var_2AF3 = undefined;
	}
	...
	if(!mayspawn())
	{
		...
		else if(maps\mp\_utility::func_26EC() && !maps\mp\_utility::func_2B08())
		{
		...
		thread func_2AE9(var_00,var_01);
		return;
	}
...
func_2B0A()
{
	...
	if(isdefined(self.pers["teamKillPunish"]) && self.pers["teamKillPunish"])
	{
		...
		if(var_01 > 0)
		{
			...
			thread func_2B40(self.origin + (0,0,60),self.angles);
			var_00 = 1;
			wait var_01;
	...
	if(var_02 > 0)
	{
		...
		var_00 = 1;
		maps\mp\_utility::func_2B0D(var_02,"force_spawn");
		self notify("stop_wait_safe_spawn_button");
	}

	var_03 = getdvarint("scr_" + level.gametype + "_waverespawndelay") > 0;
	if(maps\mp\gametypes\_tweakables::func_270D("player","forcerespawn") == 0 && self.hasspawned && !var_03 && !self.wantsafespawn)
	{
...
func_2B11()
{
	...
	if(maps\mp\_utility::func_27AF("specialty_finalstand") && !level.diehardmode)
	{
		maps\mp\_utility::func_282E("specialty_finalstand");
	}
...
showspawnnotifies()
{
	...
	{
		thread maps\mp\gametypes\_hud_message::func_2892("rested");
	}
}
...
func_2B25(param_00)
{
	...
	if(isdefined(self.setspawnpoint) && isdefined(self.setspawnpoint.notti) || func_2B24())
	{
	...
	maps\mp\gametypes\_class::func_2AEE(self.class);
	maps\mp\gametypes\_class::func_2A52(self.team,self.class);
	...
	{
		self setclientdvar("scr_objectiveText",maps\mp\_utility::func_2B3B(self.pers["team"]));
...
func_2B4B()
{
	...
	thread maps\mp\_events::func_2AC3();
	if(level.gameended)
	{
		maps\mp\gametypes\_gamescore::func_26F3();
	}
...
callback_playerconnect()
{
	...
	if(game["clientid"] <= 30 && game["clientid"] != getmatchdata("playerCount"))
	{
		...
		var_04 = var_04 + int(maps\mp\_utility::func_2D00());
	...
	self.kill_streak = maps\mp\gametypes\_persistence::func_2B58("killStreak");
	...
	if(game["state"] == "postgame")
	{
		...
		{
			maps\mp\gametypes\_menus::func_2AF3(maps\mp\gametypes\_menus::getteamassignment(),1);
		}
		else
		{
	...
	{
		thread maps\mp\gametypes\_hostmigration::func_273D();
	}
	...
	if(!isdefined(self.pers["team"]))
	{
		...
		if(maps\mp\_utility::allowteamchoice())
		{
			...
			maps\mp\gametypes\_menus::func_2AE3();
			return;
		}
	...
	maps\mp\gametypes\_menus::func_2AF3(self.pers["team"],1);
...
getrestxpcap(param_00)
{
	...
	return func_2B61(param_00,var_01);
}
...
func_2B63()
{
	...
	if(var_01 > var_03)
	{
		...
		var_06 = func_2B61(var_06,var_08);
	}
...
initplayerstats()
{
	...
	if(!isdefined(self.pers["deaths"]))
	{
		...
		maps\mp\gametypes\_persistence::func_26D8("round","deaths",0);
	}
	...
	if(!isdefined(self.pers["score"]))
	{
		...
		maps\mp\gametypes\_persistence::func_26D8("round","score",0);
	}
	...
	if(!isdefined(self.pers["kills"]))
	{
		...
		maps\mp\gametypes\_persistence::func_26D8("round","kills",0);
	}
	...
	if(!isdefined(self.pers["assists"]))
	{
		...
		maps\mp\gametypes\_persistence::func_26D8("round","assists",0);
	}
	...
	if(!isdefined(self.pers["captures"]))
	{
		...
		maps\mp\gametypes\_persistence::func_26D8("round","captures",0);
	}

	self.var_2B6D = maps\mp\_utility::getpersstat("captures");
	if(!isdefined(self.pers["returns"]))
	{
		maps\mp\_utility::initpersstat("returns");
		maps\mp\gametypes\_persistence::func_26D8("round","returns",0);
	}

	self.var_2B6E = maps\mp\_utility::getpersstat("returns");
	if(!isdefined(self.pers["defends"]))
	{
		maps\mp\_utility::initpersstat("defends");
		maps\mp\gametypes\_persistence::func_26D8("round","defends",0);
	}

	self.var_2B6F = maps\mp\_utility::getpersstat("defends");
	if(!isdefined(self.pers["plants"]))
	{
		maps\mp\_utility::initpersstat("plants");
		maps\mp\gametypes\_persistence::func_26D8("round","plants",0);
	}

	self.var_2B70 = maps\mp\_utility::getpersstat("plants");
	if(!isdefined(self.pers["defuses"]))
	{
		maps\mp\_utility::initpersstat("defuses");
		maps\mp\gametypes\_persistence::func_26D8("round","defuses",0);
	}

	self.var_2B71 = maps\mp\_utility::getpersstat("defuses");
	if(!isdefined(self.pers["destructions"]))
	{
		maps\mp\_utility::initpersstat("destructions");
		maps\mp\gametypes\_persistence::func_26D8("round","destructions",0);
	}

	self.var_2B72 = maps\mp\_utility::getpersstat("destructions");
	if(!isdefined(self.pers["confirmed"]))
	{
		maps\mp\_utility::initpersstat("confirmed");
		maps\mp\gametypes\_persistence::func_26D8("round","confirmed",0);
	}

	self.var_2B73 = maps\mp\_utility::getpersstat("confirmed");
	if(!isdefined(self.pers["denied"]))
	{
		maps\mp\_utility::initpersstat("denied");
		maps\mp\gametypes\_persistence::func_26D8("round","denied",0);
	}

	self.var_2B74 = maps\mp\_utility::getpersstat("denied");
	...
	self.pers["lives"] = maps\mp\_utility::getgametypenumlives();
	maps\mp\gametypes\_persistence::func_26D8("round","killStreak",0);
	maps\mp\gametypes\_persistence::func_26D8("round","loss",0);
	maps\mp\gametypes\_persistence::func_26D8("round","win",0);
	maps\mp\gametypes\_persistence::func_26D8("round","scoreboardType","none");
...
addtoteamcount()
{
	...
	maps\mp\gametypes\_gamelogic::func_2734();
}
...
removefromalivecount(param_00)
{
	...
	level.alivecount[self.team]--;
	return maps\mp\gametypes\_gamelogic::func_2734();
}
