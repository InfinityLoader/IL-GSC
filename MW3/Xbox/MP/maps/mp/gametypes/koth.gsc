 ...
 * Timestamp: 10/27/2023 2:26:17 AM
*******************************************************************/
...
main()
{
	...
	level.onrespawndelay = ::func_34BF;
...
initializematchrules()
{
	...
	maps\mp\_utility::func_330E("koth",1,0,9);
	setdynamicdvar("scr_koth_roundlimit",1);
	maps\mp\_utility::func_3312("koth",1);
...
updateobjectivehintmessages(param_00,param_01)
{
	...
	for(var_02 = 0;var_02 < level.players.size;var_02++)
	{
		...
		{
			var_04 = maps\mp\_utility::func_2B3B(var_03.pers["team"]);
...
func_34BF()
{
	...
	var_00 = level.radioobject maps\mp\gametypes\_gameobjects::func_2A15();
...
onstartgametype()
{
	maps\mp\_utility::func_3305("allies",&"OBJECTIVES_KOTH");
	maps\mp\_utility::func_3305("axis",&"OBJECTIVES_KOTH");
	if(level.splitscreen)
	{
		maps\mp\_utility::func_3306("allies",&"OBJECTIVES_KOTH");
		maps\mp\_utility::func_3306("axis",&"OBJECTIVES_KOTH");
	}
	else
	{
		maps\mp\_utility::func_3306("allies",&"OBJECTIVES_KOTH_SCORE");
		maps\mp\_utility::func_3306("axis",&"OBJECTIVES_KOTH_SCORE");
	}
...
func_34C6()
{
	...
	wait 5;
	var_03 = [];
	var_41["allies"] maps\mp\gametypes\_hud_util::setpoint("TOP LEFT","TOP LEFT",125,2);
	var_41["allies"].label = var_01;
	var_41["allies"].alpha = 0;
	var_41["allies"].archived = 0;
	var_41["allies"].hidewheninmenu = 1;
	var_41["axis"] maps\mp\gametypes\_hud_util::setpoint("TOP LEFT","TOP LEFT",125,2);
	var_41["axis"].label = var_01;
	var_41["axis"].alpha = 0;
	var_41["axis"].archived = 0;
	var_41["axis"].hidewheninmenu = 1;
	level.timerdisplay = var_41;
	thread func_34CC(var_41["allies"]);
	thread func_34CC(var_41["axis"]);
	var_04 = maps\mp\gametypes\_gameobjects::func_2A31();
	objective_add(var_04,"invisible",(0,0,0));
	for(;;)
	{
		var_05 = pickradiotospawn();
		var_05 makeradioactive();
		maps\mp\_utility::func_2D68("mp_suitcase_pickup");
		maps\mp\_utility::func_26E5("hq_located");
		...
		if(level.hqspawntime)
		{
			var_07 = maps\mp\gametypes\_objpoints::func_2996("objpoint_next_hq",var_05.origin + level.iconoffset,"all","waypoint_targetneutral");
			...
			updateobjectivehintmessages(level.objectivehintpreparehq,level.objectivehintpreparehq);
			var_41["allies"].label = var_01;
			var_41["allies"] settimer(level.hqspawntime);
			var_41["allies"].alpha = 1;
			var_41["axis"].label = var_01;
			var_41["axis"] settimer(level.hqspawntime);
			var_41["axis"].alpha = 1;
			wait level.hqspawntime;
			maps\mp\gametypes\_objpoints::deleteobjpoint(var_07);
			objective_state(var_04,"invisible");
			maps\mp\_utility::func_26E5("hq_online");
		}

		var_41["allies"].alpha = 0;
		var_41["axis"].alpha = 0;
		waittillframeend;
		maps\mp\_utility::func_26E5("obj_capture");
		updateobjectivehintmessages(level.objectivehintcapturehq,level.objectivehintcapturehq);
		maps\mp\_utility::func_2D68("mp_killstreak_radar");
		...
		var_06 maps\mp\gametypes\_gameobjects::func_2A25("enemy","waypoint_captureneutral");
		var_06 maps\mp\gametypes\_gameobjects::setvisibleteam("any");
		var_06.onuse = ::onradiocapture;
		var_06.onbeginuse = ::onbeginuse;
		var_06.onenduse = ::onenduse;
		level waittill(maps\mp\gametypes\_hud_util::createservertimer("objective",1.4,"allies"),var_41["allies"],maps\mp\gametypes\_hud_util::createservertimer("objective",1.4,"axis"),var_41["axis"],"hq_captured");
		var_08 = var_06 maps\mp\gametypes\_gameobjects::func_2A15();
		var_09 = maps\mp\_utility::getotherteam(var_08);
		if(level.hqautodestroytime)
		{
			thread destroyhqaftertime(level.hqautodestroytime);
			var_41[var_08] settimer(level.hqautodestroytime + level.extradelay);
			var_41[var_09] settimer(level.hqautodestroytime);
		}
		else
		{
		...
		for(;;)
		{
			...
			var_06 maps\mp\gametypes\_gameobjects::func_2A25("friendly","waypoint_defend");
			var_06 maps\mp\gametypes\_gameobjects::set2dicon("enemy","waypoint_capture");
			var_06 maps\mp\gametypes\_gameobjects::func_2A25("enemy","waypoint_capture");
			if(!level.kothmode)
			{
				var_06 maps\mp\gametypes\_gameobjects::setusetext(&"MP_DESTROYING_HQ");
			}

			var_06.onuse = ::func_34CF;
			if(level.hqautodestroytime)
			{
				var_41[var_08].label = var_02;
				var_41[var_08].alpha = 1;
				var_41[var_09].label = var_03;
				var_41[var_09].alpha = 1;
			}

			level waittill("hq_destroyed");
			var_41[var_09].alpha = 0;
			if(!level.kothmode || level.hqdestroyedbytimer)
			{
				break;
			}

			thread func_34CD(var_08);
		...
		var_05 makeradioinactive();
		level.radioobject = undefined;
		thread func_34CD(var_08,level.extradelay);
...
onradiocapture(param_00)
{
	...
	foreach(var_03 in self.touchlist[var_01])
	{
		...
		var_04 maps\mp\_utility::func_26FB("captures",1);
		var_04 maps\mp\gametypes\_persistence::func_26D8("round","captures",param_00.pers["captures"]);
	}

	param_00 thread maps\mp\_matchdata::loggameevent("capture",param_00.origin);
	var_06 = maps\mp\gametypes\_gameobjects::func_2A15();
	...
	maps\mp\_utility::func_26E5("hq_secured",var_01);
	maps\mp\_utility::func_26E5("hq_enemy_captured",var_07);
	thread maps\mp\_utility::func_2D68("mp_war_objective_taken",var_01);
	thread maps\mp\_utility::func_2D68("mp_war_objective_lost",var_07);
	level thread func_34D1(var_01);
...
func_34CF(param_00)
{
	...
	foreach(var_04 in self.touchlist[var_01])
	{
		...
		var_05 maps\mp\_utility::func_26FB("destructions",1);
		var_05 maps\mp\gametypes\_persistence::func_26D8("round","destructions",param_00.pers["destructions"]);
	}
	...
	if(level.kothmode)
	{
		maps\mp\_utility::teamplayercardsplash("callout_capturedhq",param_00);
		maps\mp\_utility::func_26E5("hq_secured",var_01);
		maps\mp\_utility::func_26E5("hq_enemy_captured",var_02);
	}
	else
	{
		maps\mp\_utility::teamplayercardsplash("callout_destroyedhq",param_00);
		maps\mp\_utility::func_26E5("hq_secured",var_01);
		maps\mp\_utility::func_26E5("hq_enemy_destroyed",var_02);
	}

	thread maps\mp\_utility::func_2D68("mp_war_objective_taken",var_01);
	thread maps\mp\_utility::func_2D68("mp_war_objective_lost",var_02);
...
destroyhqaftertime(param_00)
{
	...
	maps\mp\_utility::func_26E5("hq_offline");
	level notify("hq_destroyed");
}
...
func_34D1(param_00)
{
	...
	while(!level.gameended)
	{
		...
		{
			maps\mp\gametypes\_gamescore::giveteamscoreforobjective(param_00,int(var_04 * var_06 + 1));
		}
		else
		{
			maps\mp\gametypes\_gamescore::giveteamscoreforobjective(param_00,var_04);
		}
		...
		for(var_07 = 0;var_07 < level.players.size;var_07++)
		{
			...
			if(var_08.pers["team"] == param_00)
			{
				if(level.promode)
				{
					if(level.hqautodestroytime)
					{
						var_08 thread maps\mp\gametypes\_rank::func_2AB3("defend",int(var_02 + var_03 * var_06));
					}
					else
					{
						var_08 thread maps\mp\gametypes\_rank::func_2AB3("defend",int(var_02 + var_03));
					}
				}
				else
				{
					var_08 thread maps\mp\gametypes\_rank::func_2AB3("defend");
				}

				if(isalive(var_08))
				{
					maps\mp\gametypes\_gamescore::func_26D3("defend",var_08);
				}
			}
		}
...
getspawnpoint()
{
	...
	if(isdefined(level.radioobject))
	{
		var_01 = level.radioobject maps\mp\gametypes\_gameobjects::func_2A15();
		if(self.pers["team"] == var_01)
		{
			var_00 = maps\mp\gametypes\_spawnlogic::func_2FE6(level.spawn_all,level.radioobject.nearspawns);
		}
		else
		{
			var_00 = maps\mp\gametypes\_spawnlogic::func_2FE6(level.spawn_all,level.radioobject.outerspawns);
		}
	}

	if(!isdefined(var_00))
	{
		var_00 = maps\mp\gametypes\_spawnlogic::func_2FE6(level.spawn_all);
	}
...
makeradioactive()
{
	self.gameobject = maps\mp\gametypes\_gameobjects::func_29E7("neutral",self.trig,self.visuals,self.origin - self.trigorigin + level.iconoffset);
...
initgametypeawards()
{
	maps\mp\_awards::func_2D85("hqsdestroyed",0,::maps\mp\_awards::func_2DA7);
	maps\mp\_awards::func_2D85("hqscaptured",0,::maps\mp\_awards::func_2DA7);
}