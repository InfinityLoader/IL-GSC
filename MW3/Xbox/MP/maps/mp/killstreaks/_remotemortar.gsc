 ...
 * Timestamp: 10/27/2023 2:27:14 AM
*******************************************************************/
...
init()
{
	...
	level.killstreakfuncs["remote_mortar"] = ::func_3100;
...
spawnremote(param_00,param_01)
{
	...
	var_02 maps\mp\killstreaks\_uav::func_309E();
	var_03 = 6300;
	...
	var_02 thread func_2FBF(param_01);
	var_02 thread func_310D(param_01);
	var_02 thread func_310E(param_01);
	var_02 thread handleownerdisconnect(param_01);
	var_02 thread handleincomingstinger();
	var_02 thread func_2C53();
	return var_02;
}
...
func_3104(param_00)
{
	...
	thread func_3105(param_00);
	thread func_3106(param_00);
	thread func_3108(param_00);
}
...
func_3106(param_00)
{
	...
	for(;;)
	{
		...
		if(self attackbuttonpressed() && var_01 - var_02 > 3000)
		{
			...
			var_08 thread func_310A(param_00);
			var_08 thread func_310B(param_00);
...
{
	self endon("death");
	maps\mp\gametypes\_hostmigration::func_2740(6);
...
func_310C(param_00)
{
	...
	var_01 = maps\mp\killstreaks\_killstreaks::func_2C01("remote_mortar");
...
//Function Number: 19
removeremote(param_00,param_01)
{
	...
	{
		param_00 delete();
		param_00 maps\mp\killstreaks\_uav::func_309F();
	}
...
func_3111()
{
	...
	maps\mp\gametypes\_hostmigration::func_2740(3);
	self moveto(var_00,4,4,0);
	maps\mp\gametypes\_hostmigration::func_2740(4);
	...
	for(;;)
	{
		...
		if(self.damagetaken >= self.maxhealth)
		{
			if(isplayer(var_01) && !isdefined(self.owner) || var_01 != self.owner)
			{
				...
				var_01 thread maps\mp\gametypes\_rank::func_2AB3("kill",50,var_09,var_04);
				var_01 thread maps\mp\gametypes\_rank::xpeventpopup(&"SPLASHES_DESTROYED_REMOTE_MORTAR");
...
stingerproximitydetonate(param_00,param_01)
{
	...
	for(;;)
	{
		...
		{
			param_00.numflares--;
			param_00 thread maps\mp\killstreaks\_helicopter::func_2C4D();
...
func_2C53()
{
	...
	for(;;)
	{
		...
		level thread func_3066(var_02,var_00,var_01);
	}
}
...
func_3066(param_00,param_01,param_02)
{
	...
	for(;;)
	{
		...
		while(var_07 < var_41.size)
		{
			...
			{
				if(var_41[var_07] < 3000 && param_01.numflares > 0)
				{
					param_01.numflares-- = distance(param_02[var_06].origin,var_07);
					param_01 thread maps\mp\killstreaks\_helicopter::func_2C4D();
				...
				if(var_0A[var_41] < var_08[var_41])
				{
					var_08[var_41] = var_0A[var_41];
				}

				if(var_0A[var_41] > var_08[var_41])
				{
					if(var_0A[var_41] > 1536)
					{
						continue;
					}
					...
					if(isdefined(var_03.owner))
					{
						...
						if(level.teambased)
						{
							...
							{
								radiusdamage(var_05[var_41].origin,var_07,var_06,var_06,var_04,"MOD_EXPLOSIVE","sam_projectile_mp");
							}
						}
						else
						{
