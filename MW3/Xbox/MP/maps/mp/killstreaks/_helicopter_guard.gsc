 ...
 * Timestamp: 10/27/2023 2:27:00 AM
*******************************************************************/

//Function Number: 1
init()
{
	level.killstreakfuncs["littlebird_support"] = ::func_30BB;
...
func_30BE(param_00)
{
	...
	var_08 maps\mp\killstreaks\_helicopter::func_2C4A();
	var_08 thread maps\mp\killstreaks\_helicopter::func_2C4B();
	...
	var_08.mgturretleft thread func_30DC();
	var_08.mgturretright thread func_30DC();
	...
	var_08 thread func_30CB();
	var_08 thread func_30CC();
	var_08 thread lbsupport_watchownerdamage();
	var_08 thread func_30CE();
...
func_30C6(param_00)
{
	...
	param_00 waittill("goal");
	param_00 thread func_30C7();
	param_00 thread maps\mp\killstreaks\_helicopter::func_2C53(::func_30D2);
...
func_30C7()
{
	...
	for(;;)
	{
		...
		{
			var_00 = func_30D6(self.owner.origin);
...
lbsupport_watchownerdamage()
{
	...
	for(;;)
	{
		...
		if(isplayer(var_01))
		{
			if(var_01 != self.owner && distance2d(var_01.origin,self.origin) <= self.targettingradius && !var_01 maps\mp\_utility::func_27AF("specialty_blindeye") && !level.hardcoremode && level.teambased && var_01.team == self.team)
			{
	...
	for(;;)
	{
		...
		if(self.damagetaken >= self.maxhealth)
		{
			if(isplayer(var_01) && !isdefined(self.owner) || var_01 != self.owner)
			{
				...
				var_01 thread maps\mp\gametypes\_rank::func_2AB3("kill",300,var_09,var_04);
				var_01 thread maps\mp\gametypes\_rank::xpeventpopup(&"SPLASHES_DESTROYED_LITTLE_BIRD");
				thread maps\mp\gametypes\_missions::vehiclekilled(self.owner,self,undefined,var_01,var_00,var_04,var_09);
			}

			if(isdefined(self.owner))
			{
				self.owner thread maps\mp\_utility::func_2A60("lbguard_destroyed");
			}
...
lbsupport_empgrenaded()
{
	...
	{
		playfxontag(common_scripts\utility::func_539("ims_sensor_explode"),self.mgturretright,"tag_aim");
		playfxontag(common_scripts\utility::func_539("ims_sensor_explode"),self.mgturretleft,"tag_aim");
	}
...
//Function Number: 22
func_30D2(param_00,param_01,param_02,param_03)
{
	level endon("game_ended");
	...
	for(var_04 = 0;var_04 < param_03.size;var_04++)
	{
		if(isdefined(param_03[var_04]) && !param_02.hasdodged)
		{
			...
			var_05 thread maps\mp\killstreaks\_helicopter::func_2C55(5);
...
lbsupport_watchstingerproximity(param_00,param_01,param_02)
{
	...
	if(isdefined(self) && !param_02.hasdodged)
	{
		...
		var_03 thread maps\mp\killstreaks\_helicopter::func_2C55(5);
...
lbsupport_tu0_new_incoming_position(param_00)
{
	switch(getdvar("mapname"))
	{
		case "mp_interchange":
			...
			var_02[var_41.size][0] = (-755.1,-588.7,728);
			var_02[var_41.size][1] = (-2563.1,-1580.7,1192);
			var_02[var_41.size][0] = (-35.1,-2492.7,488);
			var_02[var_41.size][1] = (396.9,-1884.7,696);
			var_03 = randomintrange(0,100) > 50;
			var_04 = 0;
			while(var_04 < var_41.size)
			{
				if(int(var_01.origin[0]) == int(var_41[var_04][0]) && int(var_01.origin[1]) == int(var_41[var_04][1]) && int(var_01.origin[2]) == int(var_41[var_04][2]))
				{
					var_05 = 0;
