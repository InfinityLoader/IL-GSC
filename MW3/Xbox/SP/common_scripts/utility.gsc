 ...
 * Platform: Console
 * Function Count: 173
 * Decompile Time: 2902 ms
 * Timestamp: 10/27/2023 2:35:13 AM
*******************************************************************/
...
flag_wait_any_return(param_00,param_01,param_02,param_03,param_04)
{
	...
	for(;;)
	{
		...
		var_06 = level waittill_any_return(param_02,param_03,param_04,var_05,var_41);
		return var_07;
	}
}
...
brush_show()
{
	...
	if(!isdefined(self.model.var_563))
	{
		self.model show();
		self.model solid();
	}
	else
	{
		...
		var_00 setmodel(self.model.var_563);
		var_00 show();
	}

	self.brush_shown = 1;
	if(issp() && !isdefined(self.model.var_563) && self.model.spawnflags & 1)
	{
...
effect_loopsound()
{
	...
	exploder_delay();
	self.loopsound_ent = play_loopsound_in_space(var_01,var_00);
}
...
cannon_effect()
{
	...
	self.looper = spawnfx(func_0539(self.v["fxid"]),self.v["origin"],self.v["forward"],self.v["up"]);
