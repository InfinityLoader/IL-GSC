/*******************************************************************
 * Decompiled By: Bog
 * Decompiled File: maps\_audio_vehicles.gsc
 * Game: Call of Duty: Modern Warfare 3
 * Platform: Console
 * Function Count: 50
 * Decompile Time: 1159 ms
 * Timestamp: 10/27/2023 2:33:24 AM
*******************************************************************/
...
vm_init()
{
	...
	if(!isdefined(level._audio.var_118A))
	{
		level._audio.var_118A = spawnstruct();
		level._audio.var_118A.minrate = 0.1;
		level._audio.var_118A.defrate = 0.5;
		level._audio.var_118A.defsmooth = 0.1;
		level._audio.var_118A.minpitch = 0.5;
		level._audio.var_118A.maxpitch = 1.5;
		level._audio.var_118A.fadein_time = 2;
		level._audio.var_118A.callbacks = [];
		level._audio.var_118A.print_speed = 0;
		level._audio.var_118A.var_1192 = 0;
		level._audio.var_118A.print_yaw = 0;
		level._audio.var_118A.print_roll = 0;
		level._audio.var_118A.print_altitude = 0;
		level._audio.var_118A.var_1196 = 0;
		level._audio.var_118A.presets = [];
		level._audio.var_118A.maps = [];
		level._audio.var_118A.instances = [];
		level._audio.var_118A.ducked_instances = [];
		level._audio.var_118A.var_119B = [];
		level._audio.var_118A.duck_stops = [];
	}
}
...
{
	level._audio.var_118A.callbacks[param_00] = param_01;
}
...
{
	wait 0.25;
	if(isdefined(level._audio.var_118A.instances[param_00]))
	{
	...
	{
		thread maps\_audio::aud_play_linked_sound(param_04,param_02,undefined,undefined,param_05);
	}

	level._audio.var_118A.instances[param_00] = [];
	level._audio.var_118A.instances[param_00]["entity"] = param_02;
	if(!isdefined(level._audio.var_118A.presets[param_01]))
	{
		var_06 = [];
		var_06 = maps\_audio_presets_vehicles::audio_presets_vehicles(param_01,var_06);
		level._audio.var_118A.presets[param_01] = var_06;
	}

	level._audio.var_118A.instances[param_00]["entity"] = param_02;
	foreach(var_13, var_08 in level._audio.var_118A.presets[param_01])
	{
		var_09 = spawnstruct();
		...
		var_09.smoothness = level._audio.var_118A.defsmooth;
		var_09.smooth_up = undefined;
		var_09.smooth_down = undefined;
		var_09.var_11A6 = level._audio.var_118A.defrate;
		var_09.alias_data = [];
		var_09.var_11A8 = 0.5;
		if(isdefined(param_03))
		{
			var_09.var_11A8 = param_03;
		}
		...
		foreach(var_0B in var_08)
		{
			switch(var_0B[0])
			{
				...
				case "callback":
					var_0C = var_0B[1];
					var_09.custom_callback = level._audio.var_118A.callbacks[var_0C];
					break;
				...
				case "start":
					...
					for(var_0D = 2;var_0D < var_0B.size;var_0D++)
					{
						if(isarray(var_0B[var_0D]))
						{
							...
							if(!isdefined(level._audio.var_118A.maps[var_0F]))
							{
								var_10 = [];
								var_10 = maps\_audio_presets_vehicles::audio_presets_vehicle_maps(var_0F,var_10);
								level._audio.var_118A.maps[var_0F] = var_10;
							}
	
							continue;
						}
	
						level._audio.var_118A.var_119B[param_00] = var_0B[var_0D];
					}
					break;
				...
				case "stop":
					...
					for(var_0D = 2;var_0D < var_0B.size;var_0D++)
					{
						...
						level._audio.var_118A.duck_stops[param_00] = var_0B[var_0D];
					}
					break;
...
vm_stop(param_00)
{
	...
	foreach(var_03 in level._audio.var_118A.playing_presets)
	{
		if(var_03.size > 0)
		{
			foreach(var_05 in var_03)
			{
				thread maps\_audio::aud_fade_out_and_delete(var_05,var_01);
			}
		}
	}

	level._audio.var_118A.playing_presets = [];
}
...
vm_stop_preset_instance(param_00,param_01)
{
	...
	if(isdefined(level._audio.var_118A.instances[param_00]))
	{
		...
		if(level._audio.var_118A.instances[param_00].size > 0)
		{
			foreach(var_05, var_04 in level._audio.var_118A.instances[param_00])
			{
		...
		level._audio.var_118A.instances[param_00] = undefined;
	}
}
...
vmx_monitor_oneshot_ent()
{
	...
	self.sound_playing = 0;
}

//Function Number: 8
func_11C3(param_00)
{
	level common_scripts\utility::waittill_any("aud_veh_stop","aud_veh_stop_" + param_00);
	if(self.sound_playing)
	{
		self scalevolume(0,0.1);
		wait 0.1;
		self stopsounds();
		wait 0.05;
	}
...
//Function Number: 9
vmx_stop_sound(param_00)
{
	self scalevolume(0,0.05);
	wait 0.05;
	self stopsounds();
	wait 0.05;
	self.sound_playing = 0;
...
func_11C5(param_00)
{
	...
	thread vmx_monitor_oneshot_ent();
}

//Function Number: 11
vmx_init_oneshot_ents(param_00)
{
	if(!isdefined(level._audio.var_118A.start_ents))
	{
		level._audio.var_118A.start_ents = [];
	}

	if(!isdefined(level._audio.var_118A.stop_ents))
	{
		level._audio.var_118A.stop_ents = [];
	}

	if(!isdefined(level._audio.var_118A.start_ent_count))
	{
		level._audio.var_118A.start_ent_count = [];
	}

	if(!isdefined(level._audio.var_118A.var_11CA))
	{
		level._audio.var_118A.var_11CA = [];
	}

	level._audio.var_118A.start_ents[param_00] = [];
	level._audio.var_118A.start_ent_count[param_00] = 0;
	level._audio.var_118A.stop_ents[param_00] = [];
	level._audio.var_118A.var_11CA[param_00] = 0;
}
...
{
	var_01 = 0;
	if(isdefined(level._audio.var_118A.var_119B[param_00]) && level._audio.var_118A.var_119B[param_00] && func_11CC(param_00))
	{
		var_01 = 1;
	}

	if(isdefined(level._audio.var_118A.duck_stops[param_00]) && level._audio.var_118A.duck_stops[param_00] && vmx_get_stop_sound_playing(param_00))
	{
		var_01 = 1;
	}
...
func_11CC(param_00)
{
	if(level._audio.var_118A.start_ents[param_00].size > 0)
	{
		return 1;
	}
...
{
	if(level._audio.var_118A.stop_ents[param_00].size > 0)
	{
		return 1;
	}
...
{
	foreach(var_04 in level._audio.var_118A.start_ents[param_02])
	{
		if(isdefined(var_04))
		{
...
{
	foreach(var_04 in level._audio.var_118A.stop_ents[param_02])
	{
		if(isdefined(var_04))
		{
			var_04 setpitch(param_00,param_01);
...
//Function Number: 19
func_11D2(param_00,param_01,param_02,param_03)
{
	var_04 = 0;
	if(isdefined(param_03))
	{
	...
	var_05.ref = level._audio.var_118A.start_ent_count[param_02];
	var_05 playsound(param_00,"sounddone");
	var_05 thread vmx_monitor_start_ent(param_02);
	level._audio.var_118A.start_ents[param_02][var_05.ref] = var_05;
	level._audio.var_118A.start_ent_count[param_02]++;
}
...
vmx_play_stop_sound(param_00,param_01,param_02,param_03)
{
	...
	var_05 = spawn("script_origin",param_01.origin);
	var_05 linkto(param_01,"tag_origin",(var_04,0,0),(0,0,0));
	var_05.ref = level._audio.var_118A.var_11CA[param_02];
	var_05 playsound(param_00,"sounddone");
	level._audio.var_118A.stop_ents[param_02][var_05.ref] = var_05;
	level._audio.var_118A.var_11CA[param_02]++;
	wait 0.05;
...
vmx_monitor_start_ent(param_00)
{
	self endon("kill");
	self waittill("sounddone");
	level._audio.var_118A.start_ents[param_00][self.ref] = undefined;
	if(isdefined(level._audio.var_118A.ducked_instances[param_00]))
	{
		level._audio.var_118A.ducked_instances[param_00] = undefined;
	}
...
vmx_monitor_stop_ent(param_00)
{
	self endon("kill");
	self waittill("sounddone");
	level._audio.var_118A.stop_ents[param_00][self.ref] = undefined;
	if(isdefined(level._audio.var_118A.ducked_instances[param_00]))
	{
		level._audio.var_118A.ducked_instances[param_00] = undefined;
	}
...
{
	if(isdefined(level._audio.var_118A.ducked_instances[param_02]))
	{
		level._audio.var_118A.ducked_instances[param_02] = undefined;
	}
	...
	level._audio.var_118A.stop_ents[param_02][param_00.ref] = undefined;
...
vmx_stop_start_ent(param_00,param_01,param_02)
{
	...
	level._audio.var_118A.start_ents[param_02][param_00.ref] = undefined;
	param_00 scalevolume(0,var_03 + 0.05);
	param_00 notify("kill");
	wait var_03 + 0.05;
	param_00 stopsounds();
...
vmx_do_start_stop_callback(param_00)
{
	...
	level._audio.var_118A.instances[var_01]["entity"] endon("death");
	var_02 = 0;
	var_03 = 0;
	var_04 = -1;
	var_05 = "off";
	...
	for(;;)
	{
		...
		if((var_0E >= param_00.on_threshold || var_0B >= 0.99) && var_05 == "off" && var_10 > 200)
		{
			...
			var_11 = 1;
			wait 0.05;
			var_13 = level._audio.var_118A.instances[var_01]["entity"];
			thread func_11D2(param_00.start_alias_data.name,var_13,param_00.instance_name,param_00.offset);
			if(isdefined(level._audio.var_118A.stop_ents[param_00.instance_name]))
			{
				var_14 = level._audio.var_118A.stop_ents[param_00.instance_name];
		...
		else if((var_0E <= param_00.off_threshold || var_0B <= 0.01) && var_05 == "on" && var_10 > 200)
		{
			...
			if(isdefined(level._audio.var_118A.start_ents[param_00.instance_name]))
			{
				var_17 = level._audio.var_118A.start_ents[param_00.instance_name];
		...
		var_1A = undefined;
		var_1B = undefined;
		if(func_11CC(param_00.instance_name))
		{
			if(var_11)
			{
				var_11 = 0;
				...
				{
					var_1A = maps\_audio::func_117B(var_02,level._audio.var_118A.maps[param_00.start_alias_data.pitch_map_name]);
					var_1A = level._audio.var_118A.minpitch + var_1A * level._audio.var_118A.maxpitch - level._audio.var_118A.minpitch;
					vmx_scale_start_sound_pitch(var_1A,param_00.var_11A6,param_00.instance_name);
				}
			}

			if(isdefined(param_00.start_alias_data.vol_map_name))
			{
				var_1B = maps\_audio::func_117B(var_02,level._audio.var_118A.maps[param_00.start_alias_data.vol_map_name]);
				vmx_scale_start_sound_volume(var_1B,param_00.var_11A6,param_00.instance_name);
			}
		}
		...
		if(vmx_get_stop_sound_playing(param_00.instance_name))
		{
			if(var_12)
			{
				...
				{
					var_1A = maps\_audio::func_117B(var_02,level._audio.var_118A.maps[param_00.stop_alias_data.pitch_map_name]);
					var_1A = level._audio.var_118A.minpitch + var_1A * level._audio.var_118A.maxpitch - level._audio.var_118A.minpitch;
					vmx_scale_stop_sound_pitch(var_1A,param_00.var_11A6,param_00.instance_name);
				}
			}

			if(isdefined(param_00.stop_alias_data.vol_map_name))
			{
				var_1B = maps\_audio::func_117B(var_02,level._audio.var_118A.maps[param_00.stop_alias_data.vol_map_name]);
				vmx_scale_stop_sound_volume(var_1B,param_00.var_11A6,param_00.instance_name);
			}
		}

		var_04 = var_02;
		wait param_00.var_11A6;
	}
}
...
vmx_callback(param_00)
{
	...
	{
		var_08 = level._audio.var_118A.instances[var_01]["entity"];
	...
	for(;;)
	{
		...
		var_0A = 0;
		var_0B = 0;
		if(isdefined(var_08.var_11DB))
		{
			var_0A = var_08.var_11DB;
		...
		if(isdefined(param_00.multiply_by_throttle))
		{
			...
			if(level._audio.var_118A.var_1196)
			{
		...
		{
			thread func_11E0(var_0F,param_00,var_02,var_01,var_09);
		}

		if(var_09)
		{
			var_09 = 0;
			wait param_00.var_11A8;
			continue;
		}
...
//Function Number: 29
func_11E0(param_00,param_01,param_02,param_03,param_04)
{
	var_05 = undefined;
	var_06 = undefined;
	if(isdefined(param_00.pitch_map_name))
	{
		var_05 = maps\_audio::func_117B(param_02,level._audio.var_118A.maps[param_00.pitch_map_name]);
		var_05 = level._audio.var_118A.minpitch + var_05 * level._audio.var_118A.maxpitch - level._audio.var_118A.minpitch;
	}

	if(isdefined(param_00.vol_map_name))
	{
		var_06 = maps\_audio::func_117B(param_02,level._audio.var_118A.maps[param_00.vol_map_name]);
	}

	var_07 = 0;
	var_08 = 0;
	if(isdefined(level._audio.var_118A.ducked_instances[param_03]))
	{
	...
	if(!var_07)
	{
		if(!isdefined(level._audio.var_118A.ducked_instances[param_03]) && func_11CB(param_03))
		{
			var_07 = 1;
			level._audio.var_118A.ducked_instances[param_03] = gettime();
		}
	}
	...
	if(!isdefined(level._audio.var_118A.instances[param_03][param_00.alias_name]))
	{
		level._audio.var_118A.instances[param_03][param_00.alias_name] = spawn("script_origin",param_01.vehicle.origin);
		var_0F = 0;
		...
		level._audio.var_118A.instances[param_03][param_00.alias_name] linkto(param_01.vehicle,"tag_origin",(var_0F,0,0),(0,0,0));
		level._audio.var_118A.instances[param_03][param_00.alias_name] playloopsound(param_00.alias_name);
		level._audio.var_118A.instances[param_03][param_00.alias_name] scalevolume(0);
		wait 0.05;
		level._audio.var_118A.instances[param_03][param_00.alias_name] scalevolume(var_06,param_01.var_11A8);
		return;
	}

	if(isdefined(var_05))
	{
		level._audio.var_118A.instances[param_03][param_00.alias_name] setpitch(var_05,param_01.var_11A6);
	}

	if(isdefined(var_06))
	{
		level._audio.var_118A.instances[param_03][param_00.alias_name] scalevolume(var_06,param_01.var_11A6);
	}
}

//Function Number: 30
func_11E1(param_00,param_01,param_02,param_03)
{
	var_04 = "tag_origin";
	...
	if(isdefined(level._audio.var_118A.instances[param_00]))
	{
		foreach(var_08, var_07 in level._audio.var_118A.instances[param_00])
		{
		...
		level._audio.var_118A.instances[param_00]["entity"] = param_01;
	}
}

//Function Number: 31
func_11E2(param_00,param_01)
{
	if(isdefined(level._audio.var_118A.instances[param_00]))
	{
		level._audio.var_118A.instances[param_00]["speed"] = param_01;
	}
}

//Function Number: 32
func_11E3(param_00,param_01)
{
	if(isdefined(level._audio.var_118A.instances[param_00]))
	{
		level._audio.var_118A.instances[param_00]["throttle"] = param_01;
	}
}

//Function Number: 33
func_11E4(param_00,param_01)
{
	if(isdefined(level._audio.var_118A.instances[param_00]))
	{
	}
...
func_11E5(param_00)
{
	var_01 = level._audio.var_118A.instances[param_00.instance_name]["entity"];
	var_02 = var_01.angles[0];
	if(level._audio.var_118A.var_1192)
	{
...
func_11E6(param_00)
{
	var_01 = level._audio.var_118A.instances[param_00.instance_name]["entity"];
	var_02 = 0;
	if(isdefined(level._audio.var_118A.instances[param_00.instance_name]["speed"]))
	{
		var_03 = level._audio.var_118A.instances[param_00.instance_name]["speed"];
	...
	if(level._audio.var_118A.print_speed)
	{
...
vmx_get_roll(param_00)
{
	...
	if(level._audio.var_118A.print_roll)
	{
...
vmx_get_throttle(param_00)
{
	var_01 = level._audio.var_118A.instances[param_00.instance_name]["entity"];
	var_02 = 0;
	if(isdefined(level._audio.var_118A.instances[param_00.instance_name]["throttle"]))
	{
		var_03 = level._audio.var_118A.instances[param_00.instance_name]["speed"];
		var_02 = var_01 [[ var_03 ]]();
	}
...
{
	self endon("death");
	self.var_11EC = spawnstruct();
	self.var_11EC.move_lo_lp = param_00;
	self.var_11EC.rolling_lp = param_01;
	self.var_11EC.var_11EF = param_02;
	self.var_11EC.engine_rev_lo_os = param_03;
	self.var_11EC.breaks_os = param_04;
...
vmx_ground_speed_watch(param_00)
{
	...
	for(;;)
	{
		...
		if(var_05 >= var_04)
		{
			if(isdefined(self.var_11EC.var_11EF) && self.var_11F5)
			{
			...
			{
				self.do_rev = 0;
				if(isdefined(self.var_11EC.engine_rev_lo_os))
				{
					self.veh_mix_ents.one_shot playsound(self.var_11EC.engine_rev_lo_os);
				}
			}
			else
			{
				if(isdefined(self.var_11EC.rolling_lp) && !self.has_roll_played)
				{
					self.has_roll_played = 1;
					self.veh_mix_ents.roll_ent playloopsound(self.var_11EC.rolling_lp);
				}

				if(isdefined(self.var_11EC.move_lo_lp) && !self.has_move_played)
				{
					self.has_move_played = 1;
					self.veh_mix_ents.move_ent playloopsound(self.var_11EC.move_lo_lp);
				}

				if(!var_03)
				{
					if(isdefined(self.var_11EC.move_lo_lp))
					{
						self.veh_mix_ents.move_ent thread vmx_aud_ent_fade_in(var_01);
					}

					if(isdefined(self.var_11EC.rolling_lp))
					{
		...
		{
			if(isdefined(self.var_11EC.var_11EF))
			{
				if(!self.var_11F5)
				{
					self.var_11F5 = 1;
					self.veh_mix_ents.idle_ent playloopsound(self.var_11EC.var_11EF);
				}

				self.veh_mix_ents.idle_ent thread vmx_aud_ent_fade_in(0.5);
			}

			if(isdefined(self.var_11EC.breaks_os))
			{
				self.veh_mix_ents.one_shot playsound(self.var_11EC.breaks_os);
			}
