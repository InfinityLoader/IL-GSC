 ...
 * Timestamp: 10/27/2023 2:35:28 AM
*******************************************************************/
...
main()
{
	...
	var_09 = 25;
	level.var_197E["orange vase"] = 1;
	level.var_197E["green vase"] = 1;
	level.var_197E["bottle"] = 1;
	...
	common_scripts\utility::array_thread(getentarray("tincan","targetname"),::func_19A5);
	...
	common_scripts\utility::array_thread(getentarray("glass","targetname"),::func_1995);
	common_scripts\utility::array_thread(getentarray("interactive_tv","targetname"),::func_198A);
...
{
	self setcandamage(1);
	self.var_198B = undefined;
	self.var_198C = undefined;
	self.var_198B = "com_tv2_d";
	self.var_198C = "com_tv2";
	self.onmodel = "com_tv2_testpattern";
	if(issubstr(self.model,"1"))
	{
		self.var_198C = "com_tv1";
	...
	var_01 = maps\_utility::get_array_of_closest(self.origin,level.tv_lite_array,undefined,undefined,64);
	if(var_01.size)
	{
		self.lite = var_01[0];
...
func_1995()
{
	...
	func_1997(self getorigin(),self.destructible_type,var_00);
	self delete();
}

//Function Number: 7
func_1997(param_00,param_01,param_02)
{
...
oil_spill_think()
{
	self.var_1001 = common_scripts\utility::getstruct(self.target,"targetname");
	self.start = common_scripts\utility::getstruct(self.var_1001.target,"targetname");
	self.barrel = getclosestent(self.start.origin,getentarray("explodable_barrel","targetname"));
	if(isdefined(self.barrel))
	{
		self.barrel.var_199A = 1;
		thread func_199C();
	}
	...
	for(;;)
	{
		...
		var_03 = pointonsegmentnearesttopoint(self.start.origin,self.var_1001.origin,var_03);
		thread oil_spill_burn_section(var_03);
		thread oil_spill_burn(var_03,self.start.origin);
		thread oil_spill_burn(var_03,self.var_1001.origin);
		break;
	}
...
explodable_barrel_think()
{
	...
	if(!isdefined(level.var_1980["com_barrel_benzin"]))
	{
		level.var_1980["com_barrel_benzin"] = 1;
...
explodable_barrel_explode()
{
	...
	if(isdefined(self.var_19A2))
	{
		if(common_scripts\utility::issp() && self.var_19A2.spawnflags & 1)
		{
			self.var_19A2 connectpaths();
		}

		self.var_19A2 delete();
	}
...
helmet_logic()
{
	...
	var_03 = vectornormalize(self.origin - var_02);
	if(!isdefined(self.var_19A8) && isplayer(var_01))
	{
...
breakable_think()
{
	...
	{
		if(!isdefined(level.var_1980["egypt_prop_vase_o"]))
		{
			level.var_1980["egypt_prop_vase_o"] = 1;
	...
	{
		if(!isdefined(level.var_1980["egypt_prop_vase_g"]))
		{
			level.var_1980["egypt_prop_vase_g"] = 1;
	...
	{
		if(!isdefined(level.var_1980["prop_crate_dak_shard"]))
		{
			level.var_1980["prop_crate_dak_shard"] = 1;
	...
	{
		if(!isdefined(level.var_1980["prop_winebottle"]))
		{
			level.var_1980["prop_winebottle"] = 1;
	...
	{
		if(!isdefined(level.var_1980["prop_diningplate_brokenfloral"]))
		{
			level.var_1980["prop_diningplate_brokenfloral"] = 1;
	...
	{
		if(!isdefined(level.var_1980["prop_diningplate_brokenplain"]))
		{
			level.var_1980["prop_diningplate_brokenplain"] = 1;
...
func_19B4(param_00)
{
	...
	self.autoaim thread func_19B5(param_00);
	self.var_19A8 = 1;
...
func_19B5(param_00)
{
	...
	self.wait_message = 0;
	if(isdefined(self.var_19B7) && self.var_19B7 == 1)
	{
...
breakable_clip()
{
	...
	{
		self.var_19A2 = getclosestent(self.origin,level.breakables_clip);
	}

	if(isdefined(self.var_19A2))
	{
		level.breakables_clip = common_scripts\utility::array_remove(level.breakables_clip,self.var_19A2);
	}
}
...
make_broken_peices(param_00,param_01)
{
	...
	addpiece(var_03,var_04,var_05,-10,10,25,param_01,(0,0,0),"prop_crate_dak_shard")
		...
		case "bottle":
			var_41[var_41.size - 1].type = "bottle_top";
			var_41[var_41.size - 1].type = "bottle_bot";
			break;
	var_41[var_41.size]
	...
	addpiece(var_03,var_04,var_05,0,0,10,param_01,(0,0,0),"prop_winebottle_broken_top")
		...
		case "plate":
			...
			{
				case "round_floral":
					var_41[var_41.size - 1].type = "plate";
					var_41[var_41.size - 1].type = "plate";
					var_41[var_41.size - 1].type = "plate";
					var_41[var_41.size - 1].type = "plate";
					break;
	...
	common_scripts\utility::array_thread(var_41,::pieces_move,param_01.origin);
	if(isdefined(level.var_197E[var_02]) && level.var_197E[var_02] == 1)
	{
		var_06 = var_41[0].origin[2];
		for(var_07 = 0;var_07 < var_41.size;var_07++)
		{
			if(var_06 > var_41[var_07].origin[2])
			{
				var_06 = var_41[var_07].origin[2];
			}
		}

		common_scripts\utility::array_thread(var_41,::func_19BC,var_06);
		return;
	}
	...
	for(var_07 = 0;var_07 < var_41.size;var_07++)
	{
		if(isdefined(var_41[var_07]))
		{
			var_41[var_07] delete();
		}
	}
}
...
list_add(param_00)
{
	if(isdefined(level._breakable_utility_modelarray[level.var_1987]))
	{
		level._breakable_utility_modelarray[level.var_1987] delete();
	}

	level._breakable_utility_modelarray[level.var_1987] = param_00;
	level.var_1987++;
	if(!level.var_1987 < level._breakable_utility_maxnum)
	{
		level.var_1987 = 0;
	}
}
