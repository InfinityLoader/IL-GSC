 ...
 * Platform: Console
 * Function Count: 72
 * Decompile Time: 1303 ms
 * Timestamp: 10/27/2023 2:32:37 AM
*******************************************************************/
...
func_0F05(param_00)
{
	...
	level.var_F06 = [];
	level.var_F06["axis"] = [];
	level.var_F06["allies"] = [];
	level.var_F07 = [];
	level.var_F07["axis"] = [];
	level.var_F07["allies"] = [];
	var_01 = [];
	...
	level.var_F08 = [];
	level.var_F08["allies"] = "allies";
	level.var_F08["axis"] = "axis";
	level.var_F08["team3"] = "axis";
	level.var_F08["neutral"] = "neutral";
	...
	level.var_F0C = [];
	level.var_F0C[level.var_F0C.size] = "r";
	level.var_F0C[level.var_F0C.size] = "b";
	level.var_F0C[level.var_F0C.size] = "y";
	level.var_F0C[level.var_F0C.size] = "c";
	level.var_F0C[level.var_F0C.size] = "g";
	level.var_F0C[level.var_F0C.size] = "p";
	level.var_F0C[level.var_F0C.size] = "o";
	...
	foreach(var_0D in level.var_F0C)
	{
...
goto_current_colorindex()
{
	...
	var_00 = level.var_F06[get_team()][self.currentcolorcode];
	func_0F5E();
	if(!isalive(self))
	{
		return;
	...
	if(!isdefined(var_00))
	{
		var_01 = level.var_F07[get_team()][self.currentcolorcode];
	...
	for(var_02 = 0;var_02 < var_01.size;var_02++)
	{
		...
		if(isalive(var_03.var_F18) && !isplayer(var_03.var_F18))
		{
			continue;
		}
...
{
	return func_0F1D(param_00,param_01);
}

//Function Number: 9
func_0F1D(param_00,param_01)
{
...
{
	if(isdefined(level.var_F06[param_00][param_01]))
	{
		return 1;
	}

	return isdefined(level.var_F07[param_00][param_01]);
}
...
trigger_issues_orders(param_00,param_01)
{
	...
	for(;;)
	{
		...
		if(isdefined(self.var_F21) && self.var_F21)
		{
...
activate_color_code_internal(param_00,param_01,param_02,param_03)
{
	...
	for(var_04 = 0;var_04 < param_00.size;var_04++)
	{
		...
		var_0B[var_0C] = func_0F37(var_0C,param_01[var_04],param_02);
	}
...
{
	var_03 = level.var_F06[param_00][param_01];
	var_04 = spawnstruct();
	var_04.path_nodes = [];
	...
	level.var_F06[param_00][param_01] = var_03;
}

//Function Number: 23
func_0F35(param_00,param_01,param_02)
{
	return level.var_F06[param_00][param_01];
}
...
func_0F37(param_00,param_01,param_02)
{
	...
	common_scripts\utility::array_thread(var_03,::func_0F5E);
	return var_03;
}
...
issue_color_order_to_ai(param_00,param_01,param_02,param_03)
{
	var_04 = param_03;
	var_05 = [];
	if(isdefined(level.var_F06[param_02][param_00]))
	{
...
take_color_node(param_00,param_01,param_02,param_03)
{
	self notify("stop_color_move");
	self.currentcolorcode = param_01;
	thread func_0F52(param_00,param_02,param_03);
}
...
player_color_node()
{
	for(;;)
	{
		...
		var_01 = level.player.node.var_F18;
		var_00 = level.player.node;
		var_00.var_F18 = level.player;
		for(;;)
		{
			if(!isdefined(level.player.node))
			{
...
color_node_finds_user_for_colorcode(param_00,param_01)
{
	...
	var_03 = maps\_utility::func_F3F(param_01,var_02);
...
set_goal_and_volume(param_00)
{
	...
	if(isdefined(level.var_F48))
	{
		self.fixednodesaferadius = level.var_F48;
		return;
	}
...
func_0F52(param_00,param_01,param_02)
{
	...
	for(;;)
	{
		...
		{
			if(isalive(self.color_node.var_F18) && self.color_node.var_F18 == self)
			{
				self.color_node.var_F18 = undefined;
			}

			self.color_node = param_00;
			param_00.var_F18 = self;
			ai_sets_goal(param_00);
		}
	}
}
...
get_best_available_colored_node()
{
	...
	var_01 = func_0F35(get_team(),var_00,self.script_forcecolor);
	for(var_02 = 0;var_02 < var_01.size;var_02++)
	{
		if(!isalive(var_01[var_02].var_F18))
		{
...
{
	for(var_01 = 0;var_01 < level.var_F0C.size;var_01++)
	{
		if(param_00 == level.var_F0C[var_01])
		{
			return 1;
		}
	}
...
add_volume_to_global_arrays(param_00,param_01)
{
	...
	{
		level.var_F07[param_01][var_04] = self;
...
{
	self.var_F18 = undefined;
	var_02 = strtok(param_00," ");
	var_02 = array_remove_dupes(var_02);
	foreach(var_04 in var_02)
	{
		if(isdefined(level.var_F06[param_01]) && isdefined(level.var_F06[param_01][var_04]))
		{
			level.var_F06[param_01][var_04] = maps\_utility::array_add(level.var_F06[param_01][var_04],self);
			continue;
		}

		level.var_F06[param_01][var_04][0] = self;
		level.arrays_of_colorcoded_ai[param_01][var_04] = [];
		level.arrays_of_colorcoded_spawners[param_01][var_04] = [];
	}
}
...
func_0F5E()
{
	...
	if(isdefined(self.color_node.var_F18) && self.color_node.var_F18 == self)
	{
		self.color_node.var_F18 = undefined;
	}
...
{
	level.color_node_type_function[param_00][1]["allies"] = ::func_0F31;
	level.color_node_type_function[param_00][0]["allies"] = ::func_0F31;
	level.color_node_type_function[param_00][1]["axis"] = ::func_0F31;
	level.color_node_type_function[param_00][0]["axis"] = ::func_0F31;
}

//Function Number: 60
func_0F63(param_00,param_01)
{
	...
	if(isdefined(level.var_F64))
	{
		var_02 thread [[ level.var_F64 ]]();
	}
...
colornode_replace_on_death()
{
	...
	thread func_0F63(var_00,self.script_forcecolor);
	...
	for(;;)
	{
		...
		var_04 = maps\_utility::func_F3F("allies",var_02[var_01]);
...
spawn_hidden_reinforcement(param_00,param_01)
{
	...
	for(;;)
	{
		...
		if(maps\_utility::func_F77(var_02))
		{
