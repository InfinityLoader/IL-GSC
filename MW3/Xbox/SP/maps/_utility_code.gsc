 ...
 * Timestamp: 10/27/2023 2:33:40 AM
*******************************************************************/
...
waitspread_code(param_00,param_01)
{
	...
	{
		level.wait_spreader_allotment = var_41;
	...
	{
		var_41 = waitspread_insert(var_41);
		var_03++ = var_41[var_41.size];
	}

	level.wait_spreader_allotment = maps\_utility::array_randomize(var_41);
	level.active_wait_spread = undefined;
}
...
update_debug_friendlycolor_on_death()
{
	...
	self waittill("death");
	level.var_F92[var_00] = undefined;
...
update_debug_friendlycolor(param_00)
{
	...
	{
		level.var_F92[param_00] = self.script_forcecolor;
	}
	else
	{
		level.var_F92[param_00] = undefined;
	}
...
hintprint(param_00,param_01,param_02,param_03,param_04,param_05)
{
	...
	maps\_utility::func_FA3("global_hint_in_use");
	if(isdefined(self.current_global_hint))
	{
		if(self.current_global_hint == param_00)
		{
...
lerp_player_view_to_tag_oldstyle_internal(param_00,param_01,param_02,param_03,param_04,param_05,param_06,param_07,param_08)
{
	...
	var_09 = self gettagorigin(param_01);
	var_0A = self gettagangles(param_01);
	param_00 maps\_utility::func_FAC(var_09,var_0A,param_02,param_03,param_04,param_05,param_06,param_07,1);
	if(param_08)
	{
		return;
	}
...
{
	self endon("death");
	if(!isdefined(param_00.var_E1D))
	{
		param_00.var_E1D = [];
	}

	param_00.var_E1D[param_00.var_E1D.size] = self;
	thread function_stack_self_death(param_00);
	func_0FB2(param_00);
	if(isdefined(param_00) && isdefined(param_00.var_E1D))
	{
		...
		if(isdefined(param_00) && isdefined(param_00.var_E1D))
		{
			param_00.var_E1D = common_scripts\utility::array_remove(param_00.var_E1D,self);
...
func_0FBB(param_00)
{
	...
	{
		param_00.var_FBD [[ param_00.func ]]();
	}
	else if(param_00.parms.size == 1)
	{
		param_00.var_FBD [[ param_00.func ]](param_00.parms[0]);
	}
	else if(param_00.parms.size == 2)
	{
		param_00.var_FBD [[ param_00.func ]](param_00.parms[0],param_00.parms[1]);
	}
	else if(param_00.parms.size == 3)
	{
		param_00.var_FBD [[ param_00.func ]](param_00.parms[0],param_00.parms[1],param_00.parms[2]);
	}

	if(param_00.parms.size == 4)
	{
		param_00.var_FBD [[ param_00.func ]](param_00.parms[0],param_00.parms[1],param_00.parms[2],param_00.parms[3]);
	}

	if(param_00.parms.size == 5)
	{
		param_00.var_FBD [[ param_00.func ]](param_00.parms[0],param_00.parms[1],param_00.parms[2],param_00.parms[3],param_00.parms[4]);
	}
}
...
exec_func(param_00,param_01)
{
	...
	{
		param_01[var_02].var_FBD endon(param_01[var_02].ender);
	}
...
dynamic_run_speed_proc(param_00,param_01,param_02,param_03,param_04)
{
	...
	{
		maps\_utility::func_FA3("_stealth_custom_anim");
	}
	...
	self.var_FCB = "";
	self.var_FCC = self.moveplaybackrate;
	thread func_0FD1();
	...
	for(;;)
	{
		...
		if(isdefined(self.cqbwalking) && self.cqbwalking && !self.var_FD0)
		{
...
func_0FD1()
{
	...
	if(!self.var_FD0)
	{
		self.moveplaybackrate = self.var_FCC;
	}
...
{
	if(self.var_FCB == param_00)
	{
		return;
	}

	self.var_FCB = param_00;
	switch(param_00)
	{
		case "sprint":
			if(isdefined(self.cqbwalking) && self.cqbwalking && !self.var_FD0)
...
dynamic_run_speed_stopped()
{
	...
	self endon("stop_dynamic_run_speed");
	maps\_utility::func_FA4("dynamic_run_speed_stopping");
	maps\_utility::func_FA4("dynamic_run_speed_stopped");
	...
	while(maps\_utility::ent_flag("dynamic_run_speed_stopped"))
	{
		...
		{
			maps\_utility::func_FA3("_stealth_stance_handler");
		}
		...
		if(isdefined(level.dynamic_run_speed_dialogue))
		{
			...
			level thread maps\_utility::func_FE1(var_02);
		}
...
autosave_tactical_proc()
{
	...
	level thread maps\_utility::func_FEC("kill_save",5);
...
kill_deathflag_proc(param_00)
{
	...
	playfxontag(common_scripts\utility::func_539("flesh_hit"),self,"tag_eye");
...
process_blend(param_00,param_01,param_02,param_03,param_04)
{
	...
	var_05 = self.time * 20;
	var_06 = self.var_1001 - self.start;
	if(isdefined(param_04))
	{
